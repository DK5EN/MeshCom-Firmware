...
| File              | What it does                                                                   |
| ----------------- | ------------------------------------------------------------------------------ |
| `paths/*.txt`     | the chapter cut: which of the 774 paths belongs to which commit                |
...
`K01 CORE K15 K16 K17` are the code cut, 304 paths, disjoint and complete
against `git diff --no-renames upstream/dev fork-neo-test` over
`src lib variants config platformio.ini`. `K19.txt` (470 paths, `test/`) is
...
- **`--no-overlay`.** `git checkout <tree> -- <paths>` deletes nothing in its
  default overlay mode. 78 of the 304 paths are deletions.
