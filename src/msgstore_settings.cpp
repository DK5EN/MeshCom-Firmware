...
static void msgstoreLoadPlatform(uint8_t *mode, uint8_t *slots, uint16_t *hold_h, char *list, size_t list_n)
{
...
static void msgstoreSavePlatform(uint8_t mode, uint8_t slots, uint16_t hold_h, const char *list)
{
    s_msgstore_prefs.begin("Credentials", false);
...
// 'M','B','X','1' packed as a little-endian uint32 -- the struct is written
// and read raw (memcpy-style), same as nrf52_flash.cpp does for the whole
// settings struct, so this only ever has to round-trip on the same CPU.
#define MSGSTORE_FILE_MAGIC (((uint32_t)'M') | ((uint32_t)'B' << 8) | ((uint32_t)'X' << 16) | ((uint32_t)'1' << 24))
...
static void msgstoreLoadPlatform(uint8_t *mode, uint8_t *slots, uint16_t *hold_h, char *list, size_t list_n)
{
    ...
    if (s_msgstore_file)
    {
        ...
        if (stored_size == sizeof(rec))
        {
            ...
            ok = (rec.magic == MSGSTORE_FILE_MAGIC);
        }
...
void msgstoreSettingsLoad(void)
{
    ...
    msgstoreLoadPlatform(&mode_raw, &slots, &hold_h, list, sizeof(list));
