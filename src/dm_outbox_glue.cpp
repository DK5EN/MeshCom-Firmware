...
//
// Ported from fork-main onto feature-snf (docs/snf-port-campaign.md).
// aprsMessage's msg_source_path/msg_destination_path/msg_destination_call/
// msg_payload are char[] on this branch, not Arduino String -- every field
// write below goes through mc_text.h (mcSet()) or snprintf(), the same
// idiom sendMessage() itself uses for a DM (loop_functions.cpp).
// addTxRingEntryOnce() here carries the neighbour-matrix kind/need/alone
// trailing params (loop_functions.h); this call uses their defaults
// (kind=0 == RING_KIND_OTHER, need/alone=nullptr) -- an outbox ladder slot
// is not a relay slot.
#include "mc_text.h"
#include "Arduino.h"
...
static bool glueTransmit(const struct DmOutboxEntry *e, uint32_t msg_id)
{
    ...
    // "{NNN" re-attached on transmit, same snprintf idiom sendMessage() uses
    // for its own ack-suffix frame (loop_functions.cpp).
    snprintf(m.msg_payload, sizeof(m.msg_payload), "%s{%03u", e->payload, (unsigned)e->nnn);
    ...
    // kind/need/alone default (RING_KIND_OTHER, no mask): an outbox ladder
    // slot is not a neighbour-matrix relay slot.
