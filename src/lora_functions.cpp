        ...
        else
        {
            ...
            */
            ...
            if(!is_equ(aprsmsg.msg_source_last.c_str(), meshcom_settings.node_call))
            {
                ...
                // server-flag guard -- on the wire the 0x80 bit only means a
                // server-connected gateway touched the copy, and a gateway
                // that relayed or emitted this frame already appended its
                // own call to the path (udp_functions.cpp:356-359,
                // lora_functions.cpp:1577/1598), which fails the comma test
                // above on its own. The guard was redundant here.
                if(aprsmsg.msg_source_path.indexOf(',') < 0)
                    ...
                    if ((msg_type_b_lora == MSG_TYPE_TEXT || msg_type_b_lora == MSG_TYPE_POSITION || msg_type_b_lora == MSG_TYPE_HEY))
                    {
                        ...
                        if(msg_type_b_lora == MSG_TYPE_TEXT)    // text message store&forward
                        {
                            ...
                            else
                            {
                                ...
                                // server-connected gateway touched the copy (relay sets it at
                                // :1598, the emitting gateway appends its own call at
                                // udp_functions.cpp:356-359). Excluding those frames excluded
                                // every gateway-relayed or app-originated DM, which is
                                // exactly the traffic a mailbox exists for.
                                //
