...
void deliver_scaffold(bool bget_password)
{
    ...
    web_client.println(".tablewrap {overflow-x:auto;max-width:100%;margin-right:10px;}\n");
    ...
    // right after Messages. Non-eligible boards never emit this button (:666-720 guards the
    // page it points at, so a stray click would 404 anyway, but there is no click to have).
 ...
 * concatenation, matching the rest of this file's per-row discipline (advisor m10).
 */
...
void sub_page_mailbox()
{
    ...
    for (int slot = 0; slot < nslots; slot++)
    {
        ...
        int32_t heard_ms = mheardAgeMs(e->dst);
        bool stale = false;
        if (heard_ms < 0)
        {
