...
Progress: P0.1 32-env clean build and baseline running; P0.6 and P0.7 closed (the stub server
already existed as `tools/mock/meshcom_server.py` and was extended; `test/golden/normalize.py`
written). No product code has been touched — phase 0 is tooling and corpora only.
...
| P0.4 | Corpus assembly: (a) LoRa frames from `test/support/gwflood_frames.txt` and the `test_aprs_corpus` vectors, (b) UDP 1990 datagrams: GATE frames wrapping (a), one BEAT, one CONF with tags 0x00..0x04, malformed cases (odd length, zero-flood, oversize), (c) EXTUDP inbound JSON set from `test_getextern`, (d) command script: every command name from the ladder with canonical, out-of-range, non-numeric and the adversarial prefix set; destructive commands in a separate manual list                                                                                                                                                                      | `test/golden/corpus/{lora,udp1990,extudp,commands}/`          | 1.5 d           |
