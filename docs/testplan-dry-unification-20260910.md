...
Status: **in execution since 2026-09-11** on branch `dry-unification`. It plans the evidence chain
around the one-shot refactor described in `docs/optimization-audit-20260910.md` section 5.1
(structural) and 5.2 (helpers).

Progress: **every phase 0 deliverable is written.** Two are not fully exercised and the row
says which: `P0.3` has node backups for 2 of the 4 bench nodes, and `P0.8`'s BLE client is
written but its validation run is blocked on the macOS Bluetooth permission for Terminal.app.
No tool is missing; two runs are owed. P0.1 base built and tagged
`dry-base-20260911` (32 envs, baseline refreshed, all 12 native suites green = the N0 row);
P0.4 corpora generated; P0.5 lint written and passing; P0.6 stub server extended (it already
existed); P0.7 normalizer written. Gate for the whole tool set: `sh test/golden/selftest.sh`.
P0.3 done for RAK-90 and Heltec-93 (the two other nodes are still owed).
P0.8 client written; its validation against the two nodes is blocked on the macOS Bluetooth
permission for Terminal.app.
Open: no phase-0 tool. `P0.2` closed 2026-09-11, `P0.9` closed 2026-09-12; what remains of
`P0.3`/`P0.8` is bench time, not code.
No product code has been touched — phase 0 is tooling and corpora only.

Correction carried into this revision: the mesh/server UDP surface is port **1990**, not 1799.
1799 is the EXTUDP sideband. See section 10.
...
| Bench              | All four bench nodes may be reflashed; settings backed up and restored via config JSON; a local stub server drives UDP 1990                                          |
...
| P0.3 | ~~Backup bench node settings via `--export` config JSON~~ **DONE 2026-09-11 for RAK-90 and Heltec-93.** There is no `--export` command; the export is HTTP only (`GET /config.json`, `web_functions.cpp:656`). It carries the WiFi, AP and web passwords and the BLE pairing code in plaintext, so `test/golden/backup_nodes.py` writes the restorable copy **outside the repository** (`~/MeshCom-bench-backups/`, mode 600) and a masked copy under `test/golden/nodes/<node>/`. `--verify-masked` is in the tooling gate and fails if a live credential ever lands in the repo. T-Beam-92 and T-Deck-14 still owed.                                                                                                                                                                                                                                                        | `test/golden/nodes/`, see its README                          | 0.5 h                       |
| P0.4 | ~~Corpus assembly~~ **DONE 2026-09-11.** `test/golden/build_corpus.py` generates all four corpora from the repository's own fixtures, so an upstream fixture change propagates by re-running it: 28 LoRa frames (on-air corpus + gwflood), 37 UDP-1990 datagrams (GATE-wrapped frames, BEAT, CONF with all five TLVs in parser order, 7 malformed cases), 23 EXTUDP inbound objects, and the command set — 309 ladder entries, a 483-line driveable script, the adversarial prefix set, and 18 commands held back for manual running. Callsigns rewritten to `DK5EN-<ssid>` with the FCS recomputed; `*` rewritten to group 9999 in the `udp1990` corpus only.                                                                                                                                                                                                                | `test/golden/corpus/{lora,udp1990,extudp,commands}/`          | ~~1.5 d~~ 0.5 d             |
| P0.5 | ~~Foreign callsigns rewritten before anything can transmit~~ **DONE 2026-09-11.** `test/golden/corpus_lint.py` is the gate: it unwraps `GATE`/`DATA`, passes the path-less binary ack, honours an explicit `# lint-exempt: <reason>` header on the deliberately malformed entries (reported, not hidden), and reports any corpus file it cannot classify rather than skipping it. The whole generated corpus passes: 65 files, 0 violations.                                                                                                                                                                                                                                                                                                                                                                                                                                  | `test/golden/corpus_lint.py`, `test/golden/mc_frame.py`       | ~~0.5 d~~ 0.3 d             |
| P0.6 | ~~Stub server spec for UDP 1990 (`tools/bench/mc_server_stub.py`, planned)~~ **DONE 2026-09-11, and no new tool was needed.** `tools/mock/meshcom_server.py` already implemented the server side of doc 11 §2 (KEEP/DATA in, BEAT/GATE/CONF out, MAX_ZEROS drop, foreign-callsign injection guard) with a 108-test suite. Extended for this role with a datagram recorder, `--replay DIR` / `--gap` / `--repeat` corpus replay in sorted file order, `--capture-dir` (writes `udp-log.txt`, `udp-tx.bin`, `udp-rx.bin`) and `--no-redistribute` so a capture contains only what the replay sent. 115 tests green; end-to-end smoke run against a fake node passed.                                                                                                                                                                                                            | `tools/mock/meshcom_server.py`, `README.md`                   | ~~1 d~~ 0.3 d               |
| P0.7 | ~~Normalization rules for every capture (section 7)~~ **DONE 2026-09-11.** `test/golden/normalize.py`, 269 lines, stdlib only, self-test green: timestamps, dates, RSSI/SNR/frequency error in both console and JSON spelling, MAC/IPv4, heap in both spellings, the `[ETH]`/`[EXT]` durations and ages, the `[INSTR-*]` profiler figures, and msg_ids as a per-capture sequence. Plus `mask_binary()` for the BLE and UDP captures. Validated against a real RAK-90 run. Rules will need one more pass once G0 captures exist.                                                                                                                                                                                                                                                                                                                                               | `test/golden/normalize.py`                                    | ~~0.5 d~~ 0.4 d             |
| P0.8 | ~~BLE capture path: a host BLE client…~~ **WRITTEN 2026-09-11, validation blocked on one macOS setting.** `tools/bench/ble_golden.py` (option A of §12.2): scans by the NUS UUID, builds hello / timesync / 0xA0 text frames itself, replays `test/golden/corpus/ble/writes.txt`, records notifications as `ble-frames.bin` (length-prefixed), `ble-frames.txt` (decoded) and `ble-writes.txt`. Timesync uses a fixed corpus timestamp, not `now`. The PIN hash is redacted out of the recorded hello, so a capture stays committable. Frame-builder self-test green against the firmware's own `hash_pin()` rule (sha256 of the `%06u` string). Run with `uv run --with bleak`, never installed into the PlatformIO venv. **Blocked:** CoreBluetooth grants Bluetooth per application — Terminal.app must be enabled under System Settings → Privacy & Security → Bluetooth. | `tools/bench/ble_golden.py`                                   | ~~1 d~~ 0.4 d + the setting |
| P0.9 | ~~Protocol templates (section 8) created empty so the before-run only fills cells~~ **DONE 2026-09-12.** Both files created with the section 8 tables and the run summary, pre-seeded with one row per (node, step) so a run fills cells only. Two row sets are wider than section 6's node column on purpose and say so in the file: `H4` BLE (G0 captured all four nodes, not two) and `H8` EXTUDP (tracked for all four, three still owed). The filling rules are restated in each file, including the one that matters: an empty cell means the run is incomplete, not that the step was not applicable.                                                                                                                                                                                                                                                                  | `docs/testplan/protocol-before.md`, `protocol-after.md`       | ~~0.5 h~~ 0.3 h             |
...
| H6   | UDP 1990 gateway path  | stub server replays the UDP corpus; node in gateway mode against the stub                                                                                                                       | `udp-rx-log.txt`, LoRa TX ring dump, BLE frames                       | RAK-90, Heltec-93, T-Beam-92                             |
| H7   | UDP 1990 upstream path | inject LoRa corpus with gateway on; stub records the datagrams the node sends                                                                                                                   | stub log `udp-tx.bin`                                                 | same                                                     |
...
## 10. Stub server specification (UDP 1990) — implemented as `tools/mock/meshcom_server.py`

- Binds `0.0.0.0:1990`, logs every received datagram with monotonic timestamp, source, length,
...
- `--replay corpus/udp1990/` sends each datagram once, in file order, with a configurable gap;
...
**Port correction (2026-09-11).** Earlier revisions of this plan and of the audit named port
1799 for this surface. That is the EXTUDP sideband. The mesh/server UDP protocol
(`GATE`/`BEAT`/`CONF`) is **1990** — `UDP_PORT` / `LOCAL_PORT` in
`src/configuration_global.h:165-166`, used at `udp_functions.cpp:681`. Both documents are
corrected; the corpus directory is `test/golden/corpus/udp1990/`.

**Reference implementations to build the stub from, rather than deriving it from scratch:**

| Source                                      | What it gives                                                                           |
| ------------------------------------------- | --------------------------------------------------------------------------------------- |
| `docs/architecture/11-wire-format.md` §2    | the normative section for node -> server (§2.1) and server -> node (§2.2), incl. CONF   |
| `~/WebDev/mc-chat/meshcom_mock/decoder.py`  | parses everything the real servers send: the server -> node encode side, in reverse     |
| `~/WebDev/mc-chat/meshcom_mock/protocol.py` | builds KEEP/DATA/positions as a node does: the assertion side for what the stub records |
| `mc-chat/doc/proto-deviations.md`           | where the upstream reflector spec is factually wrong; the firmware wins                 |

The mock is a _client_ (softnode against the real servers), not a server, so it is not a
drop-in. Its two protocol modules are a validated frame reference and remove most of the
specification work from P0.6.
