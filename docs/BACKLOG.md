...
| TM-16 | T-Deck | PERF | Medium | `esp32_main.cpp` setup, `udp_functions.cpp:529-747` | Boot to `[BOOT];ready` takes 29 s (reset -> network phase settled; `CLIENT STARTED` at ~18 s). Operator sees the start tone, then header icons still blinking. Profile the setup path (SD mount, LVGL build, BLE init, WiFi scan+join) and cut the waits. | open |
