...
| CHR-03  | —     | all                      | ENH  | Medium | Latin-1 as a second accepted charset next to UTF-8, without transcoding            | **DONE 2026-09-09**: Bytes 0xA0-0xFF, die keine gueltige UTF-8-Sequenz bilden, passieren unveraendert (kein Transkodieren, In-Place-Kontrakt bleibt); 0x80-0x9F (C1) fallen weiter, ungueltige-aber-wohlgeformte Sequenzen fallen jetzt GANZ statt nur im Lead-Byte, sonst haette der Latin-1-Pfad Fortsetzungsbytes (0xA0-0xBF) als Fragment durchgelassen; 25 native Faelle. Ausgabe ist damit bewusst nicht mehr garantiert valides UTF-8 -> Deutung liegt beim Empfaenger (mc-chat 993b512). Offen: EXTUDP-JSON (`extudp_functions.cpp:640`) traegt die Bytes in `msg` |
...
#### CHR-03 — Latin-1 as a second accepted charset (2026-09-09)
...
The rule now: a byte that is **not part of a valid UTF-8 sequence** is re-read as ISO-8859-1 and
kept unchanged if it is a graphic character there (`0xA0`-`0xFF`). Nothing is transcoded — the
filter still only ever removes bytes, never rewrites or widens one, so every caller's in-place
buffer contract and every wire-length budget is untouched.

Three decisions worth keeping visible:

- **`0x80`-`0x9F` still falls.** That block is the C1 control range in Latin-1, and `is_c1()` already
  strips it on the UTF-8 path; letting it in through the Latin-1 door would be a back door around
  the front door's own rule. Consequence: in CP1252 those bytes are the Euro sign and the
  typographic quotes, and those do **not** pass. If the fleet turns out to send CP1252 rather than
  true Latin-1, this is the one line to revisit.
- **Illegal-but-well-formed sequences now drop whole**, continuation bytes included, where they
  ...
  continuation bytes of an overlong or surrogate sequence live in `0xA0`-`0xBF`, which the new
  Latin-1 rule calls printable. Resyncing into the middle of such a sequence would hand a fragment
  of exactly the payload the check rejects back to the caller — e.g. the `A0` of the surrogate
  `ED A0 80` as NBSP. Net behaviour for the pinned CHR-01 cases is unchanged.
- **Two adjacent Latin-1 bytes that form valid UTF-8 are read as UTF-8** (`DF BC`, Latin-1 `ß¼`, is
