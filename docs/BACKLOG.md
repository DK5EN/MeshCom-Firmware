...
| ----- | -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------- |
| UP-01 | `mheard_functions.cpp` (2 sites)       | `serializeJson` bound is the JSON's length, not `sizeof(bleBuffer)-1`. Worst case today 284/299 B (15 B headroom) — latent, goes live with the next key. | Medium | **Fix in the next PR** with a native regression test (like #1102).          |
