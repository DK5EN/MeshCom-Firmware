...
| GPS-07  | `gps_filter.h` (`ALT_KF_RESEED_N`, `ALT_KF_GATE_M`)                                      | **OPEN, found 2026-09-03 in the overnight capture of `DK5EN-14` (9791 evaluations, 0 rejects, 0 corrupt samples):** the re-seed guard (gate 15 m, 10 consecutive rejects = 30 s at the 3 s ESP32 cadence) fires inside ordinary altitude excursions of a stationary receiver, which last 60-120 s (the filter's own comment) and reached +36 m; it re-seeded the estimate 10 times in 8 h onto a single raw outlier (02:48: 493.9 -> 518.1 m, beaconed as 507 m on a ~487 m node). An offline replay of `gps_filter.cpp` reproduces every beaconed `/A=` within 1 m, so the filter runs as designed and only the guard is mis-sized. Replay: `ALT_KF_RESEED_N` 60 (3 min) or `ALT_KF_GATE_M` 30 -> 0 re-seeds; error at the 16 beacon times: raw sample mean 7.6 m / worst 18.0 m, shipped filter 3.8 / 18.5 m, with RESEED_N 60 3.2 / 10.1 m. hdop-weighted `R` adds nothing (every hdop bucket ~4.5 m median error); leave `Q`. Data and replay scripts: `~/Downloads/meshlog-20260903/` (`14.log`, `kf.py`, `kf2.py`, not in the repo). **CONFIRMED and worse 2026-09-06** on a second board (`DK5EN-93`, Heltec V3, 2 h indoor bench capture, 2371 samples, 100 % fix, HDOP 0.8-3.2): **20 re-seeds in 1.9 h**, about one every six minutes, against the 10-in-8-h that filed this item; every one snapped the estimate onto the raw outlier exactly. Consequence measured: the filter delivers **no** benefit on this corpus -- sd 17.16 m against the raw sample's 16.93 m, ADEV at 30 s improved by 2 % -- so "still better than raw even unfixed" does not hold indoors. HDOP was 1.0-1.9 across 60 m excursions, re-confirming that HDOP-weighted `R` buys nothing. [`bug-baro-altitude-20260906.md`](bug-baro-altitude-20260906.md) §4. | Medium      | One constant, rerun `test_gps_filter`, validate on **both** captures. **No longer 'better than raw even unfixed'** -- see the 2026-09-06 evidence.                                                                                                                                                                      |
| GPS-08  | `bmx280.cpp:28`, `:308-310`, `command_functions.cpp:2287`, `:898`, `nrf52_main.cpp:2352` | **OPEN, found 2026-09-06 on `DK5EN-93`:** the barometric altitude is **off by default on every node**. `fBasePress` is assigned in exactly two places in the whole firmware -- its definition (`= 0`) and the `--setpress` handler -- so `getPressALT()` hits its `fBasePress == 0.0` guard and returns 0 unless an operator has typed `--setpress`. It is a plain RAM global, absent from `esp32_flash.h` / `WisBlock-API.h`, so **it does not survive a reboot** either. `--info` on a node with a detected, working BME280 duly reported `ALT asl: 0 m`. QNH is unaffected -- `getPressASL()` self-latches two lines further down, so QNH works while barometric altitude does not, from the same sensor. Second defect, same item: even armed, the absolute anchor is **one arbitrary GPS sample** (`baroBaseRelatch()` at the convergence edge) -- it latched 474.0 m while the GPS's own 2 h mean was 486.5 m. Third: `--help` advertises `--setpress 999.9` but the handler parses no argument. Identical on nRF52. [`bug-baro-altitude-20260906.md`](bug-baro-altitude-20260906.md) §5.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Medium-high | Latch `fBasePress` where `baroBaseRelatch()` already latches the altitude (`gps_functions.cpp:1195`) -- **not** by persisting it: a new settings field bumps `FLASH_STRUCT_VERSION` and wipes the fleet. Strategy deliberately left open; decide and write down why. Both platforms.                                    |
| GPS-09  | `esp32_flash.h:194`, `WisBlock-API.h:361`, `:597`                                        | **OPEN, found 2026-09-06 on `DK5EN-93`:** `node_press_alt` is an `int` while `node_press_asl` beside it is a `float`. One metre is ~0.12 hPa, far coarser than a BME280 resolves, so whole-metre quantisation is a noise floor on the barometric channel: recomputing the same quantity in `float` from QFE with the firmware's own formula halves short-term instability (ADEV at 30 s 0.20 m -> 0.10 m; sd and long-term drift unchanged, as expected for quantisation). Harmless as a display field, blocking for GPS-05b. [`bug-baro-altitude-20260906.md`](bug-baro-altitude-20260906.md) §6.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Low         | **Do not** just retype the struct field: `int` and `float` are both 4 B so the size is unchanged, but a value already in flash is reinterpreted as a float bit pattern for the first 60 s after update. Carry `float` only on the GPS-05b path, which consumes `getPressALT()`'s inputs rather than its rounded output. |
...
| GPS-05b | design item, no single site                                                              | Moving-node altitude, boards **with** pressure sensor: baro/GPS complementary filter, mirroring mcmap's server-side `altSmoothed`. An upgrade, never a prerequisite. Runtime discriminator exists: `bBMPON`/`bBMEON`/`bBME680ON`. **Prerequisite half-met, and the stationary case is now measured (2026-09-06, `DK5EN-93`, 2 h, BME280 + L76K):** GPS-01 closed 2026-09-03; TRACK-mode logs with pressure still do not exist. Barometer alone sd 1.34 m / ADEV 0.10 m at 30 s, against GPS raw 16.93 m / 4.97 m and the reported `node_alt` 17.16 m / 4.87 m -- one to two orders of magnitude better at every averaging time. Detrended correlation GPS vs baro **-0.031**: the barometer corroborates none of the 91.6 m GPS excursion, so it is all error. Offline complementary replay (GPS low-pass + baro high-pass) has its knee at **tau = 30 min**: sd 3.27 m, 30-min ADEV 0.83 m = **5.2x / 7.2x better than what ships**. Do not take the largest tau -- 30-min ADEV worsens again above the knee (1.96 m at 60 min) as the filter starts tracking weather. Blocked on GPS-08 and on the three sensor families disagreeing about what `node_press_alt` means. [`bug-baro-altitude-20260906.md`](bug-baro-altitude-20260906.md) §7-8.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Low         | Do not start before GPS-01 lands and TRACK-mode logs with pressure exist. Doc §7.6.                                                                                                                                                                                                                                     |
