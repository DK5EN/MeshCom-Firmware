...
`extudp_functions.cpp:640` puts `msg_payload` into the JSON field `msg`. A consumer that decodes
that datagram as strict UTF-8 will now reject frames it previously accepted, because the firmware
used to delete the offending bytes for it. mc-chat's interlink path does exactly that
(`_parse_json()` uses a strict `.decode("utf-8")` and drops the message on failure); its APRS
decoder path is already lenient. Deciding whether the firmware, the interlink reader, or both
