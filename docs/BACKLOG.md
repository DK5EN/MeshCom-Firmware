...
| CTY-02 | ESP32 + nRF52 (all gateway-capable)                                                                                                                                                                               | BUG  | High   | `src/gwsrv_select.h` (new), `src/udp_functions.cpp` `startMeshComUDP()`, `src/nrf52/nrf_eth.cpp` `startUDP()`/`startFIXUDP()`                                                                                                                         | **Upstream issue [#1133](https://github.com/icssw-org/MeshCom-Firmware/issues/1133).** `--gateway srv dl` had no effect on a node with a plain Internet uplink: the setting was stored, persisted and shown by `--info`, but the destination lookup dropped it and the node registered with `meshcom.oevsv.at`. Cause: when the IT server was added (2026-08-20) the DL arm on the Internet branch was **replaced** rather than extended, deleting literal `192.68.17.26` (= `meshcom.hamnet.network`) from the tree. HAMNET was unaffected (it kept `meshcom.hamnet.cloud`), so only nodes on a normal Internet uplink were hit. The nRF52 Ethernet Internet paths never had a DL arm at all — `CTY-01` added only `IT`/`OE`. Retracts the `TM-39` verdict that called this a deliberate server-topology question. | **FIXED 2026-09-06:** country/transport matrix extracted from its three inline copies into one pure table `src/gwsrv_select.h` (returns name + matching literal + `hamnet`/`inet` path, so the resolver-based ESP32 path and the resolver-less nRF52 path cannot drift); all three call sites converted. Regression test `test/test_gwsrv_select/` (9 cases, registered in `env:native`) written against the broken behaviour first and observed red on the DL-Internet cell before the fix. Native suite 284/284; `wiscore_rak4631`, `heltec_wifi_lora_32_V3`, `heltec_wireless_tracker` all build. **Hardware end-to-end proof still outstanding** — needs a bench node on USB, run `srvprobe.py`.                                                                                                                                                                                  |
...
| `TM-39` country servers    | **Closed** — ESP32 `CONF` (`b624bd33`) and the nRF52 internet-path country split (`CTY-01`, `a1191eaa`) were the two leftovers. ~~The DL-on-internet asymmetry is a server-topology question, left to upstream.~~ **Retracted 2026-09-06:** it was a regression, fixed as `CTY-02` (upstream issue #1133). |
