...
**Last updated 2026-09-21 — Extern-UDP stack overflow fixed (DONE, field-verified) and RAM
reclaim ported (DONE on `fork-main`, hardware verification OWED).** §3.8am is the new section.
Stack fix: a foreign-destination `{"type":"msg"}` Extern-UDP datagram reset every ESP32 node on
`--extudp on` (`c754d01f`+`efc9681e`, CHANGELOG item 229); answers the ESP32 half of `UDP-01`
(§3.8l), RAK/nRF52 half still open. RAM reclaim: the byte-FIFO output rings and the `int16_t`
display-cache/web-header changes from `neo-ram-reclaim` are now on `fork-main`
(`e2861458`+`9530967d`+`36b3a895`, items 230/232) — gate is native-only, **no board has run this
build yet** (`RAM-01`). Also in this wave: `--mesh off` now holds on via paths (`45e411d4`, item
231, "What changes on the air"), and a new operator decision is filed (`MESH-01`: should
`--mesh off` also silence the gateway DM ACK and DM-store custody, both own emissions rather than
relays). `PRM-08` (§3.8al) updated to reflect the port. Hand-over: [`RESUME.md`](RESUME.md).
...
- **The inbound path has never been stack-measured — on the RAK/nRF52 side.** `N-22` measured the
  outbound path only. Inbound is deeper: `getExternUDP()` (`extudp_functions.cpp:322`) ->
  `getExtern()` (`:238`, `char val[161]` plus an ArduinoJson `JsonDocument` on the stack) ->
  `sendMessage()` -> `sendExtern()`, i.e. the whole `N-22` path with the JSON decoder stacked on
  top, called from the same 4 KB loop task (`nrf52_main.cpp:2423`). One watermark print at that
  depth settles it the same way it settled `N-22`. This is also why the operator's "send **and**
  receive" framing is the right one: the receive direction is the untested half and the deeper
  one. **`tools/stack_budget.py` (2026-09-21) cannot answer this** — it walks the compiled Xtensa
  call graph from an ELF and has no ARM/nRF52 equivalent, so this bullet is unchanged by that
  work. The same question on the ESP32 side, by contrast, has since been answered — see the
...
**2026-09-21 — the ESP32 equivalent of the first bullet turned out real, and is fixed.**
`getExternUDP() -> getExtern() -> sendMessage() -> sendExtern() -> decodeAPRS() -> printfdeb() ->
MeshSerial/lwIP` did not fit the arduino-esp32 default 8192 B loop-task stack either — a
`{"type":"msg"}` datagram with a foreign destination reset DK5EN-98 deterministically, four times,
on 2026-09-20, before the fix. `tools/stack_budget.py` now gates both `esp32loop` and
`getExternUDP` against a 12288 B budget (raised from the 8192 B framework default) at 12288 minus
a 512 B margin. Changelog item 229, `c754d01f` + `efc9681e`, field-verified 2026-09-21 (a 9 h
36 min run held `stack_hwm` at 4876 vs. 368 before). Full derivation:
`docs/bug-extudp-stack-20260920.md`. The RAK/nRF52 side above is unaffected by this and remains
open — the tool has no ARM equivalent.
...
### 3.8z DM transport campaign — five stages in the tree, nothing flashed (2026-09-14)

State at `c2db86bb`. Master table: `docs/dm-transport-impl-plan-20260913.md`. Bench handoff:
`docs/dm-bench-session-plan-20260914.md` (four nodes: 93 sender, 90 store node, 92 relay,
14 destination via OTA; blocks A-E in order).

| ID    | Item                                                                                                                                                          | State                                                                              |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| DM-01 | Stage 0: ACK repair, DM/ring counters, `--airgap` (7aeb2ac5..fe3f0640)                                                                                        | code, approved, bench T-0.x open                                                   |
| DM-02 | Stage 2.1: destination dedup on source+NNN, LoRa + both server paths (44506d0b, 0633e542)                                                                     | code, approved, bench open                                                         |
| DM-03 | Stage 3: store node + mailbox page + settings (59f21e5b..be141ecc)                                                                                            | code, approved, bench T-3.x open; fork-only until upstream accepts the role        |
| DM-04 | Stage 4: custody notice `:stoNNN`, status 0x04 held (66dea241..4440acc0)                                                                                      | code, approved, bench T-4.x open                                                   |
| DM-05 | Stage 1: outbox + ladder behind `--dmretry off\|3\|9`, default off (731e0ebc..c2db86bb)                                                                       | code, approved, bench T-1.x open                                                   |
| DM-06 | M0-1 ring-slot survival, one hour on the live net                                                                                                             | measurement open, confirmation only (outbox built regardless)                      |
| DM-07 | Stage 2.2 bounded ACK repeats                                                                                                                                 | deferred; only if stage 1 measurements show ACK loss the retry path does not close |
| DM-08 | T11: dedup-ring rotation window under nine ids per DM                                                                                                         | measure before/after stage 1 on the bench node                                     |
| DM-09 | Server and mcmap fold on (source, NNN) or DM counts inflate once `--dmretry` is on                                                                            | open, outside this repo                                                            |
| DM-10 | Clients: MCProxy ack-kind map + holder, mobile app ack states 3/4 + icons, mc-chat 0x41 parser, outbox-full nack (`docs/client-integration-store-forward.md`) | open, three repos                                                                  |
| DM-11 | Stage 3 peer-delivery tell: "SRC,GW" hop-0 server-emitted copy at a store node (advisor UNVERIFIABLE)                                                         | bench question, T-3.5                                                              |
| DM-12 | Node-settings JSON (`SN`) field for the store role, proposal only                                                                                             | waiting for the app side                                                           |
| DM-13 | Upstream framing: stages 0 and 2.1 are PR-sized and help every node; 1/3/4 stay fork-only                                                                     | after the bench                                                                    |

Pitfalls recorded in memory during the campaign: the ESP32 main radio tick is the `if(bRadio)`
one, not the EXTERNAL_RADIO tick; `msg_server` in `OnRxDone` only means a gateway touched the
frame; a `#if ENABLE_MSGSTORE` above `#include configuration.h` compiles a file to nothing; an
ack hook behind `checkOwnTx()` defeats the outbox's whole purpose (T2).
...
| OPT-D5  | BUG  | Medium | `nrf52_main.cpp:2069-2073` vs `esp32_main.cpp:3849`               | nRF52 gateway calls `sendUDP()` only in the pass where `getUDP()` returned "no packet"                                                                                                                                                                                                                                                                                                                                      | open, needs soak (`D1-09`)                                                                                                        |
| OPT-D6  | BUG  | Medium | `nrf52_main.cpp:2104`                                             | nRF52 heartbeat recovery nested under `!hasIPaddress`: live link with silent server never recovers (ESP32 has stage-2 `resetMeshComUDP()`)                                                                                                                                                                                                                                                                                  | open, needs soak (`D1-09`)                                                                                                        |
...
| OPT-D8  | GAP  | Medium | `nrf52/WisBlock-API.h` vs `esp32/esp32_flash.h`                   | nRF52 settings struct lacks `node_ntp`, `node_immediate_save`, `node_modus`, `node_mute`, `node_persist_to_flash`, `node_disp_rot`; shared code cannot touch them                                                                                                                                                                                                                                                           | open, fixed by `D1-04` (wave 3)                                                                                                   |
| OPT-D9  | BUG  | Low    | `nrf52_main.cpp:2018`/`:1974` vs `esp32_main.cpp:3462`/`:3409`    | Telemetry gate uses `bHeyFirst` on nRF52 where ESP32 uses `bTeleFirst && bAllStarted`; no `extra_hey_time` spreading on nRF52                                                                                                                                                                                                                                                                                               | open, fix standalone before `D1-10`                                                                                               |
...
### 3.8ah Safeboot OTA: status page, Auto-AP, single app slot — campaign 2026-09-13

**Intake.** Operator brief: the safeboot page showed nothing but the drop zone; the WLAN join was
the pre-TM-34 pattern upstream (TM-48 fixed it here); an aborted upload must never flash a
partial image (TM-49); and the page must show SSID, mDNS name, IP, the scanned BSSIDs with RSSI,
the connected BSSID, what the upload is doing, why it aborted, and the node must return to the
running firmware. Executed as `/orchestrate-waves` (contract-first, three writers per wave, gate
by the orchestrator, hardware by the orchestrator). Contract: `docs/safeboot-ota-contract.md`.

**What is in place** (`80e763f0`..`6a040d3e`): `src/safeboot/ota_state.h` host-tested session
machine (17 Unity tests, `native_safeboot`); `main.cpp` non-blocking join with event log, Auto-AP
after 25 s, async scan cache, `GET /ota/info`, `/ota/state`, `/ota/scan`; `ota.html` network
panel (Wifi-Client / Auto-AP wording, multi-column, below the upload card) and live state view
with abort reason and fallback countdown (jsdom harness `tools/safeboot_page_test.js`, 56
checks); `tools/webflash.py` reports the node's state after a failed upload;
`tools/bench/ota_abort.py` (+40 unit tests) drives kill/stall/doublestart/cancel on hardware;
`src/safeboot/safeboot_log.h` tees the console onto native USB.

**Findings that changed the design.** (1) One app slot on every board: no fallback after the
first written byte; `app_valid` + notice instead of a 180 s reboot loop (the old safeboot looped
the same way, unmeasured). (2) A superseded upload request's completion handler hit the new
session (generation per request). (3) The S3 safeboot log never reached a native-USB port. (4)
AsyncTCP's RX timeout closes a stalled upload in ~4 s, the 30 s watchdog is the second line.

| ID    | Type | Prio   | Location                      | Item                                                                                                                                                                                 | Status                                         |
| ----- | ---- | ------ | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------- |
| SB-01 | FEAT | High   | `src/safeboot/`               | Status page with network panel and live state, Auto-AP after 25 s, `/ota/info` `/ota/state` `/ota/scan`                                                                              | **DONE 2026-09-13**, bench 6/6 on three boards |
| SB-02 | BUG  | High   | `src/safeboot/main.cpp`       | Fallback reboot loop into safeboot after an aborted upload (single app slot)                                                                                                         | **FIXED 2026-09-13** (`app_valid`, `958188fd`) |
| SB-03 | BUG  | Medium | `src/safeboot/ElegantOTA.cpp` | Superseded upload request's completion handler answers for the new session                                                                                                           | **FIXED 2026-09-13** (`874270bd`)              |
| SB-04 | BUG  | Medium | `src/safeboot/safeboot_log.h` | S3 safeboot console invisible on native-USB boards                                                                                                                                   | **FIXED 2026-09-13** (`99ace556`)              |
...
| DH-03 | TEST | Low  | `tools/bench/serial_session.py`                           | `--wait-boot` looks for `CLIENT STARTED`, which this firmware never prints, and terminates with CR only; commands sent before boot finished were silently swallowed. What works: wait for `[BOOT];ready`, send LF-terminated. Opening the CP2102 port also resets the node every time. | **FIXED 2026-09-13** — waits for `[BOOT];ready` (both platforms, raw `Serial.printf`), LF-terminated by default, streams output live, warns loudly on a missing marker or a zero-byte session, `--strict` exits 2, `--help` no longer parsed as a port. Also `--dtr auto`: a native-USB board (RAK4631, S3) implements CDC in firmware and stays **completely mute** until the host asserts DTR — the RAK returned 0 bytes with DTR low, no error — while a CP2102/CH340 bridge uses those lines for EN/BOOT and must not see them asserted. Auto-detected by port name (`usbmodem` = native USB). Streaming into `head` also raised an unhandled `BrokenPipeError`; now exits quietly. Verified on DK5EN-93 (Heltec V3, CP2102) and DK5EN-90 (RAK4631, native USB), both on default flags, plus a pty for the marker/no-marker/strict paths |
| DH-04 | TEST | Low  | `src/safeboot/main.cpp`                                   | Safeboot half of the DHCP-hostname change: was code + build only, OTA mode had never been entered.                                                                                                                                                                                     | **VERIFIED 2026-09-13** — `--ota-update` on DK5EN-93: safeboot came up (`[SAFEBOOT];app;image;valid;rc;0`, `/ota/info` `mode:sta`, IP 192.168.68.69) and the Deco kept showing `DK5EN-93`, not `esp32-3A8968`. Meaningful because the rename test proved the Deco does follow a changed name. Returned to the app via `GET /ota/cancel`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

### 3.8aj Ping fails loudly, and TX marker parity across platforms — 2026-09-13

**Ausgangspunkt:** Betreiber-Fehlerbericht (T-Beam 1W, `docs/bug-ping-track-mode-20260913.md`):
Ping-Budget lief herunter, `[PING]...send Ping` stand im Log, auf der Luft passierte nie
etwas. `sendPing()` stieg in der ersten Zeile aus, wenn TRACK aktiv war.

**Was geliefert wurde** (`c570e62e`): `sendPing()` gibt `PingResult` zurueck
(`PING_QUEUED` / `PING_SUPPRESSED_TRACK` / `PING_RING_REFUSED`, `src/loop_functions.h:69`).
Beide Fehlerwege drucken unbedingt per `printfdeb()` ihren Grund, ratenbegrenzt auf eine
Zeile pro Unterdrueckungs-Episode und neu scharf, sobald TRACK aus- und wieder eingeschaltet
wird. Der Ring-Ablehnungspfad kehrt vor `DisplayPong(SENT)` und vor `bPingSend = true`
zurueck, ein nie eingereihter Frame erzeugt also kein irrefuehrendes `[PONG]...fail` mehr.
Die Aufrufer melden das Ergebnis statt den Versuch anzukuendigen. TRACK-Unterdrueckung
verbraucht kein `node_pingcount` mehr (statischer Zustand: das Budget waere in N x pingtime
Sekunden leer, ohne dass je gesendet wurde), ein abgelehnter Ring schon. Dazu
Konfigurationszeit-Warnungen bei `--pingcall`, `--ping start` und `--track on`, derselbe
Marker in `SendPong()`, und die falsche Schranke `sizeof(node_call)` beim Schreiben von
`node_pingcall` (`src/command_functions.cpp:3705`) korrigiert.

Marker-Paritaet (`ec070235`): `CAD_FREE` steht auf dem nRF52 jetzt vor `doTX()` wie auf dem
ESP32 (vorher im Erfolgszweig, also dieselbe Bedeutung wie drueben `TX_START` -- gleicher
Name, anderes Ereignis), `TX_START qlen=` ergaenzt, und `OnTxTimeout()` schliesst die
MC-SM-Spur eines fehlgeschlagenen Sendevorgangs mit `rc=-1`, statt sie fuer immer bei
`TX_ACTIVE` haengen zu lassen.

Gate: 308/308 native, Builds gruen fuer `heltec_wifi_lora_32_V3`, `LilyGo_T-Beam-1W`,
`t_deck_plus`, `wiscore_rak4631`; alle Marker per String-Scan im Image nachgewiesen (die
INS-01-Lehre). Bench: DK5EN-14 (T-Deck Plus) und DK5EN-93 (Heltec V3) je 3/3, Ping-Pong-
Runde live bestaetigt (`[PONG] from:DK5EN-14 rssi:-26 snr:7`); RAK4631 4/4 (DK5EN-90), dort zusaetzlich
der zweite Rueckgabepfad `PING_RING_REFUSED` live belegt, den beide ESP32-Boards nie
ausgeloest haben, und `ec070235` auf Hardware bestaetigt. Bericht: `docs/ping-bench-20260913.md`.

**Bewusst nicht gemacht, weiterhin offen:**

| ID      | Type | Prio | Location                                 | Item                                                                                                                                                                                                                                                                                                                          | Status                                                                                                                                                                                                                                                                                                                 |
| ------- | ---- | ---- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PING-01 | BUG  | Mid  | `src/nrf52/nrf52_main.cpp:2565`          | Die nRF52-Ping-Schleife ist von der ESP32-Variante abgedriftet: kein `node_pingcount`-Budget, kein `PongFail()` bei ausbleibendem Pong, und die `[PING]...send Ping`-Zeile haengt hinter `bDisplayInfo` (die neue FAILED-Zeile bewusst nicht). Zusammenfuehren ist eine eigene Aenderung.                                     | offen                                                                                                                                                                                                                                                                                                                  |
| PING-02 | DOC  | Low  | `src/esp32/esp32_flash.cpp:327/604`      | `node_pingtime` und `node_pingmax` werden persistiert, `node_pingcount` nicht. `--ping start` ruft trotzdem `save_settings()`, das gesetzte Budget ueberlebt aber keinen Neustart, danach ist die Schleife still. Vertretbar fuer ein Diagnosewerkzeug, aber nirgends dokumentiert und so nicht lesbar.                       | offen, gefunden beim Bench 2026-09-13                                                                                                                                                                                                                                                                                  |
| PING-04 | TEST | Mid  | RAK4631 am Bench                         | Positivpfad auf dem RAK war durch das Werks-Rufzeichen `XX0XXX-00` blockiert (`addTxRingEntry()` weist unbedingt ab, `txring_functions.cpp:449`).                                                                                                                                                                             | **ERLEDIGT 2026-09-13** -- nach Betreiber-Entscheidung `--setcall DK5EN-90`, mit `ec070235` neu geflasht: Positivpfad und Ping-Pong-Runde bestaetigt (`[PONG] from:DK5EN-93 <975> rssi:-35 snr:7`), und damit zugleich die Marker-Paritaet auf Hardware belegt (`TX_START` erscheint, Reihenfolge identisch zum ESP32) |
| PING-03 | TEST | Low  | `src/loop_functions.cpp:3330`            | Der Reset des Ratenbegrenzer-Flags ist nur auf nativem USB beweisbar: ein CP2102-Board bootet bei jedem Port-Open neu und setzt den `static` ohnehin zurueck.                                                                                                                                                                 | **ERLEDIGT 2026-09-13** ueber DK5EN-14 (T-Deck Plus, natives USB, kein Reset beim Open -- belegt durch `Count:5->3` ueber Sitzungsgrenzen): die Suppressed-Zeile kam nach einem Erfolg erneut                                                                                                                          |
| TXM-01  | OPT  | Mid  | `src/lora_functions.cpp:1722` (`doTX()`) | Drei nahezu identische Sendestellen (~1854 TRACK/LoRa-APRS, ~1902 zweiter TRACK-Pfad, ~1967 normaler MeshCom-Frame), jede nochmals nach `BOARD_RAK4630` gegabelt -- sechs Varianten desselben Aufrufs. Genau daher die Marker-Divergenz: es gibt keinen einen Ort zum Instrumentieren.                                        | offen, gehoert zu 3.8af (DRY-Vereinheitlichung), nicht als Einzelpatch                                                                                                                                                                                                                                                 |
| TXM-02  | OPT  | Low  | `src/lora_functions.cpp` (`doTX()`)      | `RADIO_TX` wurde nur an einer der drei Sendestellen gedruckt, und dort nur im Nicht-RAK-Zweig -- als "Frame ging raus"-Marker wertlos.                                                                                                                                                                                        | **ERLEDIGT 2026-09-13** (`47eb2011`) -- alle sechs Varianten ueber einen Helfer, `kind=track                                                                                                                                                                                                                           | aprs | msg`angehaengt, Prefix unveraendert. Hardware:`kind=msg`auf RAK und Heltec,`kind=aprs`auf Heltec;`kind=track`nicht beobachtet (stationaere Bake`RATE:1800`, kein Positions-Frame faellig, `--sendpos` erzwingt keinen) |
| TXM-03  | BUG  | Mid  | `src/esp32/esp32_main.cpp:2620`          | Der ESP32 pollt vor dem CAD-Scan das SX1262-IRQ-Register (`HEADER_VALID` / `PREAMBLE_DETECTED`) und bricht den TX ab, wenn ein Paket im Anflug ist. Der nRF52 hat kein Gegenstueck, nur den groberen `is_receiving`/`tx_is_active`-Test vor dem Gate (`nrf52_main.cpp:1453`).                                                 | offen, **Hypothese aus dem Code-Lesen, nicht verifiziert** -- ob `Radio.StartCad()` den Fall ohnehin faengt, braucht einen Bench-Test, keine weitere Codelektuere                                                                                                                                                      |
| TXM-04  | TEST | Low  | `tools/hmac_connect.py`                  | Die Netz-Konsole (Port 2323) setzt den Knoten nicht zurueck und nimmt Kommandos an -- der einzige Weg, auf einem CP2102-Board eine Laufzeit > 5 Minuten zu erreichen, weil die serielle Leitung bei jedem Port-Open neu bootet. Bei einer zweiten Verbindung blieb `--info` unbeantwortet, waehrend der Log-Strom weiterlief. | offen, Ursache der stummen Zweitverbindung ungeklaert                                                                                                                                                                                                                                                                  |

### 3.8al Upstream sync — PRs #1147, #1148, #1149, #1150 merged back — 2026-09-20

**Ausgangspunkt:** vier eigene PRs, alle am 2026-09-20 zwischen 16:12Z und 16:49Z von Kurt
uebernommen, und zwar als **echte Merge-Commits, nicht als Squash** — unsere Commits liegen
unveraendert in `upstream/dev` (`8f4f8877`, `97e5f1ff`, `45bddbe6` + `7526295b`, `39536d0d`;
Merges `b96cc510`, `bf16f40b`, `80b85a5a`, `bbc9eb66`). Handover waren die vier
`prNNNN-followup.md` auf dem Desktop.

**Was geliefert wurde:** `fork-main` per Merge auf `upstream/dev` gezogen (`287bc11c`,
textuell konfliktfrei, weil die Divergenz des Forks in diesen Dateien raeumlich neben den
gepatchten Hunks liegt); die neun Testfaelle zu `normalizeOwnCall` aus `callsign-ssid-fork`
als einzelne Datei uebernommen (`6d448cba`, `test_regex_call` 13 -> 22); der
Koordinatenvergleich aus `setlat`/`setlon` als header-only `src/coord_compare.h` heraus-
gezogen und mit `test/test_web_setcoord` (9 Faelle) festgenagelt (`f1f8d495`, rot-vorher
belegt: ueber einen `float` gefuehrt fallen 6 der 9). `fork-neo-test` gemerged (`804de972`)
plus Advisor-Nacharbeit (`4d162cdc`); `fork-neo` anschliessend mit `tools/neo/derive.sh`
neu abgeleitet statt von Hand gemerged (siehe unten). Gate: `fork-main` native 460/460,
`fork-neo-test` native 328/328, alle drei Zweige bauen `wiscore_rak4631` und
`heltec_wifi_lora_32_V3`; `fork-neo` zusaetzlich sauber neu gebaut, `derive.sh`-Gate leer.

**Zwei Dinge, die beim naechsten Mal Zeit sparen.** Erstens: `fork-neo` ist kein Zweig, den
man merged — `tools/neo/derive.sh` loescht ihn und baut ihn aus `upstream/dev` neu auf, jede
Handarbeit daran ist beim naechsten Lauf weg. Upstream-Fixes kommen dort also von selbst an.
Zweitens: `tools/neo/gate.sh` ist der teure Gate (8 Envs x 5 Commits = 40 Builds, erwartet
38/40) und wurde hier **nicht** gefahren; Ersatz war der saubere 2-Env-Build auf der Spitze
plus das Strip-Identitaets-Gate von `derive.sh`.

**Bewusst nicht gemacht, weiterhin offen:**

| ID     | Typ  | Prio | Ort                                                              | Item                                                                                                                                                                                                                                                                                                                                                                                                                          | Status                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------ | ---- | ---- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PRM-01 | BUG  | Low  | `src/udp_functions.cpp:522`, `src/nrf52/nrf_eth.cpp:689`         | Dasselbe tote `\| 0x20`-Idiom wie in #1148. Hier geht der rohe `convBuffer` auf die Luft, ein Wrap ist also nicht moeglich — aber die per Gateway eingespeiste BLE-Kopie traegt das App-Offline-Flag ebenfalls nie, weil `encodeAPRS()` es aus dem Bool serialisiert. Gleiche copy-scoped Loesung wie #1148.                                                                                                                  | offen, Kandidat fuer einen eigenen Upstream-PR, vor dem Oeffnen fragen                                                                                                                                                                                                                                                                                                                                                                      |
| PRM-02 | BUG  | Low  | `src/t-deck/event_functions.cpp:506` (`btn_event_handler_setup`) | Das Setup-Textfeld zeigt nach dem Speichern weiter den getippten Wert, dadurch loest jeder weitere Save-Klick ein ueberfluessiges `--setcall` samt `save_settings()` aus. Einzeiler: Textarea nach `commandAction()` aus `meshcom_settings.node_call` setzen, so wie `ui_deckpro.cpp` es schon macht.                                                                                                                         | offen, Kandidat fuer einen eigenen Upstream-PR, vor dem Oeffnen fragen                                                                                                                                                                                                                                                                                                                                                                      |
| PRM-03 | BUG  | Mid  | `src/phone_commands.cpp:435` (`case 0x50`)                       | Der App-Pfad fuer das Rufzeichen validiert ueberhaupt nicht — `checkRegexCall()` wird dort nie gerufen. Scharfstellen ist heute eine Regression: der Regex verwirft siebenstellige Basis-Rufzeichen wie `DL4ABCD`. Erst den Regex weiten, dann den Waechter setzen.                                                                                                                                                           | offen, eigenes Thema, nicht mit #1149 vermischen                                                                                                                                                                                                                                                                                                                                                                                            |
| PRM-04 | TEST | Mid  | Bench, Knoten mit gefuellter MHeard-Liste                        | Bench-Nachweis zu #1147 steht aus: Knoten mit 12+ MHeard-Eintraegen, `--bledebug on`, BLE-Connect — erwartet wird kein `BLEComToPhoneRingBuff overflow!` beim Connect und das `I`-Frame **vor** den `MH`-Frames. Der Fix ist bisher nur aus dem Code begruendet.                                                                                                                                                              | offen                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| PRM-05 | TEST | Mid  | McApp gegen einen Bench-Knoten                                   | #1149 auf dem BLE-Pfad 0x50 ist nur uebersetzt, nie auf Hardware gefahren. Rufzeichen mit `-01` aus den Node Settings der App setzen, danach `--info` oder `/getparam/?setcall`: erwartet `-1`. Dabei kein Testverkehr senden.                                                                                                                                                                                                | offen                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| PRM-06 | TEST | Low  | `mcapp.local:/var/lib/mcapp/messages.db`                         | Feldbestaetigung zu #1148, zwei bis vier Wochen nachdem die betroffenen Relays aktualisiert haben: kein `max_hop >= 7` auf Gruppen-/Broadcast-Frames und keine Pfadlaenge ueber 7. Die fuenf Wrap-Knoten stehen in Abschnitt 3.1 des Befunds; Firmware vorher ueber mcmap-prod `nodes_query` pruefen.                                                                                                                         | offen, fruehestens ab 2026-10-04 sinnvoll                                                                                                                                                                                                                                                                                                                                                                                                   |
| PRM-07 | TEST | Low  | mcmap-prod `nodes_query`                                         | Feldbestaetigung zu #1149: Zahl der Rufzeichen ohne `-NN` sollte fallen, sobald die Flotte aktualisiert. Anlassfall ist DL4ALF. Bleiben Knoten nach dem Update SSID-los, ist der Migrationswaechter oder der Formtest zu eng — zuerst die Basislaenge pruefen, der Formtest fasst nur 3-6 Zeichen an.                                                                                                                         | offen, fruehestens ab 2026-10-04 sinnvoll                                                                                                                                                                                                                                                                                                                                                                                                   |
| PRM-08 | TASK | Mid  | `neo-ram-reclaim`, `feature-neighbour-matrix`                    | Beide haengen an `fork-neo-test` und koennen die vier Fixes per Merge von dort nachziehen. Bei `neo-ram-reclaim` ist #1147 nach Absicht zu portieren: `addBLEComToOutBuffer()` heisst dort weiter so und hat dieselbe Signatur, aber die Byte-FIFO-Umstellung hat die Ring-Semantik im Inneren ersetzt.                                                                                                                       | **`neo-ram-reclaim`-Haelfte direkt auf `fork-main` nachgezogen, nicht gemerged** (`e2861458`+`9530967d`, 2026-09-21, CHANGELOG 230): `addBLEComToOutBuffer()` behaelt Name und Signatur, traegt jetzt aber denselben Byte-FIFO-Ring wie die anderen zwei Ausgangsringe, inklusive der zuvor fehlenden Sperre — genau das nach Absicht. `feature-neighbour-matrix` weiterhin offen. Auf Hardware noch nicht geprueft, siehe §3.8am `RAM-01`. |
| PRM-09 | BUG  | Low  | `src/configuration_global.h:261`, `src/esp32/esp32_main.cpp:317` | Advisor-Nebenfund zu #1147: auf der Variante `ENABLE_TBEAM` ist `MAX_RING` 10 und `json_configs_cnt` ebenfalls 10, der Ring fasst aber nur `MAX_RING-1` = 9. Die zehn Config-Frames allein verdraengen damit `config_cmds[0]` (`--info`) — dieselbe Fehlerform wie #1147, aber ohne MHeard. **Kein ausgeliefertes Env definiert `ENABLE_TBEAM`** (Kommentar dort: nur fuer Entwicklertests), also ist kein Release betroffen. | offen, hoechstens eine Bemerkung an Kurt wert                                                                                                                                                                                                                                                                                                                                                                                               |
| PRM-10 | DOC  | Low  | `docs/CHANGELOG-stability.md:208` und `:244`                     | Die Nummer **222 ist doppelt vergeben** — Extern-UDP-Originator-Keys und Safeboot-OTA-Kampagne. `docs/BACKLOG.md` §3.8ak spricht ausserdem von "Changelog 225" fuer den Extern-UDP-Eintrag, der dort 222 heisst. Nicht stillschweigend umnummerieren, es gibt Querverweise.                                                                                                                                                   | offen, vor der naechsten Release-Ausgabe klaeren                                                                                                                                                                                                                                                                                                                                                                                            |

**Was bewusst NICHT angefasst wurde.** Der Rufzeichen-Regex in `src/regex_functions.cpp`
bleibt unveraendert: er validiert auch die Quell-Rufzeichen **empfangener** Frames
(`src/aprs_functions.cpp`), eine Pflicht-SSID dort wuerde jeden Frame der bereits auf der
Luft stehenden SSID-losen Stationen verwerfen. `release-notes.md` und `release.md` sind
Momentaufnahmen je Release und wurden nicht angefasst; die vier Eintraege stehen als 225-228
in `docs/CHANGELOG-stability.md`.

### 3.8am Extern-UDP stack overflow fixed and field-verified; RAM reclaim ported — 2026-09-21

**Ausgangspunkt:** zwei getrennte Befunde am selben Tag. Erstens ein reproduzierbarer Absturz
auf DK5EN-98: ein Extern-UDP-`{"type":"msg"}`-Datagramm mit fremdem Ziel legte jeden ESP32-Knoten
mit `--extudp on` um, vier von vier Malen. Zweitens ein harter Blocker aus der laufenden
RAM-Rueckgewinnungs-Portierung von `neo-ram-reclaim`: `E22_XML-DevKitC` (in `default_envs`) linkte
seit `731e0ebc` nicht mehr.

**Stack overflow — FERTIG und gegated.** `c754d01f` (P1, `ARDUINO_LOOP_STACK_SIZE=12288`) +
`efc9681e` (P2/P3, `c_json`/`c_tjson` nach BSS auf ESP32 + `tools/stack_budget.py`-Gate). Feldnachweis
2026-09-21: der bisherige Reproduzierer laeuft jetzt durch (`stack_hwm 4668`), ein 9 h 36 min
Nachtlauf hielt `stack_hwm` konstant bei 4876 gegen vorher 368. CHANGELOG-Item 229, volle
Herleitung in `docs/bug-extudp-stack-20260920.md`. Beantwortet die ESP32-Haelfte von `UDP-01`
(§3.8l); die RAK/nRF52-Haelfte bleibt offen, `stack_budget.py` ist Xtensa-only.

**RAM-Rueckgewinn — komplett auf `fork-main`, Hardware-Verifikation OFFEN.** `e2861458` +
`9530967d` (Byte-FIFO-Ringe fuer Telefon-Daten, Telefon-Kommandos, UDP-Ausgang, CHANGELOG-Item 230) + `36b3a895` (Web-Header im String, `int16_t`-Zeilenkoordinaten, Langtext-Seiten nur auf
TFT/E-Paper, CHANGELOG-Item 232). Gate: 873/873 Host-Testfaelle, 14 Board-Envs SUCCESS
(`t5_epaper` und `esp32-external-radio` waren schon am unveraenderten HEAD rot). **Kein Board hat
diesen Build gefahren.** Zwei konkrete Luecken: der ESP32-Ausgangsring-Pfad (Byte-FIFO) und der
nRF52-Drain (eigene Implementierung in `nrf52_main.cpp`) — siehe `RAM-01` unten. `PRM-08` (§3.8al)
entsprechend aktualisiert.

**Gefunden, aber bewusst nicht gefixt: `--mesh off` deckt zwei eigene HF-Pfade nicht ab.**
Beim Suchen nach dem Via-Pfad-Fix (`45e411d4`, CHANGELOG-Item 231) bestaetigt: das Gateway-DM-ACK
(`lora_functions.cpp:1555`/`1579`) und die Custody-Zustellung des DM-Stores
(`msgstore_glue.cpp:113`) senden auf HF ohne `bMESH`-Pruefung. Beides sind eigene Emissionen des
...
| MHD-01  | BUG  | High | `src/mheard_functions.cpp:790` (`sendMheard()`)                       | Die Drossel vor `addBLEComToOutBuffer()` vergleicht `bf_used()` (Bytes **inklusive** gelesenem Verlauf) mit der Ringkapazitaet. `bf_pop()` senkt `bf_used()` nicht, nur die Verdraengung tut es — nach dem ersten vollen Ringumlauf steht `bf_used()` also dauerhaft dicht an `cap` und die Bedingung ist ab da immer wahr. `sendMheard()` kehrt dann sofort zurueck: die MHeard-Liste erreicht das Telefon nie wieder. Am echten Ring nachgestellt (2048 B, 200-B-Frames, Drain leert nach jedem Push): `used` bleibt ab Frame 10 bei 2010 stehen, die Bedingung ist ab da dauerhaft wahr. | **erledigt 2026-09-21** (CHANGELOG 233). Die Drossel schrankt jetzt gegen die **ungelesenen** Bytes (`bf_unread() * 256`) statt gegen `bf_used()`. Das Praedikat liegt als reine Funktion in `src/mheard_throttle.h` (Muster `coord_compare.h`), weil `mheard_functions.cpp` Arduino und ArduinoJson hereinzieht und sonst nicht host-pruefbar waere. Drei Faelle in `test/test_byte_fifo/`: der Drain-Fall ist mit der alten `bf_used()`-Bedingung rot und mit dem Fix gruen, der Gegenfall zeigt, dass die Drossel ueberhaupt greift, der dritte nagelt die Einheiten fest. Gate: 876/876 Host-Testfaelle, 6 Board-Envs SUCCESS. |
