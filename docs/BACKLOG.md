...
nur `GPSPI2.clock`). Offen bleiben: TD-09, E22-01, TLM-01..03, MEM-02
...
| TM-46 | ESP32 safeboot | BUG | Low | `src/safeboot/`, `tools/webflash.py` | **Safeboot recovers dirty from an aborted OTA upload.** Found 2026-08-31 flashing the release: on the T-Deck (largest image of the fleet, 2.17 MB, WLAN at the documented -74 dBm margin) the upload stalled and the safeboot abandoned the session mid-transfer and rebooted into the app (anti-brick fallback, tool saw the OLD build come up, then `Errno 32`). Correct so far — but the SECOND attempt then got **HTTP 400** on a fresh `/ota/start`: the aborted session leaves state behind, and the node was still serving the OTA page minutes later. A safeboot should accept a clean new session after any abort. Repro: interrupt an OTA upload, retry. USB flash is the workaround (used for DK5EN-14). | open |
| TM-47 | tools | BUG | Low | `tools/webflash.py` (`ENV_HARDWARE`) | **`webflash.py` hardware check blocks T-Deck Plus:** the map expects `TDECK_PLUS` for env `t_deck_plus`, but the firmware self-reports `TDECK` (web UI shows `TDECK+`) — every legitimate flash needs `--force`, which defeats the wrong-board guard exactly where boards are most similar. Align the expected string with what the firmware actually reports. | open |
