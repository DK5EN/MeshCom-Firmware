...
| RF-08   | LEAD | Medium | **Country 5 (868 MHz) carries `track_freq = 999`, and on ESP32 that value reaches the radio.** Every other country sets `node_track_freq` to `LORA_APRS_FREQUENCY` or a real frequency (Poland uses 434.855); case 5 alone assigns the literal `999` (`src/country_profile.cpp`, the block carved out of `lora_setcountry()` unchanged). On the SX127x path `lora_setchip_aprs()` takes the APRS frequency straight from that field (`lora_setchip.cpp:455`, `float rf_freq = meshcom_settings.node_track_freq;`), so an ESP32 node on `--setctry 5` with track enabled would configure the APRS radio to **999 MHz**. The SX126x path is unaffected: it uses `LORA_APRS_FREQUENCY` directly (`:441`), which is another platform split.                                                                                                | **OPEN, unverified, filed as a lead not a claim.** Found 2026-09-11 by `test_country_twin`: the value stands out the moment all 15 codes are tabulated side by side, which is what characterization tests are for. Pre-existing -- the C5 carve moved this block byte-identically. Open questions before it can be called a field bug: is `999` a deliberate sentinel meaning "no APRS frequency in this region" that something downstream honours, and is track mode reachable at all on an 868 MHz node? Neither is answered by reading `lora_setchip_aprs()`, which contains no guard on the value |
