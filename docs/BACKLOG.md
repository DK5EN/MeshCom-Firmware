...
| TM-44  | RAK4631                       | BUG  | Low  | `src/nrf52/nrf_eth.cpp` `udpUpdateTimeClient()` (`EthStall("ntp")`, ~:1185), `src/ntp_async.cpp` send path, W5100S RTR/RCR defaults | **1.6 s loop-task block at the `ntp` site while the WAN is dead — first real catch of TM-35's recurring-stall marker** (`[ETH];stall;ntp;ms;1609`, TM-38 run `apreboot_ap1_20260830-220423`, t0+65 s: LAN link already back, router still rebooting). Not DNS — this path uses a literal IP (`nrf_eth.cpp:1159`). Prime suspect: the W5100S's **on-chip retry schedule blocking the UDP send while the gateway's ARP entry is unanswered** — chip defaults RTR 200 ms x RCR 8 = 1600 ms, matching the measured 1609 ms almost exactly; `nrf_eth.cpp` never configures RTR/RCR. Candidate fixes, in order: skip the NTP request while the gateway is unreachable (cheap gate), or lower RTR/RCR for bounded sends (affects every socket — needs care), or accept (1.6 s once per WAN outage, self-recovering). Reproduction without killing the bench Mac's session: detached capture (`meshlogger.py` or a `serial_session` redirect) on the RAK, then pull the **router uplink** (not the RAK's LAN cable — that tests the link-down path, which is gated and never sends) for ~2 min around a 15-min NTP refresh, reconnect, analyse offline. | open — single occurrence, low severity, filed from the TM-38 run                                                                                                                                                                                                                                                                                            |
