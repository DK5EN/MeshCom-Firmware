...
| TD-09  | Medium               | T-Deck Karte (`tdeck_sdmap.cpp`) | Tile-Cache fuer die Karte (PSRAM): ohne ihn kostet jeder TD-07-Pan-Schritt 0,33-0,79 s (PNG-Decode ~170 ms/Kachel dominiert). Von TD-07 abgespalten per Operator-Entscheid 2026-08-31. | open                                          |
...
### 3.8p Wire-Hygiene, JSON, T-Deck, Doku (intake 2026-08-31, operator list)

Operator intake 2026-08-31 (aus einer parallelen Analyse-Session uebernommen; Kurztexte
sind die dortigen Befunde). Dazu BAT-02 als Nachtrag aus Wave 2.

| ID      | Sev.   | Location                                                                                                        | Item                                                                                                                                                                                                                                                                                                                                                             | Status                                                                                                                                                                                                                                                                     |
| ------- | ------ | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CHR-01  | Medium | `aprs_functions.cpp:380` (`decodeAPRS`), `:1068` (`encodePayloadAPRS`)                                          | Zeichensatz-Filter fuer Nachrichtentext, RX (LoRa + UDP) und TX. Zwei Chokepoints decken alles: decodeAPRS() fuer beide Empfangswege, encodePayloadAPRS() fuer alle Sender. Offene Designfrage: MCU-taugliche Naeherung der MCProxy-Regel (kein `unicodedata.category()` auf dem Node).                                                                          | **FIXED 2026-08-31** (`361ac9fe`): UTF-8-Allowlist (Operator-Entscheid) in src/charset_filter.*, Chokepoints decodeAPRS/encodePayloadAPRS; 15 native Faelle, APRS-Korpus unveraendert gruen                                                                                |
| CHR-02  | Medium | APRS-Freitext: `pos_atxt`, `node_atxt`, HEY                                                                     | Derselbe Filter fuer APRS-Freitext. Struktur-Trennzeichen, die er nicht anfassen darf, sind bekannt; Falle: die 25-Byte-Grenze kann eine UTF-8-Sequenz halbieren.                                                                                                                                                                                                | **FIXED 2026-08-31** (`361ac9fe`): Separator-Strip-Modus ({ } : ; , /) an allen drei atxt-Frame-Build-Stellen, UTF-8-sichere Truncation bei 25/16 Byte; --setatxt-Speicherstelle bewusst ungefiltert (Frame-Build deckt alle Writer)                                       |
| JSN-01  | High   | drei Web-JSON-Endpunkte (u.a. `web_functions.cpp:1920`-Klasse), `extudp_functions.cpp`, 13 BLE-Register-Builder | Vier kaputte JSON-Erzeuger: Web-Endpunkte escapen gar nichts (offener HIGH aus `code-audit-20260508.md`); EXTUDP escaped doppelt und nutzt `measureJson()` statt `sizeof(buffer)` — erreichbar von jedem unauthentifizierten LoRa-Peer; 13 BLE-Register-Builder mit demselben Bug plus Truncation mitten im String. Referenzfix existiert in `ble_json_frame.h`. | **FIXED 2026-08-31** (`361ac9fe`): drei Web-Endpunkte streamen serializeJson; EXTUDP ohne strEsc-Doppel-Escape, sizeof(buffer)-Bound; 13 BLE-Builder ueber sendBleJsonRegister() + bleJsonFrameFailSoft() (Feld-Drop statt Mid-String-Cut); +3 native Faelle               |
| PM-01   | Medium | `node_sset3`, EXTUDP                                                                                            | `NoPMOther`, gescoped auf EXTUDP. Freies Bit 0x8000 in `node_sset3`, kein Struct-Bump, kein Fleet-Wipe. Polaritaet so waehlen, dass die Bestandsflotte mit 0 den neuen Default bekommt.                                                                                                                                                                          | open                                                                                                                                                                                                                                                                       |
| NTP-01  | Medium | `ntp_async.*`, Caller in `esp32_main.cpp`                                                                       | Refresh ist 15 min, extern getrieben — die 1-h-Einstellung im Code ist tot, weil der Caller `requestNow()` erzwingt. Retry 2,5 s/5 s/60 s. Dazu: Bericht, `--ntpsync`-Kommando (Template: `--srvip`-Hook) und Bench-Szenario.                                                                                                                                    | open                                                                                                                                                                                                                                                                       |
| TD-07   | Medium | T-Deck Karte                                                                                                    | Pan auf i/j/k/l + Recenter-Taste. Vier Auto-Recenter-Stellen muessen gegated werden, sonst kaempfen sie gegen den Pan. Offen: Tile-Cache als Voraussetzung — ohne ihn 0,33-0,79 s pro Schritt.                                                                                                                                                                   | **DONE 2026-08-31** (`361ac9fe`): Pan i/j/k/l (1/4 Viewport), Recenter o, vier Auto-Recenter-Stellen gegated (inkl. esp32_main 30-s-Poll); ohne Tile-Cache 0,33-0,79 s/Schritt akzeptiert (Operator-Entscheid) -> TD-09. Bench-Skript im Agent-Report, Hardware-Test offen |
| TD-08   | Low    | T-Deck Tastatur                                                                                                 | Sym-Eingabemodus remapped a-z vor dem Map-Dispatch, g/h sind dort schon heute tot.                                                                                                                                                                                                                                                                               | **FIXED 2026-08-31** (`361ac9fe`): Map-Tasten dispatchen vor dem Sym-Remap, g/h-Zoom im Sym-Modus wieder aktiv                                                                                                                                                             |
| WEB-04  | Low    | `web_setup.cpp:407-412`                                                                                         | Der `sendpos`-Setup-Handler schaltet `--nomsgall` — Copy-Paste aus dem Block darueber (verifiziert).                                                                                                                                                                                                                                                             | **FIXED 2026-08-31** (`361ac9fe`): sendpos-Setparam-Handler feuert sendpos/sendtrack statt --nomsgall                                                                                                                                                                      |
| CTY-01  | Low    | `nrf_eth.cpp` DHCP-Pfad                                                                                         | nRF52-DHCP-Pfad kennt keine Laenderunterscheidung, der Fix-IP-Pfad schon. In `bench-country-servers.md` nicht erfasst.                                                                                                                                                                                                                                           | **FIXED 2026-08-31** (`a1191eaa`): startUDP() Internet-Zweig mit IT/else-Split wie startFIXUDP(); bench-country-servers.md ergaenzt                                                                                                                                        |
| BAT-02  | Medium | `batt_function_old.cpp` (NRF52_SERIES-Zweig)                                                                    | Die RAK/nRF52-Batterieerkennung teilt die BAT-01-Luecke: keinerlei No-Battery-Erkennung im nRF52-`read_batt()`-Zweig. Detektor aus BAT-01 dorthin spiegeln (gleiche Konvention `global_batt==0` -> USB).                                                                                                                                                         | **FIXED 2026-08-31** (`a1191eaa`): Detektor-Guard auf den RAK4631-Zweig geweitet (T114/T-Echo ausgenommen); 30-s-Lesetakt -> Erkennung nach ~3 min; Bench-Proof auf DK5EN-90 offen                                                                                         |
| PRES-01 | Low    | `docs/presentation/`                                                                                            | Meshtastic (Repo 2020-02-01) und MeshCore (2025-01-19), beide primaerquellenbelegt, beide widersprechen kursierenden Angaben. Fussnote mit den echten BNetzA-Zahlen (869,4-869,65 MHz: 500 mW ERP, 10 % DC) und AFuV Par. 16 Abs. 8 im Wortlaut. 33 cm gibt es in Region 1 nicht — nicht behaupten.                                                              | **DONE 2026-08-31** (Docs-Commit): Meilensteine + BNetzA-Fussnote + AFuV Par. 16 Abs. 8 woertlich (doppelt gefetcht) in meshcom-protocol.html; Deck enthielt vorher keine dieser Angaben (add statt fix)                                                                   |
| PRES-02 | Low    | `docs/presentation/`                                                                                            | Digimode-Timeline: WSJT 2001, WSPR 2008, WSJT-X 2012-10-25, FT8 2017-06-29, JS8Call GA 2019-04-01. Fuenf Eintraege in den dichten rechten Bereich — Achse neu rechnen.                                                                                                                                                                                           | **DONE 2026-08-31** (Docs-Commit): fuenf Digimode-Meilensteine, Achse 8->15 Punkte, viewBox 960->1360                                                                                                                                                                      |
| DOC-01  | Medium | `docs/presentation/` (neu)                                                                                      | Main-Loop-HTML mit allen gemessenen Vorher/Nachher-Zahlen. RX-Verlustmodell ist auf ESP32 und nRF52 verschieden und darf nicht vermischt werden; "Pakete pro Sekunde Stall" ist nirgends gemessen — nicht erfinden.                                                                                                                                              | **DONE 2026-08-31** (Docs-Commit): docs/presentation/main-loop-timing.html; RX-Verlustmodelle getrennt, kein erfundener Stall-Durchsatz                                                                                                                                    |
| DOC-02  | Medium | `--help` (`command_functions.cpp`)                                                                              | Zwei ausgelieferte Bugs: `--settime` steht mit U+2212 statt ASCII-Minus drin (Copy-Paste schlaegt fehl); fuenf T-Deck-Befehle werden auf jedem Board beworben. ~60 Befehle fehlen, einer (`--SS on`) existiert nicht. Clean/Dev-Schnitt liegt nicht, wo er aussieht: `INSTRUMENT_ENABLED` ist per Default an, die Bench-Befehle sind in jedem Board-Build.       | open                                                                                                                                                                                                                                                                       |
| DOC-03  | Medium | `docs/` (neu)                                                                                                   | Endianness/Kompatibilitaet: CONF-Koordinaten sind little-endian (fruehere Doku sagte big-endian), kein `htonl` im ganzen Baum, IPs als ASCII gespeichert. Der eigentliche Kompatibilitaetshebel: ESP32 speichert per NVS-Key, nRF52 den ganzen Struct als Blob.                                                                                                  | **DONE 2026-08-31** (Docs-Commit): docs/wire-compat.md; Korrektur: 2x htons (sin_port) statt null Treffer                                                                                                                                                                  |
| DOC-04  | Medium | `docs/` (neu)                                                                                                   | Register-Referenz: 107 (ESP32) / 103 (nRF52) Register. Ein Leichenfund: `node_gpsbaud` hat nirgends einen Leser. Dazu vier Key/Member-Namensdreher und fuenf Secret-Felder, von denen nur drei dokumentiert sind.                                                                                                                                                | **DONE 2026-08-31** (Docs-Commit): docs/settings-registers.md; 107/103 bestaetigt, node_gpsbaud tot, 4 Namensdreher, 5 Secrets (node_opwd/node_passwd bisher undokumentiert), 6 Boot-Clamp-Divergenzen                                                                     |

Dazu offen aus 3.8o: NC-02, MH-02, ETH-01, CONF-01 — werden in derselben Kampagne umgesetzt.
