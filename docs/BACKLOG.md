...
**Last updated 2026-09-11 — APRS parser contract campaign, §3.8ae; previous update 2026-09-05
...
`ack-heard-foreign-msgids-fix.md`. `WQ-02` **DONE** (`b1cc8cd5`, item 194): the QRS marker in
...
> `CHANGELOG-stability.md`, `pr-draft-20260831.md`, `code-quality-2.0.md` — refers to. The item
...
| TLM-05 | GAP  | Low  | `src/mcp17_bits.h` (new), `src/loop_functions.cpp` `PositionToAPRS()`/`sendTelemetry()`, `src/aprs_functions.cpp` `decodeAPRSPOS()`, `src/extern_tele_json.h`         | **MCP23017 port A inputs were read every 5 s but never left the node** (upstream issue 1076: the `T#` digital slot was hardcoded `00000000`; a second request asked for the bits in the position beacon). **IMPLEMENTED 2026-09-06 (`b179fdff`, changelog 207):** `/D=<8 bits>` in the beacon and the same string in the `T#` digital slot, GPA0 first, output pins `0`, only when the chip answered at boot; receiver parses `/D=` into `aprsPosition.din`, EXTUDP `tele` gains an optional `din` key. 19 native cases, bench proof of the receive/export path on `DK5EN-93` via `--injectraw`. **Open:** transmit path unproven on hardware (no MCP23017 on the bench); server/app support is the upstream PR (`pr-draft-mcp17-din-20260906.md`); reference `mcp23017-digital-field.md`, §3.8z; beacon-on-change deliberately not implemented, regular interval only. |
...
| What the MCP23017 `/D=` field is and why `D`          | `docs/mcp23017-digital-field.md`                      |
