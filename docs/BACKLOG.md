...
| TD-03 | BUG   | High   | `lv_obj_functions.cpp:2770` (H1), `esp32_audio.cpp` (C2, fixed in TM-01..04) | Heap defect: rendered message list never trimmed (~250 B/msg internal, `abort()` at ~390). **Next run, high priority** (decision 2026-08-29). | open        |
...
| TM-27 | Heltec, T-Beam, E290 | TEST | Medium | `loop_functions.cpp` `sendDisplay1306()` | Pixel-level verification for OLED/e-paper: the U8g2 frame buffer is in RAM — print a CRC32 of the buffer per push (`[OLED];crc`) so the harness can assert screen content, which the T-Deck cannot (no panel readback). | open — Heltec + T-Beam (U8g2 buffer CRC); T-Deck has no readback |
