...
| DS-01  | ESP32 (all non-e-ink boards), nRF52 (RAK4631)                                                                                                                        | BUG  | Medium | `src/command_functions.cpp:1042-1118` (`--deepsleep`), `src/batt_functions.cpp:428-463` (low-battery guard, dead code)                                                                                                                               | Verdict for upstream issue #962: the automatic low-battery deepsleep is not misbehaving, it is switched off entirely — upstream commented the whole block out for issue #1053 in `e0043a56` (v4.35p.07.11), for every board. `--deepsleep` itself also arms no wake source, leaves the LoRa chip in RX, never powers down the OLED or (on T-Beam) the PMU rails outside the `WP_DISP` branch; e-ink boards already got the fix template via upstream PR 1050. RAK4631 `--deepsleep` is a no-op (`esp_deep_sleep_start()` excluded). Full analysis and implementation plan: `issue-962-deepsleep-verdict.md`.                                                                  | **open — verdict only, no fix committed.** Recommendation: fix `--deepsleep` itself (small, uncontroversial) plus bring back the low-battery guard as an opt-in timer-wake loop with hysteresis. Not started.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
