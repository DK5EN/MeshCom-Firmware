...
| TD-07   | Medium | T-Deck Karte                                                                                                    | Pan auf i/j/k/l + Recenter-Taste. Vier Auto-Recenter-Stellen muessen gegated werden, sonst kaempfen sie gegen den Pan. Offen: Tile-Cache als Voraussetzung — ohne ihn 0,33-0,79 s pro Schritt.                                                                                                                                                                   | open   |
| TD-08   | Low    | T-Deck Tastatur                                                                                                 | Sym-Eingabemodus remapped a-z vor dem Map-Dispatch, g/h sind dort schon heute tot.                                                                                                                                                                                                                                                                               | open   |
| WEB-04  | Low    | `web_setup.cpp:407-412`                                                                                         | Der `sendpos`-Setup-Handler schaltet `--nomsgall` — Copy-Paste aus dem Block darueber (verifiziert).                                                                                                                                                                                                                                                             | open   |
| CTY-01  | Low    | `nrf_eth.cpp` DHCP-Pfad                                                                                         | nRF52-DHCP-Pfad kennt keine Laenderunterscheidung, der Fix-IP-Pfad schon. In `bench-country-servers.md` nicht erfasst.                                                                                                                                                                                                                                           | open   |
| BAT-02  | Medium | `batt_function_old.cpp` (NRF52_SERIES-Zweig)                                                                    | Die RAK/nRF52-Batterieerkennung teilt die BAT-01-Luecke: keinerlei No-Battery-Erkennung im nRF52-`read_batt()`-Zweig. Detektor aus BAT-01 dorthin spiegeln (gleiche Konvention `global_batt==0` -> USB).                                                                                                                                                         | open   |
| PRES-01 | Low    | `docs/presentation/`                                                                                            | Meshtastic (Repo 2020-02-01) und MeshCore (2025-01-19), beide primaerquellenbelegt, beide widersprechen kursierenden Angaben. Fussnote mit den echten BNetzA-Zahlen (869,4-869,65 MHz: 500 mW ERP, 10 % DC) und AFuV Par. 16 Abs. 8 im Wortlaut. 33 cm gibt es in Region 1 nicht — nicht behaupten.                                                              | open   |
| PRES-02 | Low    | `docs/presentation/`                                                                                            | Digimode-Timeline: WSJT 2001, WSPR 2008, WSJT-X 2012-10-25, FT8 2017-06-29, JS8Call GA 2019-04-01. Fuenf Eintraege in den dichten rechten Bereich — Achse neu rechnen.                                                                                                                                                                                           | open   |
| DOC-01  | Medium | `docs/presentation/` (neu)                                                                                      | Main-Loop-HTML mit allen gemessenen Vorher/Nachher-Zahlen. RX-Verlustmodell ist auf ESP32 und nRF52 verschieden und darf nicht vermischt werden; "Pakete pro Sekunde Stall" ist nirgends gemessen — nicht erfinden.                                                                                                                                              | open   |
...
| DOC-03  | Medium | `docs/` (neu)                                                                                                   | Endianness/Kompatibilitaet: CONF-Koordinaten sind little-endian (fruehere Doku sagte big-endian), kein `htonl` im ganzen Baum, IPs als ASCII gespeichert. Der eigentliche Kompatibilitaetshebel: ESP32 speichert per NVS-Key, nRF52 den ganzen Struct als Blob.                                                                                                  | open   |
| DOC-04  | Medium | `docs/` (neu)                                                                                                   | Register-Referenz: 107 (ESP32) / 103 (nRF52) Register. Ein Leichenfund: `node_gpsbaud` hat nirgends einen Leser. Dazu vier Key/Member-Namensdreher und fuenf Secret-Felder, von denen nur drei dokumentiert sind.                                                                                                                                                | open   |
