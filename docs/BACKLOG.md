...
merged upstream PRs had overtaken and are corrected: `BP-11` is **upstream in PR #1135**, not
fork-only; `TD-10` and `SL-01..07` are **upstream in PR #1125**; `TLM-05` is **upstream in
PR #1135**. What that leaves owed on those rows is bench work, not code.

**Previous update 2026-09-11 — APRS parser contract campaign, §3.8ae; before that 2026-09-05
late evening (paragraphs below), grooming 2026-09-03 evening, §3.8x.** Since 09-05: releases
v4.35s.09.06 / 09.06.2 / 09.09 and v4.35t.09.10 (upstream sync, letter `t`), TRK-01 (§3.8ac),
CHR-03, INS-01..03 (§3.8aa), CTY-02 withdrawn (§3.8ab), the DM transport concept (§3.8ad,
...
gates only, **no board flashed since v4.35s.09.09**. BP-11 (item 179) is upstream in PR #1135
(section 3); no upstream PR is owed, and the earlier "stays fork-only" reading is wrong. Also closed 2026-09-11: the "Standing risk" box (`N-03..N-06` +
`SEC-02`/`N-18` upstream since PR #1102), the German PR texts for `TD-12`/`TD-13`/`CDC-01`
(upstream in PR #1135), and the `--track on` lead from §3.8ac (not chased). **Newly filed 2026-09-11: §3.8af** — the DRY
unification / RAM audit of 2026-09-10 (`optimization-audit-20260910.md`) was never referenced
here; now `OPT-01..04` (one-shot PR plan, golden capture, decisions, gated leftovers) and
`OPT-D1..D13` (thirteen defects, all still present). Still open: `MH-01`, `MH-03`, `MEM-04`, `GPS-10`,
...
`CQ-02..CQ-12`, `SL-01..07`, `DM-01..DM-06`, `APRS-02..APRS-04` (§3.8ae), `OPT-01..04` +
`OPT-D1..D13` (§3.8af), ACK stage 4 / R5,
...
`MH-03`, `MEM-04`, `GPS-10`, `INS-03`, `TD-09`, `TD-10`, `E22-01`, `TLM-01..03`, `MEM-02` (parked),
...
**2026-09-05 afternoon (four items, all on `fork-main`, pushed):** `WQ-01` **DONE** — LoRa queue
panel on the rxlog web page (TX ring per priority as a JS-rendered cell bar with QRS/QRT marks,
back-pressure state, dedup memory window against the 40-48 min corridor, channel utilisation of
the last 5-min STAT window; collapsible, fixed 600 px, three text sizes on the page; verified on
dk5en-98). `TD-12` **FIXED** (tab switch after Send/Save without animation, `3f6a35d5`) and
`TD-13` **FIXED** (trackball button pulses on the press edge only, `7368539e`) from the two
Desktop plans — both plan numbers collided with existing IDs and were renumbered; `TD-13`
bench-proven on DK5EN-14 after the harness inject was rebuilt to model press/hold/release,
`TD-12` closed by the operator without the hand test. The operator's "cursor dead time" report then
turned into two findings: `TD-14` (map recomposed twice when the map tab is picked from the tab
bar, plus zoom/beacon rebuilds — TD-09 cost, filed) and the real cause `CDC-01` **FIXED**
(`f3fddc91`: HWCDC prints blocked the main loop 100 ms each once the USB host was gone; proof via
...
~~the German PR texts for `TD-12`/`TD-13`/`CDC-01`~~ (all three went upstream in PR #1135, 2026-09-10; TD-14 text: `archive/pr-tdeck-maptab-draft-20260905.md`).
...
`ack-implementierungsplan.md` §7). The bench then surfaced `ACK-01` **FIXED** (`b2336e6f`,
item 193): a gateway sent one heard and one gateway-ACK status frame to the phone for every
foreign message it had forwarded from the server, because `own_msg_id[]` holds those msg_ids
and the legacy first-frame branch was not origin-gated; McApp had booked ~150 false
`send_success` rows per day since before 08-26. Gateway-only — the LoRa relay path never
inserts. Only the BLE emit is gated now, state writes untouched (web rxlog ticks survive).
Verification is manual via McApp on 98, by operator decision; evidence and query in
`archive/ack-heard-foreign-msgids-fix.md`. `WQ-02` **DONE** (`b1cc8cd5`, item 194): the QRS marker in
the queue panel forecasts the depth at which the operator's next own messages raise QRS
instead of drawing the fixed line. Open from the ACK work: stage 4 (gateway hash appendix on
the wire, `--ackinfo` coupling), R5 (no cap on heard frames per msg_id with the flag on), and
the pre-existing quirk that both UDP peer-ACK sites push a 0x01 frame to the phone even when
`checkOwnTx()` fails (noted in the fix doc, not filed).
...
Upstream-Nachtrag: [`archive/pr-draft-20260831.md`](archive/pr-draft-20260831.md)** — offene
...
[`archive/tdeck-backspace-autorepeat-20260831.md`](archive/tdeck-backspace-autorepeat-20260831.md)), E22-01, TLM-01..03, MEM-02
...
**2026-09-09 (evening, concept only, nothing in code):** DM transport reliability designed
end to end, §3.8ad. Root cause verified in code: the 40 s same-`msg_id` retry is swallowed by the
dedup gate at every node that heard the original, the destination included
(`src/lora_functions.cpp:896`), so it repairs first-hop loss only and never a lost ACK, and giving
up is silent. Four stages filed as `DM-01..DM-04`, counters as `DM-05`, open questions `DM-06`.
Two documents: `proposal-dm-transport-reliability-20260909.md` (English) and
`konzept-dm-transportsicherung-20260909.html` (German, with an interactive simulator and a
352-run outcome matrix; a copy sits on the Desktop). Supersedes `concept-dm-store-and-forward.md`
where they differ: NNN stays stable, only the msg_id is fresh per re-flood; no probes; classic
ESP32 included via parked ring slots.
...
| G12 | Timing (all at once vs. spread over weeks) — **decided 2026-09-03: all at once.** See §3.7    |
...
| #   | Decision                                                                                                                                                                            | Status                                                                                                                                                                                                                                                                                                                                      |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G12 | All at once, or spread over weeks?                                                                                                                                                  | **DECIDED 2026-09-03: all at once — CLOSED.** Overtaken by events rather than argued: the campaign ran as one continuous push, and its output is already upstream (§3.8q PRs #1115/#1117/#1118) and released on the fork as v4.35s.09.03-stability. §3.6a's sizing table is kept as the record of what the estimate looked like beforehand. |
...
root cause, fix, and verification per item: [`archive/bug-loganalyse-toolchain-20260824.md`](archive/bug-loganalyse-toolchain-20260824.md).
...
| TOOL-06 | DOC         | Low    | `.claude/skills/logauswertung/SKILL.md` §12,§15 | "any rc!=0 is a bug" is wrong; align with TOOL-01/02                          | **DONE**   |
...
section of [`archive/bug-loganalyse-toolchain-20260824.md`](archive/bug-loganalyse-toolchain-20260824.md).

**TOOL-06 re-verified on disk 2026-09-03** (worth checking: git-ignored skill files do not travel
into worktrees, so this is exactly the kind of edit that gets silently lost). The single copy is
`.claude/skills/logauswertung/SKILL.md` in this repo — there is no second copy under
`~/.claude/skills/` — and all four corrections are present: the State-Machine section reports the
`MC_SM_ERRORS` / `MC_SM_CSMA_BACKOFF` split and names `TX_PREPARE -> IDLE rc=-1` as normal CSMA
backoff rather than a fault; the Status-Criteria block keys FEHLER on `MC_SM_ERRORS>0`, not on any
`rc!=0`; the priority-drop breakdown is described as the _dropped_ packet's own `prio`+`type` with
heartbeat shedding called out as by design; and the high-hop section carries the note that `H` is
`max_hop` remaining, not path length. Nothing left to change. It stays uncommittable:
`.claude/…/SKILL.md` is git-ignored (`.gitignore` rule `.*`), so traceability lives here and in the
bug document. CI wiring was deliberately deferred (test is local-only). The `git add -p`-based per-id split
...
| ID    | Type  | Sev.   | Location                                                                                                   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Status                     |
| ----- | ----- | ------ | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| TD-01 | BUG   | High   | `udp_functions.cpp:629-698`                                                                                | WLAN association fails on ~50 % of boots; BSSID pinning prevents fallback to the 2nd AP                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **CLOSED 2026-08-30**      |
...
| TD-04 | TASK  | Medium | `tdeck_sdmap.cpp`; SD card                                                                                 | Install Europe map tiles from `download.tiles.coalition.space`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | **CLOSED 2026-09-03**      |
| TD-05 | PERF  | Medium | `lv_obj_functions.cpp` (4313 lines)                                                                        | GUI is sluggish — cause unidentified                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | **CLOSED 2026-09-03**      |
| TD-06 | TEST  | High   | `tools/`                                                                                                   | Automated test harness driving the device over serial + net console                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **DONE 2026-09-03**        |

#### TD-01 — WLAN association, and the "wrong access point?" question

**CLOSED 2026-08-30 — the row above was left stale until 2026-09-03.** The fix landed with TM-34
Wave W (join by SSID only, driver-owned AP selection, PMF off) and TM-11 closed on the same
evidence: a 24-boot run on `DK5EN-14` gave **24/24 first joins, 0 disconnects, 0 connection-error
bursts, 0 `[WIFI];stall`**, `got_ip` median 10 394 ms / max 10 437, ready median 10 949 ms
(`bootloop_TD01_close_20260830_20260830-134157/`). The chain in full: 4/12 (TM-11 baseline) →
0/24 at 55.8 s (arm A0, BSSID pin + SAE) → 24/24 at 14.1 s (Wave W) → 24/24 at 10.4 s (after
TM-16). Record in RESUME.md, "2026-08-30 late".

Intake, kept for the record. **It does not pick the wrong AP.** `startNetwork()` scans all channels, selects the strongest RSSI
...
**CLOSED 2026-09-03 (operator).** The Europe tile set is installed on the card and the map renders
on `DK5EN-14`. Nothing in the firmware had to change for it.

**One finding does not close with it.** `G09` (`tdeck_sdmap.cpp:216`, unbounded
`malloc(f.size())` on the PNG raw buffer) is still in the tree and still unfixed — it is only no
longer a _gate_: the bulk install is done, so the remaining exposure is a single corrupt or
oversized tile file rather than a 30 GB population run. `G10`/`G11` are likewise still open in
§3.8c; the populated card demonstrates that neither is fatal in practice (`G10`'s compile-time
clamp accommodates the installed set, `G11` needs a failed load to bite). Track them there, not
here.

Original intake, kept for the record. Card present and healthy: **SDHC, 30 436 MB**, mounts under MeshCom (the LilyGo factory firmware
...
**CLOSED 2026-09-03 (operator).** §3.8e answered the cause — `full_refresh = 1` with a single
buffer and non-DMA `pushColors` at 27 MHz meant ~45 ms of blocking SPI on _every_ invalidation,
including the 1 Hz clock tick, on the bus that also fronts SD and LoRa (`LV_ANIM_ON` scroll
multiplied it to 200-300 ms per message). Partial refresh is now the shipped setting
(`src/t-deck/tdeck_main.cpp`, `disp_drv.full_refresh = 0`) and works on hardware; the
`// EXPERIMENT` comment has been replaced by the rationale. `G07` (draw-buffer size in pixels, not
bytes) was the prerequisite and is already fixed — partial refresh is exactly the mode that would
have turned it into a ~150 kB overflow.

Original intake, kept for the record. Unquantified at the time: `lv_obj_functions.cpp` is 4313 lines
and builds the whole widget tree; the LVGL tick/refresh path, the SD map lookups on the draw path,
and `Print::printf` heap churn were the three suspects.

#### TD-06 — automated tests

**DONE 2026-09-03 — the row was left on "in progress" long after the rig was in daily use.** The
missing piece named below, `--wifi on/off` (HL-01), shipped
(`command_functions.cpp:4821`), so a virgin node can be brought fully online unattended.

What exists and is used: `tools/bench/tdeck_harness.py` (2304 lines, **20 scenarios** — `idle`,
`tabs`, `drawer`, `inject`, `audio`, `audio_stall`, `map`, `nav`, `input`, `heap`, `trim`, `boot`,
`uptime`, `displaycmd`, `screen`, `sleep`, `disptest`, `gps_experiment`,
`flush_lora_correlation`, `touch_inject`), holding one serial session open for a whole run;
`tdeck_parse.py` for offline re-scoring; the sibling rigs `rak_harness.py`, `oled_harness.py`,
`soak_harness.py`, `ota_regression.py`, `extudp_peer.py`, `serial_session.py` and the
`tools/bench/experiments/` set. The findings it carried are throughout §3.8b/§3.8e/§3.8f — TD-03's
`trim` arm, TM-33's `displaycmd` 0/4 → 4/4, TD-10's key-repeat verdict, TM-16's 24-boot numbers.

Non-perturbing observation over the net console 2323 is served by a **separate** tool by design —
`tools/meshlogger.py` records a node's console to disk for days, and the console is single-client,
so folding it into the harness would fight the harness's own session. The second-node leg
(`DK5EN-98` watching what `DK5EN-14` transmits) runs the same way.

**One limit stands, and it is not this row's to fix:** the §3.8e correction still holds — both fork
workflows are `disabled_manually`, so nothing here runs automatically. Every green claim is a
manual local run. That is tracked as CI enablement, not as a missing harness.

Intake, kept for the record. Serial control was nearly complete (§3.8a); `--wifi on/off` (HL-01)
was the missing piece. Planned instruments: USB serial for provisioning and assertions, net console
2323 for non-perturbing observation, and a second node (`DK5EN-98`) to watch what `DK5EN-14`
actually transmits.
...
> **ID collision, corrected 2026-09-03.** This section's second item was filed as `MH-02` on
> 2026-08-28; §3.8p independently filed a _different_ `MH-02` (dead MHeard eviction logic) on
> 2026-08-31, and that one is the one that was fixed (`a1191eaa`) and that every other document —
> `CHANGELOG-stability.md`, `archive/pr-draft-20260831.md`, `code-quality-2.0.md` — refers to. The item
> below is renamed **`MH-03`** to end the ambiguity. §3.8p's `MH-02` keeps its ID.

| ID    | Sev.   | Location                          | Item                                                               | Status |
| ----- | ------ | --------------------------------- | ------------------------------------------------------------------ | ------ |
| MH-01 | MEDIUM | `lora_functions.cpp:705`          | `\| 0xF0` marker overwrites the country nibble; `0xF` == `PL` (15) | open   |
| MH-03 | LOW    | `aprs_functions.cpp:127, 458/464` | Absent optional trailing fields keep the **receiver's** hw/fw      | open   |

**MH-01 — not fixed (re-checked 2026-09-03).** The question was asked directly; the answer is no.
`src/lora_functions.cpp:705` still reads
`mheardLine.mh_mod = aprsmsg.msg_source_mod | 0xF0;  // set mod not from last`, unchanged. Neither
`MH-01` nor `MH-03` has ever been in a commit — what shipped in `a1191eaa` was §3.8p's `MH-02`, a
different defect in a different file (see the collision note above). The line numbers in the
original write-up have drifted (587 -> 705, 126 -> 127, 448/454 -> 458/464); the code at them is
identical. Both remain open, and the "take both in one wave, two commits" ordering below still
applies.

`msg_source_mod` packs two nibbles: low = `getMOD()` (range 3..8,
...
**MH-03** (filed as MH-02 on 2026-08-28). `decodeAPRS` opens with `initAPRS(aprsmsg, 0x00)` (`aprs_functions.cpp:127`), which fills
...
**Order matters:** MH-03 is what makes MH-01's `else` branch reachable at all for legacy frames.
Fixing MH-01 alone leaves the marker mostly dead; fixing MH-03 alone increases how often the country
...
| TM-09 | Heltec V3/V4, Stick V3, T-Beam V3 | PERF | High   | `loop_functions.cpp:364-374`                            | OLED on `_1_SW_I2C`: switch to `_F_HW_I2C` on a hardware `Wire` instance (V2/RAK already do), set `setBusClock(400000)`. Expected ~10x faster frame; measure with `[INSTR-FLUSH]` before/after.                                                                    | **FIXED** — Heltec V3/V4, Stick V3: `U8G2_*_F_2ND_HW_I2C` on `Wire1` (SDA 17/SCL 18, `Wire1.setPins()` before `begin()`, 400 kHz, `-D U8X8_HAVE_2ND_HW_I2C=1` in the variant because arduino-esp32 never defines `WIRE_INTERFACES_COUNT`). Measured DK5EN-93: frame push 579 ms -> 34.5 ms, loop max 645 -> 39 ms. T-Beam v1.2 was already on `Wire` HW I2C (37.8 ms). **T-Beam Supreme (`BOARD_TBEAM_V3`) closed 2026-09-06** (`0b0c0984`, changelog 203): it was still on U8g2 software I2C with a one-page buffer, measured in the field as `[INSTR-LOOP];gap;ms;577;in;display_tick` four times a minute at the 15 s clock refresh. Now `U8G2_SSD1306_128X64_NONAME_F_HW_I2C(U8G2_R0, U8X8_PIN_NONE, 18, 17)` on **`Wire`**, not `Wire1` as on Heltec V3/V4 -- the Supreme shares the OLED bus with PMU, RTC and sensors (`SDA_PIN 17` / `SCL_PIN 18`). The full buffer re-enables the TM-10 unchanged-frame skip. **Compile-verified only, no Supreme board on the bench.** **Field result 2026-09-12: 4.35t hangs in `u8g2->begin()` on two Supreme nodes** (`bug-tbeam-supreme-435t-display-hang.md`): the explicit constructor pins make u8g2 `pinMode(OUTPUT)` the pins the I2C controller already owns. **FIXED 2026-09-12** (changelog 220): constructors without pins, `setBusClock(100000)`; compile-only here, field confirmation open. In the DS-03/TD-16 upstream PR. |
...
| TM-49 | ESP32 safeboot | BUG | Medium | `src/safeboot/ElegantOTA.cpp` (upload completion handler), `main.cpp` `onOTAEnd()` | **A disconnect mid-upload can reach the completion handler with `hasError == false`** — the `final` frame was never seen, `Update.end()` never ran, so nothing set an error; the handler may then set `_reboot` and `onOTAEnd(success=true)` runs `setBootPartition_APP()` after a PARTIAL write. Observed indirectly on the bench (node back in the app seconds after a killed upload instead of via the 180-s fallback). On the 16-MB T-Deck the bootloader's slot validation saved it; on a 4-MB single-slot board (Heltec) this could switch to a half-written app image. Fix direction: the completion path must treat "final never received" as an error (track a `sawFinal` flag; guard `_reboot` and the partition switch on it), and `onOTAEnd(success)` must only switch partitions when `Update.isFinished()` confirms a complete image. Needs a bench proof on a 4-MB board before the next release. | **FIXED 2026-09-03 — bench proof still owed.** Operator decision: invert the gate instead of patching each escape. `ElegantOTAClass::_ota_image_valid` is **false from `/ota/start`** and is set true in exactly one place — after `Update.end(true)` has returned true (length + the client MD5 handed to `Update.setMD5()`) _and_ `Update.isFinished()` confirms it, logged as `[SAFEBOOT];ota;verify;result;ok`. Both completion handlers (async and sync) now gate `postUpdateCallback()`, `_reboot` and the HTTP 200 on that flag rather than on `!Update.hasError()`, and abort the session with reason `incomplete_upload` when it is false, so a client that vanishes before the `final` frame gets a 400 and the node falls back to safeboot's 180 s timer instead of switching partitions. `abortActiveUpdate()` clears the flag **only while `Update.isRunning()`** — a late AsyncTCP disconnect must not retract a verdict that was already earned. Builds: `esp32-safeboot`, `esp32-S3-safeboot` (Flash 18.9 %, RAM 19.2 % on S3); native gate 568/568. **Open:** the 4-MB bench arm (kill an upload mid-transfer on the Heltec, assert HTTP 400, no `setBootPartition_APP()`, recovery via the fallback) — procedure in `docs/bench-ota-regression.md`. |
...
| BP-11 | BUG  | High   | `src/backpressure.h`, `src/loop_functions.cpp`, `src/bp_notice_frame.h`, `test/test_bp_echo_guard/`                                                                                                                                                                                                | A client re-injects the node's own back-pressure wording back into `sendMessage()`: `mcmap messages_query` shows `IZ5CND-10` msg-ids `462F82A6..AE`, nine frames in 3 s with the `QRT NOT SENT - ` prefix stacking 2->5. Left unblocked, the accumulated echo goes on the air once the ring drains.                                | **DONE 2026-09-04** -- new `bpIsOwnWording()` in `backpressure.h` (exact, case-sensitive match against `bpNackPrefix()`/`bpNoticeText()`, no allocation), wired into `sendMessage()` right after the `{ZIEL}` parse and before `bp_state.refusing()`/any msg-id mint; on a hit: `[BP];echo;ms;T` marker (no text), `BP_SEND_INVALID`, no nack, no notice, no `bp_state` call. Option A (strict block on prefix alone), not Option B (strip-and-resend): field timing shows a machine client answering every nack within milliseconds, and the firmware's own `sendExternNotice()`/`getExtern()` EXTUDP pair can loop a nack node-to-node without any app -- stripping would still radiate every retry once, block ends the loop at the first echo. See `docs/archive/node-msg.md` §2 for the full reasoning. `bp_notice_frame.h`'s `msg_app_offline` comment corrected (catch-up marker, not a send inhibit). New native suite `test_bp_echo_guard`: bare notices, drift guard over all literals, 1-5 stacked prefixes, mixed QRT/QTA, prefix+content, the field case, ordinary/empty/nullptr text, mid-sentence wording, off-by-one variants, leading spaces, lower-case and tab-prefixed all pass. Gate 578 native / 12 envs, 6 board builds SUCCESS. Bench §5.2 cases 1-4 PASS on DK5EN-98 2026-09-04 (serial-over-2323 and web path, one `[BP];echo;` each, zero nack/notice, regression message accepted); case 5 (ring flood) still open, to run on DK5EN-14. Installed 2026-09-04 on DK5EN-98 (OTA, build Sep 4 08:14) and DK5EN-14 (USB, build Sep 4 08:19). **Correction 2026-09-11:** the "not published" hold of 2026-09-04 was lifted; BP-11 went upstream inside the collective PR **#1135** (section 3, `bpIsOwnWording()`), merged 2026-09-10. No separate upstream PR is owed, and the earlier "stays fork-only" note is wrong -- it is in `upstream/dev`. Reasoning and options: `docs/archive/node-msg.md` §2. Still open: bench case 5 (ring flood) on DK5EN-14.                                                                                                                                                                                                                |
...
| MEM-04 | classic ESP32   | RISK | High   | `variants/ttgo_tbeam*`, `variants/E22_XML*`                                                                                | **Four envs are within 4 kB of a link failure on a clean tree**: `ttgo_tbeam`, `ttgo_tbeam_SX1262`, `ttgo_tbeam_SX1268` at **20 bytes** of `iram0_0_seg`, and `E22_XML-DevKitC` at **1 160 bytes** of `dram0_0_seg` (plus 3 456 B IRAM; the row said 920 B from the 2026-09-05 measurement, re-measured against `dry-base-20260911`). Any feature that touches IRAM breaks the T-Beam family; this is what broke upstream CI on `9d885b1a`. | **IRAM half CLOSED 2026-09-12 (`R3-02`).** The three `ttgo_tbeam*` envs go from **20 B to 4 972 B** of `iram0_0_seg` by unflagging `BOARD_HAS_PSRAM` + `-mfix-esp32-psram-cache-issue`, which drops `spiram_psram.c` (4 427 B, IRAM-resident because it runs with the cache disabled). Verified on T-Beam-92 after flashing: `[PSRM]` 4 191 863 -> 0, init free internal heap **146 488 -> 151 348 (+4 860 B)**, SX1276 / AXP2101 / GPS-fix / WiFi all up, no backtraces. **The reading that decided it contradicted the prediction:** PSRAM _works_ on that board -- the flash-clock mismatch theory (40 MHz header vs SDK-prebuilt 80 MHz SPIRAM) was wrong. It was traded anyway because the firmware used **596 B** of the 4 MB and has no `ps_malloc` call site in any classic-T-Beam file (all of them are `src/t-deck/` or `src/t5-epaper/`, S3 boards), while IRAM is what actually binds -- and PSRAM is data memory behind the same cache, so it cannot relieve IRAM at all. Moving the static rings there is closed off regardless: `CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY` is not set in the prebuilt SDK. **DRAM half still open:** `E22_XML-DevKitC` is `az-delivery-devkit-v4`, has no PSRAM at all, and still needs the tinyxml2 lever (~5.8 kB): `mem-headroom-classic-esp32-20260905.md`. |
...
| `E22_XML-DevKitC`   |        **1 160 B** |            3 456 B |

The `E22_XML-DevKitC` DRAM figure was **920 B** when this table was taken on
2026-09-05 and is **1 160 B** re-measured against `dry-base-20260911` (the C1
carve-out gate, 2026-09-11) -- the tree gained 240 B of DRAM headroom in between,
not through any headroom work. Every other figure in the table still matches to
the byte, the T-Beam family's 20 B included. Re-measure before acting on any of
these numbers:
`python3 tools/resource_watch.py regions --env <env> --map .pio/build/<env>/firmware.map`.
...
| CHR-03  | —     | all                      | ENH  | Medium | Latin-1 and CP1252 as a second accepted charset next to UTF-8, without transcoding | **DONE 2026-09-09**: Bytes 0x80-0xFF, die keine gueltige UTF-8-Sequenz bilden, passieren unveraendert (kein Transkodieren, In-Place-Kontrakt bleibt). 0xA0-0xFF ist in Latin-1 und CP1252 gleich; 0x80-0x9F wird per Operator-Entscheid als CP1252 durchgelassen (Euro, typografische Anfuehrungszeichen). Ungueltige-aber-wohlgeformte Sequenzen fallen jetzt GANZ statt nur im Lead-Byte, sonst haette der Legacy-Pfad Fortsetzungsbytes als Fragment durchgelassen; 27 native Faelle plus ein Ende-zu-Ende-Fall in test_aprs_decode. Ausgabe ist bewusst nicht mehr garantiert valides UTF-8 -> Deutung liegt beim Empfaenger (mc-chat 993b512). Offen: EXTUDP-JSON (`extudp_functions.cpp:640`) traegt die Bytes in `msg`; die drei Empfaengerstellen liegen ausserhalb dieses Repos (MCProxy udp_handler.py:236 und ble_protocol.py:235, mc-chat interlink.py:363) |
...
| APRS-01 | 8     | all (paper)              | DOC  | Medium | Research paper: forwarding received APRS data to YAAC / aprs.fi app / APRSdroid    | **ANSWERED 2026-09-03 by PR #1114 — closed as a paper, one decision left.** The paper was to be written because nothing existed to measure; PR #1114 built it, we reviewed it, upstream merged it and reverted it the same day. Overlay below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| TD-10  | T-Deck / T-Deck Plus                                                                                                                                                                                              | ENH  | Low    | `src/t-deck/tdeck_main.cpp` (`keypad_get_key`/`keypad_read`, :749/:782)                                                                                                                                                                               | Auto-Repeat fuer Backspace (Taste halten = mehrfach loeschen). Analyse abgeschlossen, Konzept liegt: LVGL-Keypad-Repeat (`lv_indev.c:488-514`) existiert schon, es fehlt nur die Halte-Erkennung — die liefert der Raw-Mode der Stock-Keyboard-Firmware (I2C-Cmd `0x03`/`0x04` an `0x55`, 5-Byte-Matrixframe, Backspace = Byte 4 Bit 3); Degradation auf heutiges Verhalten, falls das Keyboard den Cmd nicht kennt. ~40-60 Zeilen, nur `tdeck_main.cpp`; Bit-Polaritaet am DK5EN-14 zu verifizieren. Volle Analyse: [`archive/tdeck-backspace-autorepeat-20260831.md`](archive/tdeck-backspace-autorepeat-20260831.md)                                                                                            | **IMPLEMENTIERT 2026-09-02, UPSTREAM in PR #1125 (Abschnitt 3), gemergt 2026-09-03** -- 39 native Testfaelle, Review-Verdikt `docs/archive/review-verdict-tdeck-keyrepeat-20260902.md`, Umsetzungsplan `docs/archive/tdeck-keyrepeat-impl-plan-20260902.md`. Worktree `mc-keyrepeat` und Branch `feat-tdeck-keyrepeat-20260902` existieren nicht mehr. **Offen bleibt nur der Bench-Nachweis, und der ist hier nicht erbringbar:** die Bench-T-Deck-Plus (`DK5EN-14`) hat alte Keyboard-Controller-Firmware, antwortet auf die Raw-Mode-Probe mit `00 00 00 00 00` und nimmt daher immer den Ein-Zeichen-pro-Druck-Pfad. Es braucht ein Geraet mit neuerer Tastatur-Firmware                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| TD-12  | T-Deck / T-Deck Plus                                                                                                                                                                                              | BUG  | Medium | `src/t-deck/event_functions.cpp:674`, `:734` (Save Setting / Send handler)                                                                                                                                                                            | Tab-Wechsel bleibt nach Send mitten in der Scroll-Animation stehen (linke Haelfte Nachrichtenliste, rechte Haelfte Eingabeseite, um etwa eine halbe Bildbreite verschoben). Kette: `lv_tabview_set_act(..., LV_ANIM_ON)` startet eine 200-400 ms Scroll-Animation; ein zweiter Druck (Doppeltipp, Touch-Bounce, Trackball) auf ein fokussierbares Widget loest ueber `SCROLL_ON_FOCUS` `lv_obj_scroll_to_view_recursive` aus, das die laufende Animation loescht; der Ersatz-Scroll wird durch `LV_DIR_NONE` auf dem Tabview-Content genullt, und die SCROLL_END-Rettung des Tabview bricht ab, weil der Indev noch gedrueckt ist. Plan: `archive/tdeck-tab-anim-stall-plan.md` (Desktop, now archived)            | **GEFIXT 2026-09-05** (`3f6a35d5`): beide Aufrufe auf `LV_ANIM_OFF`, wie alle anderen `lv_tabview_set_act` im Fork. Builds t_deck/t_deck_plus/t_deck_pro gruen. **CLOSED 2026-09-05 (Operator-Entscheidung):** Handtest nicht mehr gefordert, Fix bleibt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| TD-13  | T-Deck / T-Deck Plus                                                                                                                                                                                              | BUG  | Medium | `src/t-deck/tdeck_main.cpp` `mouse_read()` Pegelvergleich (`dir != last_dir[i]`, Index 4 = Trackball-Taste)                                                                                                                                           | Trackball-Taste liefert zwei LVGL-Clicks pro Druck: Press- und Release-Flanke erzeugen je einen Ein-Poll-Puls (`left_button_down` ist ein per-Poll-Local), LVGL macht aus jedem Puls ein `LV_EVENT_CLICKED`. Hamburger oeffnet den Drawer und schliesst ihn beim Loslassen wieder; Einweg-Buttons (send, sendpos, zoom, clear) laufen doppelt. Aus Upstream geerbt; der TM-18-Interrupt-Zaehler (`i < 4`) und AceButton sind nicht beteiligt. Plan: `archive/tdeck-trackball-double-click-plan.md` (Desktop, now archived)                                                                                                                                                                                         | **GEFIXT 2026-09-05** (`7368539e`): Puls nur auf der Press-Flanke (`steps = (i == 4 && dir) ? 0 : 1`), Bench-Inject `--ball click` landet immer als Press-Flanke. **Bench-Nachweis 2026-09-05 auf DK5EN-14:** alte Flankenlogik `--ball click 1` auf dem Hamburger -> Drawer wieder zu (0/5 offen, zwei Pulse 140 ms auseinander); Fix -> Drawer bleibt offen (5/5), `--ball click 2` -> auf und wieder zu. Voraussetzung war ein Umbau des Bench-Injects: der Plan-Hunk (immer Press-Flanke) lieferte zwei Press-Flanken ohne Release dazwischen, LVGL sah einen langen Druck und der Doppelclick war nicht reproduzierbar; der Inject modelliert jetzt Press, 3 Polls Halten, Release. Handtest (Halten, Loslassen, Reihe darunter) und ein `--sendpos` pro Druck bleiben Operator-Aufgabe                                                                                                                                                                                                                                                                                                                                                   |
| TD-14  | T-Deck / T-Deck Plus                                                                                                                                                                                              | PERF | Low    | `src/t-deck/lv_obj_functions.cpp` (`tabview_event_cb` case 3, `tdeck_hide_tab_menu`), `tdeck_sdmap.cpp`                                                                                                                                               | Picking the map tab from the tab bar recomposes the SD map twice: once for the view with the bar visible (294x140 px), once after the bar collapses (294x182 px), 616 + 635 ms back to back with the main loop blocked. Zoom keys/buttons and an own-position beacon cost one recomposition (470-750 ms) each. This is the TD-09 tile cost showing up through more triggers than the pan; RCA of the operator's "cursor dead time" report in `archive/tdeck-cursor-stall-rca-20260905.md` (now archived; no regression, message tab clean in three instrumented runs)                                                                                                                                              | **FIXED 2026-09-05.** Root cause: LVGL sets `LV_OBJ_FLAG_EVENT_BUBBLE` on the tab button matrix (`lv_tabview.c:233`), so a bar click reaches `tabview_event_cb` twice (SCROLL_END path with the bar visible, then the bubbled btnmatrix event with the bar hidden). Fix: drop the bubbled duplicate (`lv_event_get_target != lv_event_get_current_target`), hide the bar before `sdmap_refresh()` in case 3, dedupe guard in `sdmap_refresh()` (set/zoom/lat/lon/viewport key), caller tag `from=` on every rebuild line. Bench `tdeck_harness.py --scenario map_tab_pick`: baseline 2 rebuilds 294x140 + 294x182, lvgl gap 1337 ms; fixed 1 rebuild 294x182, gap 718 ms. Zoom/beacon single rebuilds remain TD-09                                                                                                                                                                                                                                                                                                                                                                                                                             |
| TD-15  | T-Deck / T-Deck Plus                                                                                                                                                                                              | BUG  | Medium | `src/lora_functions.cpp:1263-1283` (`tdeck_add_pos_point` only for POSITION frames), `src/t-deck/lv_obj_functions.cpp:3860-3960` (`savePosPersistence`/`loadPosPersistence` persist the POS table, not the marker arrays)                             | Map shows only stations whose POSITION beacon (30 min interval) arrived since the last boot: markers are RAM-only, `/pos.dat` restores the POS table but not `map_pos_*`, and HEY frames -- which carry a position and feed the mheard lat/lon -- never reach the map. Visible after a bench day with ten reboots. RCA `tdeck-map-empty-after-reboot-rca-20260905.md`                                                                                                                                                                                                                                                                                                                                              | **FILED 2026-09-05.** Not a regression. Fix candidates: feed the map from HEY frames (largest effect), rebuild markers from the persisted table at boot (store raw doubles)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| TD-16  | T-Deck / T-Deck Plus                                                                                                                                                                                              | BUG  | Medium | `src/t-deck/lv_obj_functions.cpp` (`tft_on()`), `src/t-deck/tdeck_helpers.cpp` (`setBrightness()` KBL sync)                                                                                                                                           | Field report 2026-09-11 (`bugreport-tdeck-keylock-kbl.md`): with the keylock engaged (SYM+K) and the keyboard light off, every incoming message lit the keyboard at 150 until the display timeout. `tft_on()` carried a forced sync that tested `node_keyboardlock` instead of the light setting (inverted since `583782a9b`); keyboard, trackball and touch never reach `tft_on()` while locked, `msg_focus_and_alert()` does.                                                                                                                                                                                                                                                                                    | **FIXED 2026-09-12** (`9c3bdf6b`): block removed, `setBrightness()` already syncs from `node_kbllightlock`. Proven by the new harness scenario `keylock_kbl` (`5d9877e2`, fork-only `[KBL];set` marker): `[KBL];set;150` on the old code, no keyboard write on the new one, DK5EN-14. PR to upstream `dev` with DS-03.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| CDC-01 | ESP32-S3 (every env with `ARDUINO_USB_CDC_ON_BOOT=1`: T-Deck/Plus/Pro, T-Beam Supreme, T3-S3, T-Connect-Pro, T-ETH-ELITE, T5-epaper, Vision Master, Wireless Tracker)                                             | BUG  | High   | `src/net_console.cpp` `MeshSerialClass::begin()`, `src/esp32/esp32_main.cpp` (Serial.begin)                                                                                                                                                           | Serial prints block the main loop for 100 ms each once the USB host is gone: arduino-esp32 2.0.14 HWCDC raises its TX timeout to 100 ms on the first host read and never lowers it; with the cable pulled the 256 B ring never drains. On the T-Deck the trackball cursor and the touch input freeze in the rhythm of the GPS log (every 3 s) and of every `[BALL]` line. RCA `archive/tdeck-cursor-stall-rca-20260905.md` (now archived)                                                                                                                                                                                                                                                                          | **GEFIXT 2026-09-05:** `setTxTimeoutMs(0)` + `setTxBufferSize(4096)` after `Serial.begin()`. Proof DK5EN-14 (`tdeck_cdc_unplug.py`, RTC-carried `[INSTR-PREV]`): 7 gaps / 1.8 s max in 44 s without the fix, 1 gap (586 ms, map) in 33 min with it. Upstream PR candidate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| WQ-01  | ESP32 / nRF52 (web UI)                                                                                                                                                                                            | ENH  | Medium | `src/web_functions/web_functions.cpp` (`sub_page_rxlog`, scaffold JS `mcRenderQueue`), `src/txring_functions.cpp` (`txRingPrioCounts`), `src/loop_functions.cpp` (`stat_last_window`, `bp*` getters), `src/setlog_lines.cpp` (`setlogDedupWindowMin`) | Visibility into the LoRa-related RAM rings on the rxlog page: TX ring occupancy per priority as a cell bar with the QRS/QRT thresholds, back-pressure state, dedup memory window computed from the newid rate of the last 5-min STAT window (corridor 40-48 min), rx/tx channel utilisation bars. Fragment carries data-* only, the scaffold JS renders (innerHTML never runs injected scripts).                                                                                                                                                                                                                                                                                                                   | **DONE 2026-09-05** (`88f7b181`..`9fb54ec9`, 7 commits): collapsible card, default open, fixed 600 px, three text sizes on the page; +6.8 kB flash Heltec V3, RAM unchanged; 9 new native tests. Verified in Chrome on dk5en-98                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| WQ-02  | ESP32 / nRF52 (web UI)                                                                                                                                                                                            | ENH  | Low    | `src/backpressure.h` (`qrsForecastDepth`, `userMsgsCounted`), `src/loop_functions.cpp` (`bpQrsForecast`), `src/web_functions/web_functions.cpp` (`data-qrsf`, faint fixed tick)                                                                       | The QRS tick sat at the fixed line (depth 5) although QRS fires only on the third own message at/above it and the depth counts relay/ACK/beacon frames too. Forecast `max(line + remaining - 1, depth + remaining)`, clamped at QRT; solid tick + label follow it, fixed line stays faint. Snapshot at render, said so in the caption.                                                                                                                                                                                                                                                                                                                                                                             | **DONE 2026-09-05** (`b1cc8cd5`, item 194): 5 native cases in `test_backpressure`; RAK4631 build green. Not hand-checked in the browser yet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| WM-01  | ESP32 / nRF52 (web UI)                                                                                                                                                                                            | BUG  | Medium | `src/web_functions/web_functions.cpp` (`sub_content_messages()`, `sub_page_rxlog` JS scaffold `updateMessages`/`loadPage`)                                                                                                                            | The messages page followed the phone's BLE read cursor (`toPhoneRead`) into the shared 20-slot ring; with a connected BLE client draining slots in ~100 ms, the web page mostly found the ring empty, and `updateMessages()` overwrote the panel from the ring window every 10 s so anything scrolled out of the ring vanished within minutes on a busy node. No fork regression — the web page never had its own read pointer (`87c6c200`, upstream 2025-05-17). Plan `archive/web-messages-plan-20260905.md` (now archived).                                                                                                                                                                                     | **DONE 2026-09-05** (`e655b75a` ring-scan, `3d6bdbeb` browser-side history merge keyed by `data-id` cap 200, `5564b0d3` group tabs from `node_gcb[]`): node RAM unchanged, browser-memory only, localStorage persistence deferred. `/fable-review` and upstream PR text still owed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ACK-01 | ESP32 / nRF52 gateways                                                                                                                                                                                            | BUG  | Medium | `src/lora_functions.cpp` (`handleACK()` gateway-ACK site, `OnRxDone()` heard site), `src/ack_attribution.h` (comment)                                                                                                                                 | Gateway sent one heard + one gateway-ACK status frame to the phone per foreign message it forwarded from the server: `own_msg_id[]` holds those msg_ids (`insertOwnTx` in `udp_functions.cpp:476` / `nrf_eth.cpp:655`, LoRa relay never inserts) and the legacy first-frame branch tested membership only. McApp booked ~150 false `send_success` rows/day.                                                                                                                                                                                                                                                                                                                                                        | **FIXED 2026-09-05** (`b2336e6f`, item 193): BLE emit wrapped in `ackMsgIdFromNode()`, outer condition and state writes unchanged. Flashed to 98 + 14. Manual McApp verification by the operator pending; doc `archive/ack-heard-foreign-msgids-fix.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| DS-01  | ESP32 (all non-e-ink boards), nRF52 (RAK4631, T114, T-Echo)                                                                                                                                                       | BUG  | Medium | `src/esp32/esp32_sleep.cpp`, `src/nrf52/nrf52_sleep.cpp` (both new), `src/command_functions.cpp` (`--deepsleep`), `src/batt_functions.cpp:428-463` (low-battery guard, still dead code)                                                               | Verdict for upstream issue #962: the automatic low-battery deepsleep is not misbehaving, it is switched off entirely — upstream commented the whole block out for issue #1053 in `e0043a56` (v4.35p.07.11), for every board. `--deepsleep` itself also armed no wake source, left the LoRa chip in RX, never powered down the OLED or (on T-Beam) the PMU rails outside the `WP_DISP` branch; e-ink boards already got the fix template via upstream PR 1050. RAK4631 `--deepsleep` was a no-op (`esp_deep_sleep_start()` excluded), T114 needed a second call to undo its soft-off, T-Echo's manual command did nothing. Full analysis and implementation plan: `archive/issue-962-deepsleep-verdict.md`.         | **`--deepsleep` FIXED 2026-09-05** (`e242a4cf` ESP32 Option A, `37537b11` nRF52 System OFF): shared `esp32EnterDeepSleep()`/`nrf52EnterDeepSleep()` helpers put the radio to sleep, power down the display, cut PMU/peripheral rails, and arm a wake source on every board except Vision Master E290 (e-ink not cleared, no bench hardware, tracked with TM-28). Both waves reviewed by an independent advisor pass before commit (caught a floating wake-pin, a compile-time/runtime button-pin mismatch, an unconditional `Serial1.end()` hang risk, and an unbounded `systemOff()` pin lookup — all fixed); all 32 build environments verified clean. T-Beam Supreme's AXP2101 rail cut is compile-verified only, no bench hardware for that variant. **Low-battery guard (Option B) not started** — still open, opt-in timer-wake loop with hysteresis per the doc's §3 Option B.                                                                                                                                                                                                                                                          |
| CDC-02 | ESP32-S3 (every env with `ARDUINO_USB_CDC_ON_BOOT=1` + `ARDUINO_USB_MODE=1`: T-Deck/Plus/Pro, T3-S3, T-Beam Supreme, T-ETH-ELITE, T5-epaper, Vision Master E213/E290, Wireless Tracker, Heltec V4, T-Connect-Pro) | BUG  | High   | `src/net_console.cpp` `MeshSerialClass::begin()`, `src/esp32/esp32_main.cpp` `esp32setup()`                                                                                                                                                           | CDC-01 called `setTxBufferSize(4096)` after `Serial.begin()`. `HWCDC::begin()` (arduino-esp32 2.0.14) already creates the 256 B ring and enables the SERIAL_IN_EMPTY ISR; the resize deletes the ring and leaves `tx_ring_buf` NULL for a few lines while the ISR (no NULL check) can fire. A host port-open during boot -- which is also what resets these boards -- hit `assert failed: xRingbufferReceiveUpToFromISR ringbuf.c:1269`, then a 2-3 reboot crash loop ending in `RESET_REASON=4 PANIC` (DK5EN-14, 2 of a handful of opens). SD card and keyboard I2C stayed dead across the burst until a USB power cycle (secondary, tracked by the bench). Analysis: `gpio-hold-and-hwcdc-followups.md` Issue 1. | **FIXED 2026-09-06** (`b57daf44`): ring sized before the first `begin()` (no ISR exists yet; `begin()` keeps a preset ring), once-flag for the double `begin()` on T5-ePaper/T-Deck Pro. Bench regression `tools/bench/tdeck_cdc_portopen.py` (50x open/close loop, greps each boot for the assert/crash markers) -- **bench-proven 2026-09-06 on DK5EN-14**: pre-fix firmware 9 of 30 port-opens crash with the ringbuf.c:1269 assert (first at iteration 5), fixed firmware 0 of 80 (50 + 30), SD card and keyboard OK on every boot (the secondary wedge did not reappear). Logs `tools/bench/runs/cdc_portopen_*20260906*.log`. Upstream PR candidate together with CDC-01.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| DS-02  | Wireless Paper, Vision Master E213                                                                                                                                                                                | BUG  | High   | `src/esp32/esp32_main.cpp` `esp32setup()` radio init; sleep side `src/Platforms/{WirelessPaper,VisionMasterE213}/power_controls.cpp` `prepareToSleep()`                                                                                               | `prepareToSleep()` holds `PIN_LORA_NSS` (GPIO8) HIGH with `gpio_hold_en()` before deep sleep; nothing on the boot path called `gpio_hold_dis()`. Per ESP-IDF gpio.h the hold survives the wake reset, so after the first `--deepsleep`/wake the SX1262 chip-select could never go LOW again: radio dead until a power cycle -- same mechanism as the T-Deck/T-Beam-1W fix `828a2567`. Found by code inspection, `gpio-hold-and-hwcdc-followups.md` Issue 2.                                                                                                                                                                                                                                                        | **FIXED BLIND 2026-09-06** (`f7801a1c`): `gpio_hold_dis(PIN_LORA_NSS)` before the radio init, no-op on a cold boot; compile-verified on both envs. **FIELD-VERIFIED 2026-09-11** by OE3LCR (PR #1135 comment) on both boards, v4.35t after #1137: `--deepsleep`, EXT1 wake via GPIO0 (E213 OE3LCR-11 PRG key; WP V1.2 OE3LCR-10 GPIO0 pulse via DTR), boot log `RESET_REASON=8 DEEPSLEEP`, `wake: 3` (EXT1), `SX1262 E290 chip Initializing ... success`, `Starting to listen ... success`; SPI live afterwards (E213 `--posshot` sent, WP relayed a foreign frame RX+TX). OE3LCR's caveat: pre-fix 4.35p already survived timer wakes with working LoRa RX in July, consistent with the sleep side never arming `gpio_deep_sleep_hold_en()` -- the hold likely does not survive the wake and the call is a no-op in practice; harmless, kept as a guard against a later sleep-side change. Test hints: RST gives POWERON (`wake: 0`) and clears holds anyway, only PRG/button wakes are meaningful; on CP2102 boards (WP) pulse DTR (IO0 LOW, RTS untouched) after `--deepsleep` to automate the wake, not possible on the E213 (native USB). |
| DS-03  | Heltec V2/V3/V4, Stick V3, Tracker, TLORA V2.1.6 (ESP32); Heltec T114, T-Echo (nRF52); Wireless Paper, E213                                                                                                       | BUG  | High   | `src/esp32/esp32_sleep.{h,cpp}` (`esp32WaitButtonRelease()`), `src/command_functions.cpp` (WP_DISP `--deepsleep` branch), `src/nrf52/nrf52_sleep.cpp`, `src/onebutton_functions.cpp:316` (`attachLongPressStart`)                                     | Field report 2026-09-12 (`bugreport-heltec-v3-longpress-deepsleep.md`): since DS-01 the shared helpers arm the user button as wake source, but `PressLong()` fires 800 ms into the press with the button still held, so the ext1 `ANY_LOW` / `systemOff(pin, LOW)` condition is already true at sleep entry and the node reboots at once (`RESET_REASON=5`). On 4.35s the same press left the node dark only because no wake source was armed at all.                                                                                                                                                                                                                                                              | **FIXED 2026-09-12** (`40c29e7f`): wait for the release (bounded 10 s + 100 ms debounce) before arming, in both helpers and the WP_DISP branch; gated on `bButtonCheck` because with `--button off` the pin is never configured and floats (RAK4631 WB_IO6 reads LOW, burned the full 10 s on the bench). Bench: Heltec V3 `--deepsleep` over serial sleeps at once, no reboot in 15 s; long-press-and-hold with `--button on` passed two cycles (dark, stays dark after release, wakes on the next press). T114/T-Echo compile-only. PR to upstream `dev` with TD-16. **Automated 2026-09-13**: `tools/bench/deepsleep_button.py` (CP2102 DTR = GPIO0 press; `866e328f`, design `deepsleep-button-autotest.md`) -- PASS on the fork image, REGRESSION (reboot 815 ms after GO, still held) on official v4.35t, PASS again after reflash; `a3bd7f19` adds the `[BOOT] WAKE_CAUSE` line it reads (EXT1).                                                                                                                                                                                                                                        |
| WEB-05 | ESP32 / nRF52 (web UI)                                                                                                                                                                                            | ENH  | Low    | `src/web_functions/web_functions.cpp` (scaffold JS `mcUnreadCount`/`mcSeedWm`/`mcMarkRead`/`mcApplyTab`, `data-ts` on both message divs, `.mctab-new`/`.mcbadge` CSS)                                                                                 | Operator request after the group tabs shipped: a tab should turn green and show a count when unread messages arrived on it. Node side is one attribute (`data-ts`, the ring insert time); browser side a session id set plus a persisted per-tab watermark (localStorage `mcWm`), seeded on first sight, All clears everything, hidden window keeps messages unread, own messages and ack updates never count. Poll cadence unchanged, Messages page only.                                                                                                                                                                                                                                                         | **DONE 2026-09-06** (`714fce85`, item 195): `tools/webgui_badge_test.js` (jsdom against the live node, 30 checks) green on DK5EN-98; operator eyeballed it in the browser. Open by design: nav-button badge from other pages (needs the poll on every page), count is a lower bound after a long absence (20-slot ring), unsynced-clock messages count only within the session.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| WEB-06 | ESP32 / nRF52 gateways (web UI)                                                                                                                                                                                   | BUG  | Low    | `src/web_functions/web_functions.cpp` `sub_content_messages()`                                                                                                                                                                                        | A gateway that pulls the `{CET}` time beacon from the server puts it into the phone ring for the app's clock sync; the web list rendered it as a broadcast, and with WEB-05 every beacon would have lit the `*` and All tabs. Screenshot from the operator 2026-09-06 09:31.                                                                                                                                                                                                                                                                                                                                                                                                                                       | **DONE 2026-09-06** (`9076824d`, item 196): `{CET}` payloads skipped next to the `:ack` skip; ring and app untouched. Operator-checked in the browser.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
#### CHR-03 — Latin-1 and CP1252 as a second accepted charset (2026-09-09)

`CHR-01`/`CHR-02` shipped a pure UTF-8 allowlist, which by construction deletes a Latin-1 umlaut:
Latin-1 writes `ü` as the single byte `0xFC`, UTF-8 as `C3 BC`, and a lone `0xFC` is not valid
UTF-8. Senders that do this exist in the live network — PinPoint among them — so `Grüße` arrived as
`Gre`, and `äöü` in a row arrived as an empty message. mc-chat hit the same wall from the other side
and fixed it in its decoder on 2026-09-08 (`993b512`).

The rule now: a byte that is **not part of a valid UTF-8 sequence** is a legacy single-byte
character and passes through unchanged. That is the whole range `0x80`-`0xFF`. Nothing is
transcoded — the filter still only ever removes bytes, never rewrites or widens one, so every
caller's in-place buffer contract and every wire-length budget is untouched.

Four decisions worth keeping visible:

- **`0x80`-`0x9F` passes as CP1252** (operator decision 2026-09-09, widened from the Latin-1-only
  first cut the same day). This is the one block where the two encodings disagree: CP1252 puts the
  Euro sign, the typographic quotes and the dashes there, ISO-8859-1 leaves it as C1 controls. The
  senders here that use single-byte umlauts use CP1252, so the block passes. **Consequence, accepted
  knowingly:** a receiver that reads the stream as ISO-8859-1 rather than CP1252 sees C1 control
  codes in message text, and the five bytes undefined even in CP1252 (`0x81`, `0x8D`, `0x8F`, `0x90`,
  `0x9D`) are relayed too and render as U+FFFD at the far end.
- **That does not contradict the C1 rule on the UTF-8 path**, which still strips U+0080-U+009F
  arriving as a proper `C2 80`..`C2 9F` sequence. The two cases mean different things: a raw `0x80`
  in a legacy stream is a Euro sign, a deliberately UTF-8-encoded U+0080 is the control character
  PAD. Pinned by `test_c1_as_utf8_sequence_still_stripped`.
- **Illegal-but-well-formed sequences now drop whole**, continuation bytes included, where they
  previously dropped only the lead byte and resynced. This is load-bearing, not cosmetic: the
  continuation bytes of an overlong or surrogate sequence live in `0x80`-`0xBF`, which the new
  legacy rule passes. Resyncing into the middle of such a sequence would hand a fragment of exactly
  the payload the check rejects back to the caller — e.g. the `A0` of the surrogate `ED A0 80` as
  NBSP. The pinned CHR-01 case
  `test_invalid_and_overlong_dropped_without_corrupting_neighbors` was updated **deliberately** for
  this: its lone `0x80` between letters is not a sequence and now passes, while all three illegal
  sequences in the same vector still vanish whole.
- **Two adjacent legacy bytes that form valid UTF-8 are read as UTF-8** (`DF BC`, Latin-1 `ß¼`, is
  well-formed UTF-8 for U+07FC). Unchanged from before and not fixable byte-locally — telling the
  two readings apart needs statistics over the whole text.

Follow-up, filed not fixed: the output is deliberately **no longer guaranteed valid UTF-8**, and
`extudp_functions.cpp:640` puts `msg_payload` into the JSON field `msg` (ArduinoJson writes the
bytes through; it escapes `"` and `\` but does not validate UTF-8). The receivers have to be able
to read that. Traced 2026-09-09, three sites, none of them in this repo:

- **MCProxy `src/mcapp/udp_handler.py:236`** (`strip_invalid_utf8`, reached from
  `_process_received_message` at `:543`) — this is the real consumer of Extern-UDP :1799.
  `data.decode("utf-8", errors="ignore")` **silently deletes** every legacy byte, so the umlaut is
  lost here exactly as it used to be lost in the firmware. Primary site.
- **MCProxy `src/mcapp/ble_protocol.py:235`** — the BLE message body, same `errors="ignore"`, a
  second independent ingress (`udp_handler.py:552` calls it "this filter's only bypass").
- **mc-chat `meshcom_mock/interlink.py:363`** — strict `.decode("utf-8")`, drops the **whole**
  datagram on one bad byte. Pre-existing and not caused by this change: the interlink feed is
  server-to-server and has carried other senders' CP1252 bytes all along.

A correction to the first version of this entry, which named mc-chat's interlink as the Extern-UDP
consumer: it is not. mc-chat has no :1799 listener at all — interlink is a separate server feed.

Second-order, once the decode is fixed: MCProxy's `is_allowed_char()` whitelist
(`udp_handler.py:195`) lists `äöüÄÖÜß` and friends explicitly and lets CP1252 punctuation through
via its `S`/`P` category test, but other Latin-1 letters (`ø æ þ ð ý`) are still dropped one
character at a time. Fixing the decode without widening that list gets the German umlauts back and
nothing else.
...
**Closed as a paper on 2026-09-03: PR #1114 answered it with running code, and upstream then
...
firmware-side option is closed for us as long as the feature stands reverted upstream.

---

##### Original intake (kept for the record)

Scope set by the operator: all three clients. Ground truth the paper started from, all verified at
the time — note that the first sentence below has since been overtaken by PR #1114:
...
`docs/archive/ram-comparison-20260517.md`, predates `MEM-01` by 3.5 months — **a fresh snapshot is a
...
not a substitute: `docs/archive/command-changes-pr1102-1103.md` covers two merged upstream PRs only and its
...
4. Separately, the questions upstream leaves open: whether to react to the brown-out detector at all,
   ...
   #962 verdict and plan now filed as `DS-01`).
...
| ID      | Where                                                                        | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Sev.        | Action                                                                                                                                                                  |
| ------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GPS-01  | `esp32_main.cpp:3079`, `configuration_global.h:178`, `gps_functions.cpp:889` | **IMPLEMENTED 2026-09-02 on `feat-gps-nmea-20260902` (worktree `mc-gps`):** `WZ_GPS_Feed()` drains the UART every loop pass, `WZ_GPS_Loop()` evaluates on its own timer (§3.1). **CLOSED 2026-09-03** — bench proof delivered by the overnight `DK5EN-14` capture (`~/Downloads/meshlog-20260903/14.log`, 8 h, **9791 GPS evaluations, 0 rejects, 0 corrupt samples**) on the fixed firmware at the ordinary 3 s ESP32 cadence; the pre-fix model predicted ~5 corrupt commits per 2 h at that cadence. The planned A/B/C TRACK-mode arms are superseded: they were only ever the cheap falsification of the same claim. Review verdict `docs/archive/review-verdict-gps-20260902.md`. Was: GPS UART drained only every 3 s; 420 B of NMEA into a 256 B ring → ~165 B lost per cycle, always. nRF52 already polls at 1 s.                                                                                                                                                                                                                                               | High        | Split `WZ_GPS_Feed()` (drain, every loop) from the evaluation (every 3 s). **Not** `setRxBufferSize` — see below.                                                       |
| GPS-02  | `gps_functions.cpp:919`, `:959`, `:1009-1024`                                | **IMPLEMENTED 2026-09-02 on `feat-gps-nmea-20260902` (worktree `mc-gps`):** `gpsSamplePlausible()` gates null island, angle, altitude range and impossible calendar dates; `gpsTimePlausible()`/`gpsDatePlausible()` (review fix F1) also guard both clock-write call sites; rejects are counted and logged (`reject:` line). **CLOSED 2026-09-03** — bench proof delivered by the overnight `DK5EN-14` capture (`~/Downloads/meshlog-20260903/14.log`, 8 h, **9791 GPS evaluations, 0 rejects, 0 corrupt samples**) on the fixed firmware at the ordinary 3 s ESP32 cadence; the pre-fix model predicted ~5 corrupt commits per 2 h at that cadence. The planned A/B/C TRACK-mode arms are superseded: they were only ever the cheap falsification of the same claim. Review verdict `docs/archive/review-verdict-gps-20260902.md`. Was: No plausibility gate. `gpsData.valid` written, never read; the fix gate checks only sat/HDOP, which a damaged RMC leaves intact. Corrupt sample lands in persistent settings verbatim.                                        | High        | Gate on `isValid()` + `age()`; reject lat/lon exactly 0.0 and impossible dates.                                                                                         |
| GPS-03  | `gps_functions.cpp:1022`, `command_functions.cpp:4131`                       | **IMPLEMENTED 2026-09-02 on `feat-gps-nmea-20260902` (worktree `mc-gps`):** scalar Kalman filter (`AltFilter`, dt-scaled process noise via `ALT_KF_DT_REF_MS`, review fix F2) replaces the raw sample; `--setalt` seeds it and now rejects out-of-range input instead of clamping to 0 (F7). **CLOSED 2026-09-03** — bench proof delivered by the overnight `DK5EN-14` capture (`~/Downloads/meshlog-20260903/14.log`, 8 h, **9791 GPS evaluations, 0 rejects, 0 corrupt samples**) on the fixed firmware at the ordinary 3 s ESP32 cadence; the pre-fix model predicted ~5 corrupt commits per 2 h at that cadence. The planned A/B/C TRACK-mode arms are superseded: they were only ever the cheap falsification of the same claim. Review verdict `docs/archive/review-verdict-gps-20260902.md`. Was: `node_alt` is one raw unfiltered sample, beaconed every 30 min. `--setalt` is silently overwritten by the next fix — the command is a no-op on any node with a GPS.                                                                                            | Medium      | Scalar Kalman, constant `R`, `q`≈0.01 (τ≈400 s) — 8 B, measured 4.36→1.52 m RMS. **Not** a 5-sample median: that measures 2–5 %. Make `--setalt` sticky, then `--help`. |
| GPS-04  | `bmx280.cpp:326` (+ `bme680.cpp`, `bmp390.cpp`)                              | **IMPLEMENTED 2026-09-02 on `feat-gps-nmea-20260902` (worktree `mc-gps`):** `baroBaseRelatch()`/`baroBaseLatchAllowed()` re-latch QNH on filter convergence and on `--setalt`/`--setpress` (F9); the latch is withheld only while a fix currently exists and the filter has not converged (F4), so a node that never fixes still latches its persisted altitude as before. **CLOSED 2026-09-03** — bench proof delivered by the overnight `DK5EN-14` capture (`~/Downloads/meshlog-20260903/14.log`, 8 h, **9791 GPS evaluations, 0 rejects, 0 corrupt samples**) on the fixed firmware at the ordinary 3 s ESP32 cadence; the pre-fix model predicted ~5 corrupt commits per 2 h at that cadence. The planned A/B/C TRACK-mode arms are superseded: they were only ever the cheap falsification of the same claim. Review verdict `docs/archive/review-verdict-gps-20260902.md`. Was: QNH reference altitude latched to the first fix after boot, never corrected. An outlier at boot costs ~±7 hPa for the whole session — and this node rebooted twice in 7 minutes. | Medium      | Re-latch on `--setalt` and on the GPS-03 filter's convergence signal (`P` below threshold) — not on the first fix.                                                      |
| GPS-06  | `gps_functions.cpp` (`detectBaudrate()`), `variants/t_deck/configuration.h`  | **IMPLEMENTED 2026-09-02 on `feat-gps-nmea-20260902` (`79037fd5`), field finding OE5HWN:** T-Deck without Plus, self-wired module: fix on 4.35d, silent on 4.35p. Upstream `a672d18b` swapped the pin defines to the LilyGo assignment (`RX 44 / TX 43`), the reverse of 4.35d's `SoftwareSerial(43, 44)` on this board. `detectBaudrate()` re-scans once on the pre-4.35p pins when the variant defines `GPS_FALLBACK_RX_PIN`/`GPS_FALLBACK_TX_PIN` (only `variants/t_deck`), keeps the effective pins, logs a request to swap the wires. **CLOSED 2026-09-03** — the field finding is fixed in the tree and the release note below is the whole remaining deliverable. Plan §3.7.                                                                                                                                                                                                                                                                                                                                                                                     | Medium      | Release note for upstream: "T-Deck ohne Plus: Modul-TX an GPIO44, Modul-RX an GPIO43, wie beim Plus".                                                                   |
| GPS-07  | `gps_filter.h` (`ALT_KF_RESEED_N`, `ALT_KF_GATE_M`)                          | **FIXED 2026-09-06 (`531d66b4`):** gate 15 m / 10 rejects re-seeded 20x in 1.9 h on `DK5EN-93` (and 10x in 8 h on `DK5EN-14`), each time onto the outlier itself, so the filter matched the raw sd. Now 30 m / 60 rejects: offline either change alone still re-seeds 5x on the 93 corpus, the pair 0x on both; bench 2 h after the fix 0 re-seeds, Kalman sd 9.1 m vs raw 12.2 m. Regression test `test_feldserie93_keine_reseeds` (fixture `ALTB93_ALT`). [`bug-baro-altitude-20260906.md`](bug-baro-altitude-20260906.md) §4, §11.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Medium      | Cost: a node moved >30 m vertically outside TRACK follows after 3 min (ESP32) / 1 min (nRF52) instead of 30 s / 10 s.                                                   |
| GPS-08  | `bmx280.cpp` (`getPressALTf()`), `command_functions.cpp` (`--setpress` help) | **FIXED 2026-09-06 (`49769eda`):** `fBasePress` had exactly one writer, the `--setpress` handler, and is not persisted, so the barometric altitude was 0 after every boot on every BMx280 node. `getPressALTf()` now self-latches it from the current QFE once `fBaseAltidude` exists (set by `baroBaseRelatch()` at filter convergence, or by `getPressASL()` on no-GPS nodes) -- the same pattern QNH already used. Not persisted on purpose (struct bump wipes the fleet; stale pressure after a long power-off is worse than none). Bench: cold boot, no command, armed at t=312 s, ALT 474 m. Help text fixed, no argument added. nRF52 half **untested** (`DK5EN-90` not on the bench; on RAK4631 the GPS path is not compiled, the no-GPS latch branch applies). Doc §5, §11.                                                                                                                                                                                                                                                                                    | Medium-high | Verify on a RAK with `--setalt` set: `--tel` must show a non-zero `ALT asl` one WX tick after boot.                                                                     |
| GPS-09  | `bmx280.cpp` (`getPressALTf()`)                                              | **FIXED 2026-09-06 (`49769eda`, consumed by `bc3ee68b`):** struct field left `int` on purpose (a `float` reinterprets the persisted value once, doc §9); the float value is carried by the new accessor and consumed only by the GPS-05b fusion. Bench: barometric ADEV 30 s 0.10 m float vs 0.17 m whole-metre; host test `test_float_baro_schlaegt_int_baro` fails on the rounded input. Doc §6, §11.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Low         | Display/beacon fields unchanged.                                                                                                                                        |
...
| GPS-05b | `alt_fusion.{h,cpp}`, `gps_functions.cpp` (after the Kalman update)          | **IMPLEMENTED 2026-09-06 (`bc3ee68b`), gate partly missed:** GPS low-pass + barometer high-pass, tau 30 min, float barometer, active only while the pressure reference is armed on a BMx280 board; everything else keeps the Kalman value bit-for-bit; reset with the Kalman filter on TRACK entry. Host test `test_alt_fusion` replays the 93 corpus (sd 2.76 m, 5-min ADEV 0.97 m). **Bench 2 h stationary:** reported sd **6.64 m** (raw 12.2, Kalman alone 9.1, before the fix 17.2), p2p 18 m (was 72), ADEV 30 s 0.22 m (was 4.87). The sd <= 4 m gate was not met: the GPS mean walked 30 m over an hour and a 30-min low-pass follows it. Offline on the same run: tau 2 h -> sd 3.55 m, 4 h -> 2.44 m, at ~5 m of weather leakage per 2 h. **Open: tau 30 min (shipped) or 2 h + re-run.** TRACK-mode capture with pressure still owed. Doc §7, §11.                                                                                                                                                                                                           | Low         | Decide tau, then a second 2 h run. TRACK capture before any release claims moving-node altitude.                                                                        |
| GPS-10  | `nrf52_main.cpp:2871` (`getGPS()`, `ENABLE_RAK_GPS` build path only)         | **OPEN, filed 2026-09-06 (operator, deferred until the GPS-05b run is closed):** the legacy nRF52 GPS path `getGPS()` smooths the altitude with its own integer EMA, `node_alt = (node_alt*10 + alt)/11`, and never sees the GPS-03 Kalman filter, the GPS-07 constants or the GPS-05b fusion -- all of those live in `WZ_GPS_Loop()` under `ENABLE_GPS`, which the RAK4631 variant does not define. Integer arithmetic truncates toward zero on every sample, so the EMA has a permanent downward bias of up to 1 m and a 10-sample (100 s at the 10 s refresh) time constant with no gate: one corrupt altitude moves it by a tenth of the excursion. The bench RAK (`DK5EN-90`) has no GPS module, so this path is untested here. Decide whether to route `ENABLE_RAK_GPS` through `WZ_GPS_Loop()` or to port gate + filter; do not touch before the GPS-05b bench closes.                                                                                                                                                                                           | Low         | Next GPS item after GPS-05b. Needs a RAK with a GPS module on the bench.                                                                                                |
...
**Cheapest falsification — run, and the model held (2026-09-03).** The plan was a two-hour bench
node in TRACK mode (`gps_refresh_intervall` = 1.0 s, `esp32_main.cpp:3076`) with `--gpsdebug 1`,
predicting near-zero corrupt samples against ~5 at the 3 s cadence. What was actually run is
stronger: an **8-hour overnight capture of `DK5EN-14` on the fixed firmware at the ordinary 3 s
cadence** — `9791` evaluations, `0` rejects, `0` corrupt samples
(`~/Downloads/meshlog-20260903/14.log`, not in the repo). The GNSS module is exonerated and §4 of
the doc stands. GPS-01..GPS-04 and GPS-06 are closed on this evidence; the same capture is what
found **GPS-07**, **fixed 2026-09-06** in `531d66b4` (gate 30 m / 60 rejects, changelog 205).
...
| ID    | Where                                                                                                                                                  | Finding                                                                                                                                                                                                                                                                                                                                                                                                                         | Sev.   | Action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CQ-01 | `command_functions.cpp` (`--info`, `...BTCODE %06i`), `esp32/esp32_main.cpp` (`Server PassKeyRequest <%06i>`)                                          | The BLE pairing PIN is printed in clear on serial and on the unauthenticated 2323 console. N-31 masked `node_pwd`/`node_webpwd`/`node_passwd` and missed `bt_code`. Found by `CQ2-C14` (pattern C14). The PIN is the only thing between a phone in radio range and the command channel (N-07).                                                                                                                                  | High   | **CLOSED 2026-09-03 (operator): intended behaviour, risk accepted, no fix.** The PIN is printed on purpose — the operator needs to read it off the console to pair. The exposure is real and is accepted rather than unknown: anyone who can already reach the serial line or the 2323 console is inside the trust boundary that the PIN protects against. Was: `maskSecret()` on both sites (or drop the PassKeyRequest print); bench `--info` over 2323 must not contain the PIN; grep `CQ2-C14` must return 0. |
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| SL-01 | `src/lora_functions.cpp` (`OnRxDone`, `handleACK`), `src/loop_functions.cpp` (`printBuffer_aprs()`/`printBuffer_ack()` tail parameter) | RX/ACK line carried no signal level and no dedup verdict; copy rate had to be estimated from raw counts.                              | Medium | Append `RSSI:`/`SNR:`/`DUP:`/`OWN:`/`t=` to the existing, byte-identical line.                                               | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
| SL-02 | `src/lora_functions.cpp` relay block (`skip_relay:`)                                                                                   | Why a frame was not relayed was invisible in a capture.                                                                               | Medium | New `RLY` line with `q=<code>`, one of ten reason codes in checking order.                                                   | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
| SL-03 | `src/lora_functions.cpp` (`doTX`/`setlogPrintTx`)                                                                                      | Queue latency, starvation and CAD cost were only visible via straggler timing.                                                        | Medium | New `TX` line with `wait=`/`q=`/`cad=`, printed on successful send start (not on `TX_DONE`).                                 | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
| SL-04 | `src/lora_functions.cpp` (`OnRxError`), `src/esp32/esp32_main.cpp` (CRC path)                                                          | Collision/error rate was ESP32-only and gated behind `--loradebug`.                                                                   | Medium | New `ERR` line, both platforms, plus an unconditional `stat_rx_err` counter.                                                 | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
| SL-05 | `src/esp32/esp32_main.cpp`, `src/nrf52/nrf52_main.cpp` (5-minute `PRIO_STAT_INTERVAL_S` tick)                                          | Channel utilisation, ring high-water, drops and heap were only visible under `--loradebug` at 10 s resolution.                        | Medium | New `STAT` line every 5 min: `util=`/`newid=`/`dup=`/`err=`/`txn=`/`txfail=`/`ringmax=`/`drop=`/`mh=`/`heap=`/`trk=`/`fw=`.  | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
| SL-06 | `src/udp_functions.cpp`, `src/nrf52/nrf_eth.cpp` (server inject), `src/lora_functions.cpp:1372` (`addNodeData` call)                   | Gateway multiplication across several co-visible gateways, and the upload-before-decrement ordering, were not traceable per `msg_id`. | Low    | New `GWI`/`GWU` lines; `ringSource[]` byte marks server-injected ring entries (`src=g` in `TX`).                             | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
| SL-07 | `tools/berglog.py`, `.claude/skills/logauswertung/SKILL.md`, this file, `CHANGELOG-stability.md`                                       | The new line kinds needed a parser, fixtures and a reference doc, or `berglog.py` would call every one of them `undecodable`.         | Low    | Parser + new report sections (Welle 2, agent E); skill reference + backlog/changelog entries (Welle 2, agent F, this entry). | implemented 2026-09-02 (Welle 0/1/2), **upstream in PR #1125** (merged 2026-09-03); bench Welle 3 pending — no bench node attached |
...
### 3.8x Backlog grooming 2026-09-03 — fourteen items

One pass over the open rows, driven by an operator list plus two stale statuses caught on a
read-through. Twelve closed, one fixed, one confirmed still open. No new defects found; what
changed is the status of items whose evidence had already arrived without the row being updated —
`TD-01` had been carrying `open` for four days after it was closed with a 24/24 run, and `TD-06`
`in progress` while its harness was in daily use.

| Item                       | Outcome                                                                                                                                                                                                                                                         |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TD-01` WLAN association   | **Closed, backdated to 2026-08-30** — the row was stale, not the work. Fixed by TM-34 Wave W; 24-boot run gave 24/24 first joins, `got_ip` median 10 394 ms.                                                                                                    |
| `TD-06` test harness       | **Done** — `tools/bench/tdeck_harness.py` (20 scenarios) plus the sibling rigs are in daily use, and HL-01 (`--wifi on/off`) closed the last gap. Net-console observation is `meshlogger.py`, separate by design. CI enablement stays open elsewhere.           |
| `TD-04` Europe map         | **Closed** — tiles installed, map renders on `DK5EN-14`. `G09`/`G10`/`G11` stay open in §3.8c but are no longer a gate.                                                                                                                                         |
| `TD-05` GUI latency        | **Closed** — partial refresh (`disp_drv.full_refresh = 0`) shipped and working; the `// EXPERIMENT` comment in `tdeck_main.cpp` replaced by the §3.8e rationale.                                                                                                |
| `GPS-01..04`, `GPS-06`     | **Closed** — the 8 h overnight `DK5EN-14` capture is the bench proof the rows were waiting for: 9791 evaluations, 0 rejects, 0 corrupt samples at the ordinary 3 s cadence. `GPS-07` was **closed 2026-09-06** (`531d66b4`); `GPS-10` is the remaining GPS row. |
| `TM-35` nRF52 `getUDP()`   | **Closed** — gate met since the async-NTP build; the nRF52 work since has been in the field through the v4.35s.09.03-stability release and the stall never reproduced. Marker stays wired.                                                                      |
| `TM-39` country servers    | **Closed** — ESP32 `CONF` (`b624bd33`) and the nRF52 internet-path country split (`CTY-01`, `a1191eaa`) were the two leftovers. The DL-on-internet asymmetry is a server-topology question, left to upstream.                                                   |
| `TM-49` OTA completion     | **Fixed** — fail-closed `_ota_image_valid` gate in `src/safeboot/ElegantOTA.{h,cpp}`, exactly the inversion the operator proposed. Bench arm on a 4 MB board still owed.                                                                                        |
| `TM-44` W5100S NTP stall   | **Closed** — accepted as-is, no fix, no gate.                                                                                                                                                                                                                   |
| `MH-01` MOD nibble         | **Still open** — re-checked in the tree, `lora_functions.cpp:705` is unchanged. Also corrected a duplicate ID: §3.8d's second item is now `MH-03`.                                                                                                              |
| `CQ-01` BLE PIN on console | **Closed** — intended, risk accepted.                                                                                                                                                                                                                           |
| `APRS-01` APRS to clients  | **Answered, closed as a paper** — PR #1114 built and measured it; upstream merged and reverted it the same day. One decision left: bridge in MCProxy, or leave unserved.                                                                                        |
| `TOOL-06` skill doc        | **Done** — re-verified on disk; all four corrections present, nothing to change.                                                                                                                                                                                |
| `G12` all-at-once          | **Decided: all at once** — overtaken by events; the campaign shipped as one push and is upstream and released.                                                                                                                                                  |
...
- **A duplicate backlog ID existed for five days.** `MH-02` named two unrelated defects filed
  ...
  section that did not allocate it. A sweep of every table row in this document found **no other
  collision** — `CONF-01` appears twice on purpose (summary row in §3.8p, enriched row below it).

- **Status rows go stale faster than the work.** Three of this pass's items (`TD-01`, `TD-06`,
  `TM-35`) were finished days or weeks before the row said so, and `TM-39`'s two leftovers were
  closed by commits filed under other ids (`b624bd33`, `CTY-01`). The evidence was in RESUME.md and
  in the tree the whole time. Cheap countermeasure for the next wave: when a commit closes a row
  that lives in another section, edit that row in the same commit.
...
The reporter dates it to swapping a broken BME280 for a BME680. Root cause established by code
reading, write-up in [`bug-extudp-tele-qfe-20260904.md`](archive/bug-extudp-tele-qfe-20260904.md)
(now archived): the
relayed-node `tele` datagram copies `aprspos.qfe`, which is fed from the APRS `/F=` field, and
`/F=` has carried `node_press_alt` — the barometric **altitude in metres** — since V4.25. The real
pressure (`/P=`, parsed into `aprspos.press`) is never written to the datagram. The gateway's own
...
| ------ | ---- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TLM-04 | BUG  | Low  | `src/extudp_functions.cpp:600`, `src/loop_functions.cpp:4326`, `src/bme680.cpp:159-161`                                                                               | **`"qfe"` in the `lora` tele datagram was an altitude, not a pressure.** **FIXED 2026-09-05**: `qfe` ← `aprspos.press` (`/P=`), `/F=` altitude → new key `pressure_alt`, builder extracted to `src/extern_tele_json.h`, native test `test_extern_tele_json` (3 cases). Ruled a bug fix, not a contract change. Doc note for the installed base in `ext_udp_telemetry.md` §6. Write-up `archive/bug-extudp-tele-qfe-20260904.md` §8 (now archived).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| TLM-05 | GAP  | Low  | `src/mcp17_bits.h` (new), `src/loop_functions.cpp` `PositionToAPRS()`/`sendTelemetry()`, `src/aprs_functions.cpp` `decodeAPRSPOS()`, `src/extern_tele_json.h`         | **MCP23017 port A inputs were read every 5 s but never left the node** (upstream issue 1076: the `T#` digital slot was hardcoded `00000000`; a second request asked for the bits in the position beacon). **IMPLEMENTED 2026-09-06 (`b179fdff`, changelog 207):** `/D=<8 bits>` in the beacon and the same string in the `T#` digital slot, GPA0 first, output pins `0`, only when the chip answered at boot; receiver parses `/D=` into `aprsPosition.din`, EXTUDP `tele` gains an optional `din` key. 19 native cases, bench proof of the receive/export path on `DK5EN-93` via `--injectraw`. **Open:** transmit path unproven on hardware (no MCP23017 on the bench); **upstream in PR #1135 (Abschnitt 6), gemergt 2026-09-10** -- der separat vorbereitete PR-Text blieb ungenutzt (`archive/pr-draft-mcp17-din-20260906.md`); server/app support is not ours; reference `mcp23017-digital-field.md`, §3.8z; beacon-on-change deliberately not implemented, regular interval only. |
...
### 3.8z MCP23017 port A inputs on the air — `/D=` and the `T#` digital slot (2026-09-06)

Upstream issue 1076 plus a field request; implemented as `TLM-05` in `b179fdff`, changelog 207.
The reference for wire format, bit order, letter choice and what is verified is
[`mcp23017-digital-field.md`](mcp23017-digital-field.md); the German PR text is
[`archive/pr-draft-mcp17-din-20260906.md`](archive/pr-draft-mcp17-din-20260906.md).

- **Format:** `/D=` plus exactly eight `0`/`1` characters, GPA0 first, output pins `0`, port B not
  sent; the same string fills the digital slot of the APRS `T#` frame. Sent only when the chip
  answered at boot, no setting, no struct bump.
- **Letter:** `/I=` was proposed and is taken (INA226 current); `D` was the free letter with a
  mnemonic. Table of the occupied letters in the reference doc §3.
- **Receiver:** `decodeAPRSPOS()` → `aprsPosition.din`, EXTUDP `tele` key `din` (omitted when
  empty). Old firmware skips the token.
- **Proven:** 19 native cases, three clean builds, receive/export path on `DK5EN-93` via
  `--injectraw`. **Not proven:** the transmit path on hardware (no MCP23017 on the bench).
- **Open:** bit order to be confirmed with the server side before the upstream merge;
  beacon-on-change deliberately not implemented.

### 3.8aa Release cut 2026-09-06 (v4.35s.09.06.2) — the instrumentation guard's blast radius (INS-01..03)

`INSTRUMENT_ENABLED` was defaulted to 0 in `0b0c0984` (changelog 202) because field nodes printed
`[INSTR-LOOP];gap;...` unprompted and users reported those lines as error messages. Checking
`release-notes.md` against the tree during the release cut showed the guard had taken more with it
than the change described. Both defects were fixed in the same release (`b0d10b59`) and folded into
changelog item 202 rather than numbered separately.

| ID     | Kind | Sev    | Files                                                                                      | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Status                                                                                                                                                                                                                                                                                    |
| ------ | ---- | ------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| INS-01 | BUG  | High   | `src/command_functions.cpp` (block ~4722-5215 before the fix), `src/udp_functions.{h,cpp}` | Four field-diagnostic switches lived inside the `#if INSTRUMENT_ENABLED` block and were compiled out with the bench surface: `--udplog`, `--udpstat`, `--wifistat` (ESP32) and `--ethstat` + `--udplog` (nRF52). `bUDPLOG` therefore had no writer in a shipped image, so the per-datagram `[UDP];rx/tx` lines **and** the gateway's `[GW];rx;type;DATA` line (`udp_functions.cpp:327`, guarded by `gwRxType[0] != 'D' \|\| bUDPLOG`) were unreachable. A gateway operator could not log the UDP path at all. | **FIXED 2026-09-06** (`b0d10b59`): moved into their own section ahead of the guard, `else`-chain preserved, announced in `--help` again per platform. Verified on all four bench boards on the release image — marker returned on each, `--wifistat` correctly absent on nRF52.           |
| INS-02 | BUG  | Medium | `src/command_functions.cpp` (help block ~852-863)                                          | `--help` still advertised `--injectmsg`, `--injectraw`, `--loratx` and the T-Deck UI hooks (`--redrawlog`, `--uistat`, `--tab`, `--drawer`, `--playtone`, `--tft`, `--screencrc`, `--spitrace`, `--touch`) although the handlers are compiled out at `INSTRUMENT_ENABLED=0` — the mirror image of INS-01, and a direct contradiction of item 202's own claim that "`--help` announces the block only where it exists".                                                                                        | **FIXED 2026-09-06** (`b0d10b59`): those help lines are now inside the same `#if INSTRUMENT_ENABLED`. Confirmed by string-scanning the built image: `injectraw`/`injectmsg` absent from `heltec_wifi_lora_32_V3.bin`, present with `-D INSTRUMENT_ENABLED=1`.                             |
| INS-03 | SIZE | Low    | `src/test_inject.{h,cpp}`, `src/capture_functions.h`                                       | `MC_INJECT_HOOKS` still defaults to 1, so the whole injection machinery (`test_inject.cpp`, ~430 lines) is compiled into every shipped image while the commands that reach it are gone. Dead flash and DRAM on boards that have neither to spare — `E22_XML-DevKitC` already sets `MC_INJECT_HOOKS=0` by hand for exactly that reason (`variants/E22_XML-DevKitC/platformio.ini:36`).                                                                                                                         | **OPEN, filed 2026-09-06.** Decide whether `MC_INJECT_HOOKS` should follow `INSTRUMENT_ENABLED` by default. Not urgent: it costs size, not correctness, and the four IRAM-tight envs of `MEM-04` are the ones that would benefit. Measure the saving per env before changing the default. |

**Method note, worth keeping.** Both defects were invisible to the native gate (643 cases green
before and after) and to the compiler: the block still built, the commands simply were not there.
What found them was reading the release notes' own claims back against the tree, and what confirmed
them was a string scan of the built image plus a hardware run. After any change to a compile guard,
scan the image for the commands you expect **present and absent** — on nRF52 scan the `.elf`, the
`.hex` is ASCII and a `strings` scan of it is void.

### 3.8ab CTY-02 withdrawn — upstream issue #1133 refuted (2026-09-06)

| ID     | Kind | Sev | Files                                                     | Finding                                                                                                                                                                                                                                                      | Status                                                                                                                                                                                                                                                                                                     |
| ------ | ---- | --- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CTY-02 | —    | —   | `src/udp_functions.cpp`, `src/nrf52/nrf_eth.cpp` (former) | A gateway fix for upstream issue #1133 ("DL server lost on the Internet path"), written in `b9934a4e`, bench-proven on both platforms (`DK5EN-93` ESP32, `DK5EN-90` nRF52) and drafted as a German PR. The issue was then refuted: the premise did not hold. | **REVERTED 2026-09-06** (`de18bcf0`), byte-exact — `git diff b9934a4e~1 de18bcf0 -- src/` is empty. Nothing of it is in `v4.35s.09.06.2`. Per [[no-repairs-for-reverted-upstream]] the PR draft and the bench notes stay as the record of why it was dropped; do not resurrect without a new reproduction. |

### 3.8ac Track mode ships without a warning — operator intake 2026-09-09

| ID     | Kind | Sev    | Files                                                                                                                                                                                                    | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------ | ---- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TRK-01 | ENH  | Medium | `src/web_functions/web_functions.cpp:1684`, `src/web_functions/web_setup.cpp:243,768`, `src/command_functions.cpp:1627,1645,1700`, `src/loop_functions.cpp:123` (`bDisplayTrack`), ESP32/nRF52 boot path | Turning on track (SmartBeaconing) hands one station a beacon cadence down to 10 s, against a channel that carries roughly one packet every 8 s. The WebGUI switch describes it as `"enable display of SmartBeaconing"` and says nothing about the cost; the serial `--track on` says nothing either, and a node that boots with the flag already set is silent about it forever. The operator's read: this is a live candidate for the packet-loss reports that keep coming in from the field, because a handful of track users is enough to saturate a cell for everyone else. Ask: emit **`! degraded MeshCom RX performance, packetloss likely`** — deliberately blunt — in three places: (a) the WebGUI, next to the switch, shown when it is on; (b) the serial log **once at boot** while the flag is set; (c) the serial log **at the moment it is switched on**, via WebGUI **and** via `--track on`. Wording is fixed by the operator; do not soften it. | **FIXED 2026-09-09** (`7ace4b2f`). Wording lives once in `src/track_warning.h`: `TRACK_WARNING_TEXT` for the WebGUI, `TRACK_WARNING_SERIAL` with the `Track on - ` prefix for the log, where nothing else in the line says which setting fired it. (a) `<span id="track_warn">` next to the switch, rendered per `bDisplayTrack` at page build and toggled live by `setvalue()` on a successful set, so it does not wait for a reload -- `_create_setup_switch_element()` gained two defaulted parameters, the other 23 callers unchanged. (b) `[INIT]...` line at boot in `esp32_main.cpp:842` (`printfdeb`, so it also reaches the net console) and `nrf52_main.cpp:571` (`Serial.printf`, matching its neighbours). (c) **one** call site, not three as sized: `command_functions.cpp:1633` in the `track on` handler covers the WebGUI switch, serial, BLE and the TripleClick in `onebutton_functions.cpp:219` alike, because all four route through `commandAction("--track on")`; it fires on every operation, including a re-issue while track is already on. Operator wording kept verbatim, with the leading `!` dropped on request (2026-09-09) -- the GUI carries the bare text, only the log gets the prefix. Gate: `wiscore_rak4631` and `heltec_wifi_lora_32_V3` build clean with both strings present in the linked images, 12 host envs / 656 native cases green, and the extracted `setvalue()` driven against a DOM stub flips the hint on -> `""` / off -> `"none"` and is a no-op for switches without a warning span. Still open: the fleet has no telemetry for how many nodes actually run with track on. |

### 3.8ad DM transport reliability — concept and simulator (2026-09-09)

**Intake.** Operator: personal messages over pure LoRa RF still get lost; design a transport layer
that fixes that within the RAM and airtime budget, covering ping/pong, offline and temporarily
unavailable destinations, and repeat-traffic control. Eight read-only scouts over the deck, the
concept papers and the firmware; every mechanic below was then re-read in the code by hand.

**Verdict.** The end-to-end ACK (`{NNN` in the DM, `:ackNNN` from the destination) is the right
architecture for a flood network. Three mechanics break it: the retry re-floods the **same msg_id**
and dies in the dedup ring of every node that heard the original, the destination included
(`src/lora_functions.cpp:653`, `:896`, `:1072`); the timer is a constant 40 s against a round trip
of 1 min quiet and 4 min median loaded (DG0OPK per-hop 30 s median, 286 s p99); the ACK is one
fire-and-forget frame (`src/loop_functions.cpp:4858-4922`) and giving up after three retries is
silent (`src/lora_functions.cpp:2078`). DM volume is small, 1,182 per week network-wide against
3,600 to 4,600 HEY per hour, so the budget risk is local bursts, never the aggregate.

**Documents.** [`proposal-dm-transport-reliability-20260909.md`](proposal-dm-transport-reliability-20260909.md)
(English, six options, comparison, bench checks) and
[`konzept-dm-transportsicherung-20260909.html`](konzept-dm-transportsicherung-20260909.html)
(German, konzeptpapier layout, single file; simulator with seven topologies, fault presets,
click-to-break links, stages heute/0/1/2, quiet or loaded channel, and a matrix over all 352
combinations). The simulator models dedup, per-hop queues, hop budgets, own echo, gateway 0x41 and
GATE return; it does not model collisions, CAD or ring eviction. Figures passed a computational
geometry check only; no screenshot was taken (Chrome extension not connected).

| ID    | Type | Sev. | Location                                                                                                                                     | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----- | ---- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DM-01 | BUG  | High | `src/lora_functions.cpp` `updateRetransmissionStatus()` `:2044-2081`, dedup gate `:896`, `src/loop_functions.cpp` `SendAckMessage()` `:4858` | **Stage 0, repair the existing ARQ.** Same-id retry only when no own echo (`own_msg_id[][4]==0x00`) and no ACK within 15 s, max 3; split the gate at `:896` so a duplicate addressed to me with `{NNN` is re-acked (rate 1/min per source+NNN) but not displayed or relayed; report failure to app (0x41, new status) and WebGUI; mint the ACK msg_id from `millis()` and drop `save_settings()` from the nRF52 timer-task path (advisor M5). Zero new bytes on air, zero RAM. **Upstream-PR candidate.** Open.                                                     |
| DM-02 | ENH  | High | outbox: `ringBuffer` parked status (classic ESP32) or new 5-slot table (S3, nRF52); destination: both RX paths (`OnRxDone`, `getMeshComUDP`) | **Stage 1, end-to-end ARQ with two identifiers.** NNN stays the transport sequence number across attempts; every re-flood gets a fresh msg_id (`insertOwnTx()`); destination dedups on (source, NNN) with a 16-entry, 1 h cache and always re-acks; receipt `:ackNNN;H=crc16` (old parsers stop at `;`). Ladder +2, +10, +30, +60 min then hourly, x3 when the 5-min channel utilisation exceeds 25 %, max 6 blind re-floods, no action during QRS/QRT, one outbox action per 30 s. Old destinations show a re-flood twice; mcmap must fold on (source, NNN). Open. |
| DM-03 | ENH  | Med  | sender RX path (presence), `handleACK()` `src/lora_functions.cpp:331` (custody)                                                              | **Stage 2, presence and custody.** Any received frame whose source is a pending destination sends the DM at once, earliest one round trip after the last own send (2 min quiet, 6 min loaded); a gateway 0x41 for a pending DM suspends blind re-floods 15 min. No `{prb}` probe in v1; ping stays one-hop. Open, needs DM-02.                                                                                                                                                                                                                                      |
| DM-04 | ENH  | Low  | text marker `{E}` at the sender; mc-chat / T-Deck outbox                                                                                     | **Stage 3, precedence classes and app-side outbox.** EMERGENCY DMs sent twice up front with fresh msg_ids, larger budget, max_hop up to 6; outbox in the app or on the T-Deck SD for reboot survival and for classic ESP32. Official McApp is outside this repo. Open, after DM-02.                                                                                                                                                                                                                                                                                 |
| DM-05 | GAP  | Med  | `src/setlog` STAT line                                                                                                                       | **No measurement of DM outcome exists.** Add counters: DMs sent, echo heard, gateway ACK, peer ACK, gave up, median time to peer ACK; one week of baseline on DK5EN-98 and -90 before DM-01 ships. Open, first step of the rollout.                                                                                                                                                                                                                                                                                                                                 |
| DM-06 | OPEN | -    | server, apps, parser                                                                                                                         | **Questions the code cannot answer:** does the server hold DMs for offline destinations (decides how strong DM-03's custody rule is); do server and McApp tolerate `;` in an ACK payload; a user `{` in DM text already breaks NNN parsing today (strip at the sender, regression test); (source, NNN) cache ageing versus the 1000-wrap for very chatty senders. Open.                                                                                                                                                                                             |

### 3.8ae APRS position parser contract — firmware, MCProxy, app (2026-09-11)

**Intake.** Parser-drift analysis across the three consumers of the position comment
([`aprs-parser-drift-20260911.md`](aprs-parser-drift-20260911.md)): `PositionToAPRS()` emits 17
`/X=` keys, `decodeAPRSPOS()` read 14, MCProxy typed 10 and dropped one, the app read 13 with one
wrong. No document listed the key space. The app maintainer then pointed out that no parser read
the `#name` suffix the encoder appends after the free-text comment.

**Shipped.** Contract as `docs/architecture/11-wire-format.md` §1.8 (17-key grammar, 100-byte
budget, drop order) and §1.8.1 (`#name` rule: comment region ends at the first `/X=` token, name
is the text after the last `#`, `--setname` strips `#`). Firmware `6fd9c3a5` + `b6d9f3cf`
(`N-32..N-35`, items 212-216), MCProxy `9501bb0` on `development`, app PR #8 merged (`534ba45`)
plus branch `aprs-position-name` (`3f61059`, pushed to the DK5EN fork, no PR). Item 217
(`86be10c8`, group stays in the web GUI destination field) came out of the same bench session.

| ID      | Type | Sev.   | Location                                                         | Item                                                                                                                               | Status                                                   |
| ------- | ---- | ------ | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| N-32    | BUG  | Low    | `src/aprs_functions.cpp` `decodeAPRSPOS()`                       | `/R= /U= /I=` silently dropped on receive                                                                                          | **FIXED** `6fd9c3a5`, item 212, native only              |
| N-33    | BUG  | Low    | `src/aprs_functions.cpp:999-1022`                                | `/Y=` scan inherits the `/V=` buffer                                                                                               | **FIXED** `6fd9c3a5`, item 213, native only              |
| N-34    | BUG  | Medium | `src/loop_functions.cpp` `PositionToAPRS()`, `src/pos_tag_nan.h` | seven of eight NaN guards compared `cpress`; `/H=nan` etc. could go on the air                                                     | **FIXED** `6fd9c3a5`, item 214, `test/test_pos_tag_nan/` |
| N-35    | BUG  | Low    | `decodeAPRSPOS()`, `--setname` handler                           | `#name` not split from the comment, comment cut at 25 bytes / first space                                                          | **FIXED** `b6d9f3cf`, item 216, native only              |
| APRS-02 | TEST | High   | bench fleet, serial + web GUI                                    | Hardware proof for items 212-216: receive a real position carrying `/R= /U= /I=` and a `#name` comment; nothing has run on a board | **OPEN** — blocks the release cut and the upstream PR    |
| APRS-03 | OPS  | Medium | Pi (`mcapp.local`), phone                                        | Deploy MCProxy `9501bb0` to the Pi; phone test of the merged app PR #8 against a live node                                         | **OPEN**                                                 |
| APRS-04 | PR   | Medium | upstream `dev`                                                   | Firmware PR for N-32..N-35 (all upstream defects, small, self-contained); app PR for `aprs-position-name` only on request          | **OPEN** — after APRS-02                                 |

### 3.8af DRY unification and RAM audit — the one-shot PR (audit 2026-09-10, filed 2026-09-11)

**Intake.** [`optimization-audit-20260910.md`](optimization-audit-20260910.md) (+ appendix,
`d54d609f`) answered two operator questions: is the duplicate code worth unifying, and is
callsign storage worth compacting. Both hold. It is the successor of the Track-B epics
`DRY-20`/`DRY-21`/`DRY-23`/`DRY-24`, `SIMP-26`/`SIMP-27`, `STATE-28` and `ALT-31`/`ALT-32`,
which §3.5 parks as "propose upstream as a plan first": the audit **is** that plan. Nothing
of it is in code; the audit was not referenced from this file or RESUME until today.

**Verdict.** One 5,935-line dispatcher, ~1,700 lines of ESP32/nRF52 twin code with 23 named
drift bugs, three parallel UI trees, settings fields enumerated by hand in six places. RAM
bundle without touching the wire contract: ~21.8 kB static DRAM on a classic E22, 18.3 kB
Heltec V3, 16.9 kB RAK4631, 48 kB T-Deck Pro from one config line. The binding constraint is
IRAM on the three T-Beam classic envs (21 bytes free, `MEM-04`), and the only lever there is the
PSRAM board flag, blocked on one bench reading. Thirteen defects surfaced as a by-product,
all still present on 2026-09-11 (re-checked on the cited lines).

| ID      | Type | Sev.   | Location                                                          | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------- | ---- | ------ | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| OPT-01  | EPIC | High   | audit §7, waves 0-7                                               | **The one-shot upstream PR.** Wave 0 golden captures + region baseline; 1 defect one-liners + hygiene deletions; 2 RAM bundle L rows; 3 settings schema table `D1-05`, single struct `D1-04`, migration `D1-06`, `D1-07/08/02/03`; 4 command table `D2-10/06/07`; 5 RAM bundle M rows; 6 shared UDP frame handler `D1-01` + `D3-01/02/05`, `ui_common`; 7 variants restructure `D6-01..03`. Each wave has its gate in §7.1.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **OPEN, not started.** Needs the upstream window (one large PR, then back to minimal changes) — operator decision when to open it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| OPT-02  | TEST | High   | new `tools/` capture, USB + TCP 2323, 13 BLE registers            | **Golden command capture** (audit §7.3, `D2-V`): every command over both paths, canonical / out-of-range / non-numeric / adversarial prefix set, diffed pre/post on the same hardware. Does not exist; prerequisite for wave 4 and the safety net for all other waves.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **OPEN** — first concrete step of OPT-01                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-03  | DEC  | Medium | operator                                                          | Decisions owed before waves 5/6: `OPT-D3`, `OPT-D4` (behaviour / stored-value changes), `R3-11` + `D2-09` (diagnostics compiled out on some boards), `R3-03` (OneWire off on T-Beam), OLED-less board list for `R4-02/03`, nRF52 settings migration vs wipe for `D1-04`. **The `D1-04` migration-vs-wipe decision no longer rests on an unchecked premise.** `OPT-03` chose migrate over wipe because _flash is cheap_; that premise is now MEASURED for this feature rather than assumed -- see `OPT-07` and `docs/opt07-nrf52-settings-store-sizing-20260912.md`. The persist set's worst case is 2 937 B string-keyed (1 531 B tagged-record) against **149 304 B of free flash** on `wiscore_rak4631`, the tightest nRF52 env, with peak extra RAM to load <= 128 B; re-verified against the post-`W2` tree. So the decision stands and `W3` must not be argued on flash bytes. **Read the scope of that confirmation precisely:** the premise holds HERE because this feature is small relative to the headroom that happens to exist today -- it is NOT a finding that headroom is generically safe on this platform. `MEM-04` (two envs within 4 kB of a link failure) is the standing counter-example and is not repealed by it. Anyone citing "flash is cheap" for a different row owes their own measurement. | **PARTLY CLOSED 2026-09-12.** The nRF52 migration-vs-wipe question is settled: migrate, string-keyed encoding, premise measured (`OPT-07`, done). Still owed from this row: `OPT-D3`, `OPT-D4`, `R3-11` + `D2-09`, `R3-03`, and the OLED-less board list for `R4-02/03` -- all of them wave 5/6 decisions, none blocking `W3`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| OPT-04  | GAP  | Medium | audit §7.2                                                        | Left out with gates: `D1-09` common gateway block (fixes `OPT-D5`/`OPT-D6`, needs 24 h soak RAK-90 + Heltec-93), `D1-10` common loop scheduler, `R1-05` frame pool, `R2-02/03` hop-index rows, `R3-02` T-Beam PSRAM unflag (one `[PSRM]` reading on T-Beam-92 — the `MEM-04` lever), `R3-15`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **OPEN**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-07  | MEAS | High   | `nrf52_flash.cpp`, `config_json.cpp`, `variants/wiscore_rak4631`  | **Size the nRF52 keyed settings store before `W3` builds it.** `OPT-03` chose migrate-over-wipe on the premise that _flash is cheap_, and the `D1-04` target architecture depends on that premise for step 3 (a key-addressed store replacing the raw-struct blob). **Nobody has measured it.** A serializer plus per-key overhead costs flash **and** RAM during load, on the 248 KB platform, and `MEM-04` is the standing proof that "there is room" is exactly the assumption that breaks here -- two envs were within 4 kB of a link failure until 2026-09-12. Deliverables: flash delta on `wiscore_rak4631`, `heltec_t114` and `t_echo`; peak RAM during load; worst-case on-disk size for the full persist set (the `D1-04` triage output, ~89-147 fields); and the same three numbers for the **tagged-record fallback** (2-byte key id + length + value), which keeps self-description at lower cost and still retires the compat struct. Measure same-base with `tools/resource_watch.py regions` against a map, never the `RAM:`/`Flash:` summary lines and never `snapshot` (it overwrites the stale baseline).                                                                                                                                                                                            | **ANSWERED 2026-09-12 -- both encodings fit, and the premise was understated.** `docs/opt07-nrf52-settings-store-sizing-20260912.md`. Today's blob is **2 000 B** (`meshcom_settings` is `0x7d0` in the linked image). A **tagged-record** store worst-cases at **1 531 B -- 469 B SMALLER than today**; **string-keyed** at 2 937 B, against 148-218 kB free flash on the three nRF52 envs. Peak load RAM <= 128 B. **The serializer is already paid for:** `configExportJson` (1 508 B) and `configImportJson` (7 184 B) are already linked into the nRF52 image and already in production use from the web config page (`web_functions.cpp:426`, `:499`), so the marginal code cost is file-I/O glue, not a new serializer. The importer already does CRC32, per-field type and range checks, atomic apply ("writes nothing unless every check passed") and **ignores unknown keys** -- the exact forward-compatibility the target needs. Recommend **string-keyed**: headroom is not the binding constraint, it reuses the canonical text `configExportJson` already emits, and it leaves the store human-readable on the filesystem. **One catch that must not be missed in `W3`:** `config_json.cpp:619` rejects any document whose `layout` != `FLASH_STRUCT_VERSION` (`CFG_IMP_ELAYOUT`). That is right for importing a foreign config and exactly wrong for reading your own settings back -- it would reinstate the version gate this whole architecture exists to abolish. The two paths must diverge on that one check. Still unmeasured: no write path has been built, so the file-I/O cost is a paper estimate **DONE 2026-09-12** -- `docs/opt07-nrf52-settings-store-sizing-20260912.md`. Both encodings fit with room to spare on all three nRF52 envs; take the STRING-KEYED form, not the tagged-record fallback. Persist set worst case 2 937 B string-keyed / 1 531 B tagged, against 149 304 B of free flash on the tightest env (`wiscore_rak4631`) -- a 50x margin, so `W3` must not be decided on flash bytes. Peak extra RAM to load is <= 128 B. `config_json.cpp`'s 15 592 B is ALREADY PAID and the keyed store reuses that table rather than adding a second one. Re-verified against the post-`W2` tree (section 1a): every env gained a little, the verdict is unchanged. **The premise `OPT-03` leaned on holds here -- but because the feature is small, NOT because headroom is generically safe on this platform; `MEM-04` remains the counter-example and its lesson is not repealed by this result.** **CORRECTED 2026-09-12 evening -- the 2 937 B figure and the "50x margin" above are WRONG and must not be cited again.** They measured the wrong worst case: every STRING field was filled to capacity, but the numerics were left at their struct defaults. A `double` at `%.17g` is ~24 characters. Re-measured with BOTH axes at worst case together: **4 036 B against the then-4 096 B buffer -- 60 bytes**, and once `W3` restored `node_msgid` / `node_ackid` to the schema (107 -> 109 rows) it became **4 082 B, 14 bytes of headroom**. Derived independently twice, in agreement. The buffer was raised to 8 192 B in `af00c402`. The lesson is not "flash is cheap" and it is not even about flash: **two entirely routine rows consumed three quarters of what looked like a comfortable margin**, and the overflow would have been silent -- `encode()` is all-or-nothing, `settingsStoreSave()` returns false, 234 `save_settings()` call sites ignore that return, and `DO_DEBUG 0` compiles out the message that would have said why. Re-measure on every schema change; the sweep is in `docs/w3-settings-verdict.md`. |
| OPT-D1  | BUG  | Low    | `batt_function_old.cpp:198`                                       | `adc_chars[sizeof(esp_adc_cal_characteristics_t)]`: 36 copies of a 36 B struct, 1,296 B in T-Beam/V3/t_deck_pro images                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | open, one-liner (wave 1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-D2  | BUG  | Low    | `command_functions.cpp:3959`, `:4027`                             | `--setowndns` handled twice, second block unreachable (= `NET-04`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | open, delete (wave 1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| OPT-D3  | BUG  | Low    | `command_functions.cpp:3245`, `:3254`                             | `--softser app0` shadowed by the prefix match on `--softser app`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | open, behaviour change — decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| OPT-D4  | BUG  | Medium | `command_functions.cpp:4696-4729`                                 | `--specstep` writes `node_specsamples`, `--specsamples` writes `node_specstep`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | open, stored-value change — decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| OPT-D5  | BUG  | Medium | `nrf52_main.cpp:2069-2073` vs `esp32_main.cpp:3849`               | nRF52 gateway calls `sendUDP()` only in the pass where `getUDP()` returned "no packet"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | **SUPERSEDED 2026-09-12 by `DR-14` (both-valid, stays split).** The audit's 'ESP32 looks correct' was rejected at the M2 review: on RAK4631 the W5100S and SX1262 share the SPI bus, `gatewayService_nrf52()` holds `bSPI_ETH_Active` and defers the radio re-arm, so an unconditional drain would lengthen that window. Residual risk (TX ring drains only on RX-free passes) accepted knowingly; a source comment saying so is owed in the C4-diag mini-wave. Not a defect, no soak                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| OPT-D6  | BUG  | Medium | `nrf52_main.cpp:2104`                                             | nRF52 heartbeat recovery nested under `!hasIPaddress`: live link with silent server never recovers (ESP32 has stage-2 `resetMeshComUDP()`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **SUPERSEDED 2026-09-12 by `DR-15` (both-valid).** The premise was wrong: ESP32's stage 2 explicitly declines to reset when WiFi is up ("server unresponsive, waiting"), so both platforms recover on link-down and wait on link-up-but-silent. Only the diagnostic stage differed; that port is `DR-03` (C4-diag mini-wave). Fable review 2026-09-12 added the coupling: on nRF52 a cable pull reaches the recovery branch only after `MAX_ERR_UDP_TX` failed sends clear `hasIPaddress` (`udp_drain_nrf52.cpp:59`); the PHY poll never clears it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| OPT-D7  | BUG  | Medium | `nrf_eth.cpp:400` vs `udp_functions.cpp:214`                      | nRF52 UDP zero-scan reads `buf[i+1]` past the datagram on odd sizes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | open, one-liner (wave 1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-D8  | GAP  | Medium | `nrf52/WisBlock-API.h` vs `esp32/esp32_flash.h`                   | **Corrected 2026-09-12 at the M2 review (`DR-12`): this row undercounts and misnames.** Measured field-by-field (`docs/d1-04-settings-field-triage-20260912.md` §4d), nRF52's struct lacks **14** persisted ESP32-only fields, not 6: `node_disp_rot`, `node_map`, `node_audio_start`, `node_audio_msg`, `node_keyboardlock`, `node_backlightlock`, `node_kbllightlock`, `node_modus`, `node_mute`, `node_persist_to_flash`, `node_persist_to_sd`, `node_immediate_save`, `node_kbl_sync`, `node_wifion`. **`node_ntp` does not belong on the list at all** -- it is RUNTIME, recopied from `node_ownntp` on every connect (`udp_functions.cpp:985,987`), so porting it would port derived state; `node_ownntp` is the actual setting. Mirror gap in the other direction, also unnamed here: ESP32 lacks nRF52's `send_repeat_time` and `auto_join` (LoRaWAN OTAA). 13 of the 14 are T-Deck hardware UI state and are correctly platform-scoped; `node_disp_rot` is the only genuinely portable one.                                                                                                                                                                                                                                                                                                                    | **SUPERSEDED by the `D1-04` target architecture, `DR-12` decided `both-wrong` 2026-09-12.** The premise -- "shared code cannot touch them" -- only holds while the struct IS the on-disk format. Under the keyed schema store the field set stays platform-scoped via `CFG_FIELD_LIST_PLATFORM` and no struct has to grow. Do not implement this row as written; it is kept for the corrected field list                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-D9  | BUG  | Low    | `nrf52_main.cpp:2018`/`:1974` vs `esp32_main.cpp:3462`/`:3409`    | Telemetry gate uses `bHeyFirst` on nRF52 where ESP32 uses `bTeleFirst && bAllStarted`; no `extra_hey_time` spreading on nRF52                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **SCOPED 2026-09-12 by `DR-16` (esp32-correct): `bTeleFirst` only.** `bAllStarted` and `extra_hey_time` are explicitly out (network-readiness state machine and a softser-only grace). Cost today is ~15 s, not minutes; the reason to fix is the shared flag, not the delay. Ships in the C4-diag mini-wave with `DR-03` (main-loop code, `D1-10` territory), not in W6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-D10 | BUG  | Medium | `esp32_main.cpp:4380` vs `nrf52_main.cpp:2951`                    | `char msg_buffer[600]` on the ESP32 loop stack; the N-22 fix made it `static` on nRF52 only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | open, one-liner (wave 1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-D11 | BUG  | Low    | `command_functions.cpp:3695`                                      | `--pingcall` bounds the write into `node_pingcall` with `sizeof(node_call)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | open, one-liner (wave 1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| OPT-D12 | BUG  | Medium | `nrf_eth.cpp:563-581` vs `ack_attribution.h:122-137`              | nRF52 hand-built ACK phone frame sends `out[6]=0`, never the attribution suffix ESP32 sends (= `D1-02`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | open, analyst report, wave 3/6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| OPT-D13 | BUG  | Medium | `udp_functions.cpp:287,339` vs `nrf_eth.cpp:468` (appendix D1-01) | RX-01 unconfigured-source guard, `bGATEWAY_NOPOS`, `sendDisplayPosition()` for server frames, `hb_warn_logged` reset, `decodeAPRS()` return check: each on one platform only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | open, analyst report, carried by `D1-01` (wave 6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

**Process rule that stands regardless of OPT-01** (C06 in `code-quality-2.0.md`, `CQ-10` (c)):
...
| RF-03   | BUG  | Medium | **`lora_setcountry()` case 7 (MAN, manual) silently discards the user's settings on nRF52.** Its validation is written in ESP32 units (`lora_setchip.cpp:299-332`): `node_bw != 125 && != 250` is always true against an index, `node_cr < 5 \|\| > 8` is always true against a 1..4 index, and the frequency window `430.0..439.0 \|\| 869.4..869.65` is compared in MHz against a value stored in Hz, so it never matches. Every `--setcountry 7` on an nRF52 board therefore forces `node_freq = RF_FREQUENCY`, `node_bw = LORA_BANDWIDTH` and `node_cr = LORA_CR`, keeping only `node_sf`. The one case whose entire purpose is to preserve manual settings preserves nothing on that platform.                                                                                                                                    | **FIXED 2026-09-11.** Case 7 normalizes with `getBW()`/`getCR()`/`getFreq()`, decides in one unit and writes back in the platform's unit. The guard-band expression is deliberately left at `/100.0`, see `RF-04`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| RF-04   | BUG  | Low    | **The manual-mode guard band is ten times too wide, on both platforms.** `lora_setcountry()` case 7 computes `dec_bandwith = (bw_kHz/2.0)/100.0`; half of 250 kHz is 0.125 MHz, not 1.25 MHz, so the accepted window is 431.25..437.75 instead of 430.125..438.875. `--txfreq` has the same expression (`command_functions.cpp`), where it is fed `LORA_BANDWIDTH` in platform units on top -- the index 1 on nRF52, giving a 0.005 MHz margin there.                                                                                                                                                                                                                                                                                                                                                                                  | **FIXED 2026-09-12.** `/100.0` -> `/1000.0` at both sites, and `--txfreq` now converts `LORA_BANDWIDTH` through `radioBwStoredToKhz()` instead of feeding the raw macro in. **The row understated it twice.** (1) On 868 the guard was not merely wide but **unsatisfiable**: `freq >= 869.4+1.25 && freq <= 869.65-1.25` is `>= 870.65 && <= 868.40`, which no value satisfies, so that half of the `                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |     | `was never true and every 868 frequency an operator set was **silently overwritten with the 70cm board default** -- the guard does not reject, it assigns`RF_FREQUENCY`. A legal ISM band the firmware would not let you configure, with no error shown. (2) `LORA_BANDWIDTH`is`250`(kHz) on ESP32 but`1`(an index) on nRF52, so`--txfreq`computed the guard from a different **quantity** per platform, not a different scale. Windows now: 70cm`430.125..438.875`; 868 `[869.525, 869.525]`at 250 kHz BW -- a single point, which is physically correct: the SRD860 sub-band 869.4..869.65 is itself exactly 250 kHz wide, so one 250 kHz channel fits in exactly one place, and that place is country 5's own frequency. Narrower BW opens it proportionally (125 kHz ->`869.4625..869.5875`). Guarded by two new `radio_units_lint.py` rules, which is what actually protects this -- the unit test is self-contained (`env:native`cannot compile`lora_setchip.cpp`) and stays green if the source is reverted; the lint exits 1 |
| RF-05   | BUG  | High   | **`--txfreq` stored MHz instead of Hz on T114 and T-Echo.** The `*1000000` conversion was `#ifdef BOARD_RAK4630`, while `node_freq` holds Hz on `BOARD_RAK4630 \|\| USE_HELTEC_T114 \|\| BOARD_T_ECHO`. Same mistake as `RF-02`, different field, and the worst of the family: the node would have run 433 Hz through `Radio.SetChannel()`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **FIXED 2026-09-11** with `radioFreqMhzToStored()`. Found while fixing `RF-01..03`, by sweeping for the lone `BOARD_RAK4630` guard rather than by reading the one site                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| RF-06   | BUG  | Low    | **The `--info` frequency readout was raw Hz on T114 and T-Echo.** Same lone `#ifdef BOARD_RAK4630` on the `node_qrg / 1000000.0` display normalization. Display only; it never reached the radio.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | **FIXED 2026-09-11** with `radioFreqStoredToMhz()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| RF-07   | LEAD | Low    | **`taskENTER_CRITICAL()` in `lora_functions.cpp:159/166/174/181` is guarded on `BOARD_RAK4630` alone.** Filed on the assumption that T114 and T-Echo therefore lose those display-queue critical sections.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | **RESOLVED 2026-09-12 -- NOT a defect; the row's premise was factually wrong.** `BOARD_RAK4630` is not board-specific in this tree: `[nrf52_base]` (`platformio.ini:104`) sets `-D BOARD_RAK4630="RAK4630"` at `:123`, and both `heltec_t114` and `t_echo` inherit it via `extends = nrf52_base` + `${nrf52_base.build_flags}`. The ini says so in its own comment: the define is inherited by those two boards ABSICHTLICH and unlocks ~60 nRF52 code sites. All three nRF52 boards compile the critical sections; nothing was ever disabled. Comment-only change at `src/lora_functions.cpp`, which also warns against "fixing" this by widening the guard -- that would be a no-op dressed as a concurrency fix. Found while carving out the wave-5 leads; see also `displayMux`, the vestige of the queue these sections protect.                                                                                                                                                                                                                                                                                                                                                                   |
| RF-08   | LEAD | Medium | **Country 5 (868 MHz) carries `track_freq = 999`, and on ESP32 that value reaches the radio.** Every other country sets `node_track_freq` to `LORA_APRS_FREQUENCY` or a real frequency (Poland uses 434.855); case 5 alone assigns the literal `999` (`src/country_profile.cpp`, the block carved out of `lora_setcountry()` unchanged). On the SX127x path `lora_setchip_aprs()` takes the APRS frequency straight from that field (`lora_setchip.cpp:455`, `float rf_freq = meshcom_settings.node_track_freq;`), so an ESP32 node on `--setctry 5` with track enabled would configure the APRS radio to **999 MHz**. The SX126x path is unaffected: it uses `LORA_APRS_FREQUENCY` directly (`:441`), which is another platform split.                                                                                                | **RESOLVED 2026-09-12 -- NOT a defect; documented, nothing changed.** Both open questions answered by tracing the code. (a) Track mode IS reachable on an 868 node: `--track` has no country guard, so `node_track_freq` really does reach `lora_setchip_aprs()`'s ESP32 branch and `radio.setFrequency(999)`. (b) But a guard exists one level down, in RadioLib: every driver this firmware instantiates range-checks before any register write -- `SX1278` 137..525, `SX1262` 150..960, `SX1268` 410..810 MHz. 999 is above all three ceilings, so `setFrequency()` returns `RADIOLIB_ERR_INVALID_FREQUENCY`, `lora_setchip_new()` returns false (`lora_setchip.cpp:506-509`), and the TX is rolled back -- nothing reaches an antenna. So `999` is a de-facto sentinel for "no APRS sub-channel in this region", safe **only** because it sits above every shipped radio's ceiling. Made explicit as `TRACK_FREQ_NONE_SENTINEL` with the whole guard chain recorded at `src/country_profile.cpp`, including a warning to re-derive the guard before changing the value: a new chip family or a RadioLib range change would silently break it. Numeric value unchanged, so no golden baseline moved. |
| MEM-05  | GAP  | Low    | **Every nRF52 translation unit that includes `IPAddress.h` gets its own 8-byte copy of `INADDR_NONE` in BSS, plus a dynamic initializer.** The Adafruit nRF52 core declares it `const IPAddress INADDR_NONE(0,0,0,0);` at namespace scope (`framework-arduinoadafruitnrf52/cores/nRF5/IPAddress.h:76`); a namespace-scope `const` has internal linkage, and the non-trivial constructor stops the compiler discarding it. Measured on the current tree: **17 copies per nRF52 image = 136 B of BSS** and 17 `.init_array` entries, on RAK4631, T114 and T-Echo alike. **The ESP32 core already fixed exactly this** and says so in a comment: `// changed to extern because const declaration creates copies in BSS of INADDR_NONE for each CPP unit that includes it`, `extern IPAddress INADDR_NONE;` -- so ESP32 images carry none. | **OPEN, upstream-framework issue, not ours to patch directly.** Found 2026-09-11 while accounting for the C4 carve-out's RAM delta: the new TU added the 17th copy. Not worth acting on for its own sake (nRF52 boards have ~160 kB free), but it is the reason any new nRF52 TU that touches `nrf_eth.h` costs 8 B, and that should not be mistaken for the carve introducing state. A local shim header that declares it `extern` and defines it once is the obvious fix if the count ever matters                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| GLD-01  | BUG  | High   | **The G0 EXTUDP goldens are truncated at 160 characters and cannot detect a change past that point.** `extudp_peer.py:327` prints `{dg.text[:160]}`, and the G0 captures were made by redirecting that output, so every datagram longer than 160 characters is cut. Measured on all four nodes: max line 189 (prefix + 160), 2-3 lines per node with no closing brace. Every position frame is ~258 B, so its whole tail -- altitude, battery, firmware, `msg_id`, `rssi`, `snr` -- is absent from the baseline. The G1 capture drives `ExtUdpPeer` as a library instead and records in full (max line 274, zero truncated).                                                                                                                                                                                                           | **OPEN, found 2026-09-11 while running G1.** The G0 EXTUDP baseline is only valid for the first 160 characters of each datagram; a G1/G2 comparison must say so, or the captures must be retaken. Retaking them means reverting to the tag, so the honest option is to state the limit. Two further gaps in the same surface: the normalizer does not mask the live readings in EXTUDP JSON (`lat`, `long`, `temp1`) the way `ble_golden.py` masks its own volatile keys, and there is no compare tool for EXTUDP at all                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
#### Operator decisions taken 2026-09-11 (before the phase-1 bench work)

| #   | Question                                                | Decision                                                                                                                                                                                                                                                                                                   | What it costs or obliges                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `OPT-06` — how to point RAK-90 at the stub server       | **Add an nRF52 `--srvip` hook.**                                                                                                                                                                                                                                                                           | Product code, so phase 0's "no product code" rule ends here. The hook must sit behind the same `INSTRUMENT_ENABLED` guard as the ESP32 one, be listed in the PR as instrumentation, and the image must be string-scanned afterwards (`INS-01` lesson). In exchange the nRF52 UDP path becomes testable at all, which `D1-01` needs.                                                                                                                                                                                                                                                                                                                                                                               |
| 2   | Bench images for the G-runs                             | **`INSTRUMENT_ENABLED` throughout.**                                                                                                                                                                                                                                                                       | One flash per node per G-run, and `--srvip` available everywhere. But the golden then does **not** describe the shipping image, and the 76 extra command branches appear in every capture. Consequence that must not be dropped: the string scan of the command-name list across all 32 **shipping** images (audit §7.3) becomes the _only_ evidence about the shipping command set, so it is now mandatory rather than a cross-check.                                                                                                                                                                                                                                                                            |
...
SX126x, ST7789 on the other), so `D1-10` needs a carve before it can be tested
natively -- the same lesson as `C1`/`C2`/`C3`, for the fourth time.
...
##### Step 1 is done: the field triage, 2026-09-12

`docs/d1-04-settings-field-triage-20260912.md` classifies all 147 fields:
**106 PERSIST, 41 RUNTIME, 0 UNCLEAR** -- the codebase's own comments settled
every ambiguous case, so nothing rests on a guess. Persist set: **106 fields,
...
after a plain reboot, lost part of the configuration. Downgrading to a
pre-cutover image then produced a full factory reset. The node was restored
completely -- but only because the baseline export had been captured first,
which is the single thing that made this recoverable.

**The Fable review then refuted the diagnosis the fix plan rested on**
(`docs/w3-settings-verdict.md`). All six reverted fields DO have schema rows;
survivors and casualties interleave in encode order. The 724 "uncovered"
struct bytes are flash markers, the device EUI (rebuilt each boot), deliberate
exclusions and the struct's own `// nicht im Flash` block. **There were no
missing fields to add**, and the byte-coverage gate proposed in response was
itself the wrong instrument -- it invents a padding-classification problem, is
per-build-env rather than per-platform, and cannot see a field covered by a
wrong-TYPED descriptor. Its replacement is the member-level fail-closed
invariant described in the verdict, still to be built.

**Four confirmed defects were found on the way and fixed** (`af00c402`):

- `flash_reset()` called `InternalFS.format()`, which erased **every BLE
  pairing** under `/adafruit/bond_prph/` as well as the keyed store, and
  rewrote the legacy blob with defaults -- which is why the downgrade
  factory-reset the node rather than recovering it. Now removes its own two
  files; `format()` is a fallback.
- The buffer sat at 4 082 B against 4 096 B. See the correction in `OPT-07`.
- `node_msgid` / `node_ackid` had stopped persisting, restarting at 0 every
  boot and replaying msg_ids into neighbours' dedup rings. The reasoning error
  was collapsing "deliberately not **exported**" into "deliberately not
  **persisted**" -- `config_json.h` answers only the first, and the nRF52 blob
  persisted everything.
- `max_hop_text` was a second sentinel-outside-its-range row. The earlier claim
  that `node_power` was the only one was wrong because the sweep used was "the
  fields `settings_sanitize.cpp` names" instead of the correct rule, **"rows
  whose struct default lies outside their declared range"**. Sweeping properly
  now returns 0 of 109.

**Two tests had quietly stopped testing.** The malloc fault injector matched on
an exact size, so raising the buffer silently disarmed it and both
allocation-failure cases went green while asserting nothing; and the fake
filesystem truncated on write-open where real LittleFS **appends**, which would
have hidden a dropped `remove()`. Both fixed at the mechanism level -- the
...
- **8 fields classified RUNTIME are persisted anyway** -- `node_msgid`,
  `node_ackid`, `node_temp`, `node_hum`, `node_press`, `node_temp2`,
  `node_gas_res`, `node_co2`. Verified: each has a `preferences.put*` call in
  `esp32_flash.cpp`. Live sensor readings and message counters written to NVS,
  costing flash write cycles for data that is stale the moment it is read back.
- **25 fields are persisted but absent from the `X()` table**, so they survive
  a reboot but are invisible to config export/import.
- **16 fields exist on one platform only and are persisted there** -- these
  decide whether the merged struct grows or the field dies. `OPT-D8`
  undercounts this: it names 6 of the 14 ESP32-only ones.
- **The `// nicht im Flash` divider in the struct is stale.** `max_hop_text`
  and the whole T-Deck block sit below it and are persisted regardless.
  Anyone reading it as the persist/runtime boundary gets it wrong.

**A landmine for step 6, found while checking the above.** The nRF52 struct
contains **zero** `String` members today, which is exactly what keeps its
raw-struct blit safe. The two `String` fields (`node_audio_start`,
`node_audio_msg`, `esp32_flash.h:226-227`) are ESP32-only, and ESP32 persists
by NVS key, so the text is stored properly. Merge the two structs while nRF52
still blits, and nRF52 writes **a heap pointer** to flash instead of the text,
then dereferences garbage on reload. This is the concrete reason the struct
merge is step 6 and not step 1: either convert those two to fixed `char[]`
first, or land the merge only after the keyed store has replaced the blit.
...
**Nobody has measured the nRF52 cost -- tracked as `OPT-07`.** `OPT-03` chose migration on the
...
(Superseded by the wave-5 stand below -- six of the `N1` suites exist now.
This subsection is kept for the `U2`/`U6` correction it records.)
...
| Unit                      | Surface    | Baseline at G0                  |
| ------------------------- | ---------- | ------------------------------- |
| U1 UDP frame handler, in  | `H6`       | **yes**, four nodes             |
| U9 UI trees               | `H11`      | **yes**, T-Deck, by eye         |
| U1/U2 UDP upstream + send | `H7`       | no                              |
| U3 `checkSerialCommand()` | `H3`       | Heltec only, and not a baseline |
| U4 settings               | `H2`, `H9` | no                              |
| U5 command ladder         | `H3`       | Heltec only, and not a baseline |
| U6 `lora_setcountry()`    | `H2`, `H3` | no                              |
| U7/U8 APRS + mheard       | `H5`, `H3` | no                              |

Consequences that follow, and that the after-run must not paper over:

- **G1 cannot prove the C1-C5 carve-outs are behaviour-neutral.** It can prove
  it for the inbound UDP handler and the T-Deck UI. For everything else the
  neutrality claim rests on the 32-env build gate, the region gate and reading
  the diff -- which is evidence, but it is not measurement, and §9's pass
  criteria have to say so rather than report a clean G1 as if it covered the
  carve.
- **N1 has to carry more than the plan assumed.** It substitutes well for the
  settings layout (U4), ladder order (U5) and the country table (U6). It does
  not substitute for U2's datagrams or U7/U8's frames on the wire: those
  fixtures would be written from the same code being changed, so the test
  would agree with the defect.
- **The before-capture window is closed.** Once C3-C5 land, a capture taken
  then contains the carve-outs, so this decision cannot be revisited later at
  the same price. It can only be revisited by reverting to the tag.

#### Phase B4 stand after wave 5, 2026-09-12: six `N1` suites

| Unit | Test                       | Env                             | Commit     | Cases |
| ---- | -------------------------- | ------------------------------- | ---------- | ----- |
| `U1` | `test_udp_frame_twin`      | `native_udp_frame_twin`         | see `C1`   | 14    |
| `U2` | `test_udp_send_twin`       | `native_udp_send_twin`          | `bbf6665c` | 15    |
| `U3` | `test_serial_command_twin` | `native_serial_esp32`/`_nrf52`  | see `C3`   | 6 x 2 |
| `U6` | `test_country_twin`        | `native_country_esp32`/`_nrf52` | `e662a6d1` | 4 x 2 |
| `U7` | `test_aprs_epilogue`       | `native_aprs`                   | `cf762bc8` | 8     |
| `U8` | `test_mheard_render`       | `native_aprs`                   | `cf762bc8` | 15    |

`U4` (settings layout) and `U5` (ladder order) are **not** twins and will not
become twins: neither unit can be linked natively at any reasonable price, so
both are covered by static Python gates instead --
`test/golden/settings_layout_lint.py` and `test/golden/command_ladder_lint.py`,
each with `--self-test`, each wired into `selftest.sh`. That is a weaker
instrument than a twin and the row says so: a lint reads the source, it does
not run the code. `U9` (UI trees) still needs hardware and is unstarted.

**Two findings came out of `U7`/`U8`, both pinned rather than fixed.** Both are
characterization results -- the tests assert today's behaviour, so they will
fail the day someone changes it, which is the point.

| ID      | Type | Sev. | Location                                                    | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------- | ---- | ---- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OPT-D15 | BUG  | Low  | `src/aprs_functions.cpp:1356` (encode) vs `:470` (decode)   | **The APRS epilogue loses a sub-version of `0x7E` on every round trip.** Epilogue layout is `[payload] 0x00 HW MOD FCS_HI FCS_LO FW LASTHW SUB 0x7E`, with `FCS` a 16-bit big-endian sum over bytes `0..MOD`. The encoder already knows one byte value is unusable and substitutes `0x23` when `SUB == 0x00` -- but it writes `SUB == 0x7E` verbatim, and `0x7E` is the frame terminator. The decoder reads it as `'#'` (`:470`). A node whose sub-version is `~` therefore advertises a different one after one hop, silently. Pinned by `test/test_aprs_epilogue` (8 cases, 4/4 mutations caught).                                                                                    | **DECIDED 2026-09-12 (`DR-27`, both-valid): no code change, ever.** `0x7E` is declared a reserved sub-version value; the constraint is documented instead of engineered around. Fable review confirmed it is enforceable: no runtime path sets the SUB byte, and the decoder's `0x7E -> '#'` (`aprs_functions.cpp:470-473`) is the guard that holds across relays. Owed: the note in `docs/architecture/11-wire-format.md` recording BOTH guards. The '4/4 mutations' figure has no artefact in the tree                                                                                                            |
| OPT-D16 | GAP  | Low  | `src/mheard_functions.cpp`, `showMHeard()` / `sendMheard()` | **The mHeard list is not sorted -- it is a raw ring walk.** `showMHeard()` is a plain `for(int iset=0; iset<MAX_MHEARD; iset++)` over the slot array, so the displayed order is slot order, which on a fresh node is roughly reverse discovery order and looks meaningful without being it. Proved with an ordering that separates slot order from hearing order from most-recent-first (heard `C,A,B` -> displayed `B,A,C`; re-hearing `C` did not move it). Second, smaller drift: `sendMheard()` puts raw ASCII in `PLT` (`'!'` -> 33) where `showMHeard()` prints decoded text. Pinned by `test/test_mheard_render` (15 cases, 80-slot case added after a slot-0 mutation escaped). | **DECIDED 2026-09-12 (`DR-28` both-wrong, `DR-29` both-valid).** Part 1: all three renderers (`showMHeard()`, `sendMheard()`, `showMHeardTDECK()`) render most-recent-first by sorting a local `uint8_t` index array over `mheardMillis[]`; the slot-parallel storage arrays and the persisted image are never permuted (an in-place sort would corrupt eight arrays raced by `updateMheard()` from the LORA task). Part 2: `PLT` stays the numeric wire byte; MCProxy fails closed on a non-numeric value. Consumers key on `CALL`, so the order change is display-only; seen at H11, not at any G1-baselined step |

#### NCON-01 remote console on RAK-90 (filed 2026-09-12 out of `DR-11`)

| ID      | Type | Sev. | Location                                         | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Status                                                                                                                        |
| ------- | ---- | ---- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| NCON-01 | GAP  | Low  | `src/net_console.{h,cpp}`, `src/nrf52/nrf_eth.h` | **The TCP 2323 debug console is ESP32-only; RAK-90 is the Ethernet gateway and is the node where a remote console would be worth most.** `DR-11` settled the matrix question (stays split, not drift). This row carries the feature question, and it is gated on fit BEFORE any port: **answer the budget question first, then decide.** Three ESP32 couplings have to be replaced, and one of them is the risk: `<WiFi.h>` + `WiFi.status()` (`net_console.cpp:27,348`) -> W5100S link state, cheap; `<esp_random.h>` TRNG for the nonce (`:36`) -> nRF52 has `NRF_RNG`, cheap; **`<mbedtls/md.h>` HMAC-SHA256 (`:32,175`)** -> the open one. mbedtls is free on ESP32 because ESP-IDF already links it; on an Adafruit nRF52 Arduino build it is not there, and pulling it in for one HMAC is the wrong shape. A self-contained SHA-256 + HMAC is ~3 kB and avoids the dependency entirely -- note this project has already been bitten by assuming an mbedtls feature is available on a platform (see the NimBLE CMAC finding). | **OPEN, fit question owed first.** Do not start the port before the budget line below is re-measured on the then-current tree |

**Measured headroom on `wiscore_rak4631`, 2026-09-12 build gate** (this is the
number the suspicion should be checked against, not replaced by):

| region | used              | free        |
| ------ | ----------------- | ----------- |
| Flash  | 666 448 / 815 104 | **148 656** |
| RAM    | 88 892 / 248 832  | **159 940** |

So the blunt "it will not fit" is **not supported by today's numbers** -- 148 kB
of flash is ample for a TCP console plus a standalone HMAC. The real risks are
(a) that 81.8 % flash is already high and upstream keeps adding, so the margin
is shrinking and is not ours alone to spend, and (b) that taking mbedtls rather
than a small HMAC would turn a ~5 kB feature into a tens-of-kB one. Both are
arguments about **how** to build it, not whether it fits. Re-measure before
starting: this line ages with every upstream sync.

#### Fable review of the M2 decisions, 2026-09-12: four premises corrected, one live defect found

Verdict: `docs/testplan/drift-matrix-review-verdict-20260912.md` (17 findings, every
load-bearing claim re-derived against the tree). Row facts held on 26 of 29 rows; four
decisions rested on a wrong premise and were re-decided by the operator the same day
(recorded in the CSV under `RE-DECIDED 2026-09-12`):

- **`DR-24`/`DR-21` ring policy narrowed to parity.** ESP32 never wedged: `WiFiUDP::write()`
  only buffers, the real result is `endPacket()`, logged and discarded. Both platforms drop
  today. New shape: ESP32 keys on `endPacket()`, counts, resets, drops -- nRF52's shape. No
  retry counter on either side. `DR-21..26` are one change to `udp_drain_{esp32,nrf52}.cpp`
  and ride in W6 (the U2 drain had no wave).
- **`DR-18` withdrawn as written.** `sendExtern()` has no 0x41 branch, so the four-type gate
  shipped nothing. Now: shape parity (nRF52 adopts gate-then-forward, three types) plus the
  section 6.3 JSON ack from `docs/ack-wer-hat-quittiert.md` via `queueExtern()` at the
  BLE-ack sites, which MCProxy has parsed since 2026-09-05.
- **`DR-03` diagnostic port corrected and given a home.** Reads `s_ethLinkState`, not
  `hasIPaddress`; keeps the `[UDP] Heartbeat timeout` tag `serial_monitor.py` matches;
  keyed `hb_age_s`; raw `Serial.printf`; own mini-wave **C4-diag** after W6 with `DR-14`'s
  comment and `DR-16`, exempt from the `D1-09` soak because no recovery action changes.
- **`DR-26` both-wrong:** info gates, via decorates, on both drains.
- **`DR-20`:** the ESP32 mirror of nRF52's reset site is `getMeshComUDP()`, not the gateway loop.
- **`DR-28` spec** reworded to an index-array sort over three renderers.
- **Testplan 9.3 rewritten** to the `D1-04` target; the nRF52 BLE settings characteristic
  (raw struct bytes) joins the byte-compare gate as a frozen contract; 9.4 records that the
  20260724 blob needs a frozen copy of today's layout in the image.
- **Gate tooling:** `drift_matrix_lint.py` now checks `verdict` against `after_expect`,
  forbids `tie_break` off `both-valid`, and requires every moving row to be named in
  `test/golden/hw/G2/EXPECTED-DIFF.md` (seeded 2026-09-12). Still owed under `M3`: the
  `<uut>-twin-diff-before.txt` / `-after-expected.txt` fixtures for U1/U2 (none exist), the
...
planned. They are planned here, because "named in a bullet" is how an owed item
becomes a forgotten one.

| ID    | Type | Sev.   | Location                                                                                  | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----- | ---- | ------ | ----------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| M3-01 | GAP  | High   | `test/test_udp_frame_twin/`, `test/test_udp_send_twin/`, new suites                       | **10 of 29 decided rows name no asserting test, so 10 decisions are recorded but unasserted.** `drift_matrix_lint.py --phase implementation` reports the count on every run and must stop being passed once they exist. They do NOT split evenly, and that is the plan: **(a) four land in a suite that already exists** -- `DR-06` (ESP32 must gate on the `decodeAPRS()` return: feed a frame it rejects, `rsize<16` is cheapest), `DR-08` (CONF from source `0.0.0.0` while the resolved server is still `0.0.0.0`, both sides must reject), `DR-09` (drive an `:ack` payload through both handlers, compare the emitted BLE frame length AND attribution bytes) into `test_udp_frame_twin`, and `DR-21` (no drain and no ring advance while the link is down) into `test_udp_send_twin`. **(b) two need a suite that does not exist** -- `DR-12`/`DR-13` need the `U4` settings work (`test_settings_layout`, and a static gate for "in `X()` but missing from `esp32_flash.cpp`", which is cheap and should exist BEFORE the migration, not after). **(c) four have no native suite at all and cannot get one cheaply** -- `DR-03`, `DR-14`, `DR-15`, `DR-16` live in `gatewayService_*()` and the main loops, which never compile on a host. `C4` was carved but never characterised, and `D1-10` is not carved at all. For these the honest options are a `C4` twin (a fifth carve) or a bench assertion, and the choice must be made explicitly rather than by letting the rows stay blank                                                                                                                                                                                                                                                                                                                                                                           | **DECIDED 2026-09-12 for group (c): BUILD THE `C4` TWIN.** The recon recommended bench-only for all four rows on the grounds that `DR-14`/`DR-15` are documented non-changes with no correctness claim to assert. Operator overrode that, and the override is right for the reason the recon did not weigh: the risk on `DR-14` is not incorrectness, it is **reintroduction**. Two independent readers have now read that code as a bug and proposed "fixing" it -- the original audit (`OPT-D5`) and the M2 matrix row -- and a source comment has failed to prevent it twice. A call-count parity test is the instrument that does not depend on someone reading a comment first. Scope: `U10` `test_gateway_service_twin`, asserting (a) ESP32 calls `getMeshComUDP()` then `sendMeshComUDP()` on every gateway pass while nRF52 calls `sendUDP()` only on a pass where `getUDP()` reported no packet (`DR-14`), and (b) both platforms enter recovery on link-down and both take no recovery action on link-up-but-silent (`DR-15`). Cost measured by the recon: ~8 stubs, 4 reusable from `test_udp_frame_twin`. `DR-03` gains a home for its decision-logic parity as a side effect, though its PHY-link claim stays bench-only. **`DR-16` is NOT covered** -- it lives in the main loops, not the carved gateway service, so it needs `D1-10` or stays bench; that is a separate answer and must not be averaged in. **PARTLY CLOSED 2026-09-12: 10 -> 8.** `DR-14` and `DR-15` now name real cases in the `U10` twin (`test/test_gateway_service_twin`, env `native_gateway_twin`, 7 cases, mutation-verified). Remaining 8: `DR-03` (its decision-logic parity could ride in `U10`, but its actual claim is PHY link state and stays bench), `DR-06`/`DR-08`/`DR-09` (existing suite, ride with their wave), `DR-12`/`DR-13` (need the `U4` settings work, ride with `W3`), `DR-16` (main loops, needs `D1-10` or stays bench), `DR-21` (`test_udp_send_twin`, rides with the W6 drain change). **Still blocks dropping `--phase implementation`.** (a) rides with the wave that implements each row; (b) rides with `W3`; (c) needs a decision before `W1`, because "no suite exists" is not the same answer as "no test is owed" |
| M3-02 | GAP  | Medium | `test/golden/hw/G2/EXPECTED-DIFF.md`, `test/golden/compare_extudp.py`                     | **20 rows have a non-`identical` `after_expect`, so 20 rows can move a capture.** The file is seeded with all 20 and the lint now fails if a moving row is missing from it -- that half is done. What is not: the predicted diffs are named but not yet _written as predictions a tool can check_. Owed before `G2`: (1) `compare_extudp.py` must learn the section 6.3 `"type":"ack"` line, or `DR-18`'s one predicted diff reads as a failure; (2) **BLOCKED, established 2026-09-12: the plan's twin-diff fixtures cannot be generated from the suites as written.** §4.4 specifies `u1-esp32.txt`/`u1-nrf52.txt` -- an ordered sink-call sequence per corpus datagram, one file per platform -- and the before-diff between them. Neither twin produces that: `test_udp_frame_twin.cpp` and `test_udp_send_twin.cpp` have no corpus loop and no sink-sequence dump; each is a fixed set of named Unity cases that hand-builds one frame and asserts inline, with the drift pinned as hardcoded expected values in C++ rather than as data. Making the real fixture possible means rewriting both suites to iterate `test/golden/corpus/` and record an ordered per-platform call list -- a suite rewrite, not a fixture generation, and it must be budgeted as one. What exists instead is `test/golden/native/u{1,2}-test-roster.txt`: the ordered case-name/status listing, which catches a deleted or renamed case and nothing more. **It was deliberately NOT left at the name §9.1 reserves** -- a file sitting at `<uut>-twin-diff-before.txt` is what a script would pick up, and G2 would have compared against a test roster and looked satisfied; (3) the 12 `asserting_test` cases that currently PIN drift must flip to agreement cases as each row is implemented -- a drift test that still passes after the drift is removed is asserting the wrong thing | **PARTLY DONE 2026-09-12** (seeding + lint rule). The three items above are owed before `G2` and the third is owed per-wave, not once                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| M3-03 | GAP  | Low    | `docs/architecture/11-wire-format.md`, `src/udp_frame.h`, `docs/ack-wer-hat-quittiert.md` | **Three decisions were settled AS documentation and the documentation is not written.** Checked 2026-09-12, so the status is precise rather than assumed: `11-wire-format.md:62` already _describes_ `0x00`/`0x7E` being read back as `'#'`, and `:611` already calls `PLT` a "payload type byte" -- but neither states the **decision** (`DR-27`: `0x7E` is a reserved, illegal sub-version value; `DR-29`: raw byte in JSON and decoded text in the display is deliberate, do not "fix" it). The EXTUDP outbound type contract (`DR-18`) is written nowhere. Also owed at implementation time: `src/udp_frame.h:29-42` still advertises its four differences as open drift-matrix questions when all four are now decided, and the ack paper carries a wrong reason at line 266 (Fable verdict)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | **OPEN.** Cheap, and the reason to do it is that each of these three was arguable _because_ nothing was written down                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

| ID     | Type | Sev. | Location                                   | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------ | ---- | ---- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| EXT-01 | BUG  | High | `src/extudp_functions.cpp:629-665,685-687` | **A `0x3A` frame addressed to the telemetry path `100001` resets the EXTUDP socket.** The `if(msg_destination_path != "100001")` block builds no JSON for it, the `else return;` at `:664` belongs to the outer type chain, so control reaches `UdpExtern.write(c_json, 0)` with an empty buffer; `write()` returns 0 and `resetExternUDP()` tears the socket down and clears `hasExternIPaddress`. Reachable from `queueExtern("lora")` for every heard telemetry text frame on an `--extudp on` node. Found by the Fable review of `DR-18` (verdict Finding 4); code-read only | **CONFIRMED ON HARDWARE 2026-09-12, and the impact is smaller than this row claimed** -- `docs/bench/w3-baseline/README.md` section 2. Three frames injected into Heltec-93 (`--extudp on`, built from corpus `f006` with fresh ids; mesh and gateway off, nothing on air): a `9999` control produced `[EXT] Out: ... Len: 153` and a datagram; the `100001` frame produced **`[EXT] Out:  Len: 0`**, no datagram, and the socket re-announcing itself -- the predicted mechanism exactly. **But EXTUDP is not left dead:** `resetExternUDP()` clears `hasExternIPaddress` and then calls `startExternUDP()` again under `bEXTUDP && strlen(node_extern) > 7`, which is the same condition required to reach the bug, so the re-establish always fires. A third frame sent straight after went out normally (`Len: 151`). The real cost is therefore **a UDP socket teardown and rebuild per telemetry text frame heard** -- continuous churn on a gateway, and a `stop()`/`begin()` window on a port the node also listens on -- not the export outage the original wording implies. Fix is unchanged (early return before the send block) and the priority argument now rests on churn, not on data loss. nRF52 parity is code-read only: `c_json` is `static` there but memset at function entry, so it reaches `write(..., 0)` identically; RAK-90 was deliberately NOT reflashed because it is the article for the `W3` upgrade proof. ORIGINAL PLAN: inject a `100001` text frame on Heltec-93 with `--extudp on`. **The exact marker to watch for is `[EXT] Out:  Len: 0`** (`:673`, printed with an empty `c_json` immediately before the `write(..., 0)` that returns 0), followed by the socket reset -- re-verified at the source 2026-09-12, so the bench run is confirming a read mechanism, not searching for one. If confirmed, W6 adds an early return before the send block with a twin case. **Sequencing:** fix this before or with `DR-18`'s section 6.3 ack datagram, not after -- that work adds emissions into a send block that can currently tear down its own socket |
...
**M2 happened the same day** (`7dfbf246`, `07478ff2`): all 29 rows carry a verdict, and the
Fable review section above records what the review changed. The paragraph above describes
the pre-M2 state and is kept as history.
...
| TD-18 | BUG  | High   | `tools/bench/tdeck_harness.py` | `scenario_input` reported **PASS with keys=7/7** on 2026-09-11 19:19 on a node whose keyboard was completely dead to the operator -- the TD-19 lock was on at the time. It counts `[KEY];<hex>;` lines, printed near the top of `keypad_read()` (`src/t-deck/tdeck_main.cpp:785/803`), while the lock drops the key at the bottom of the same function by forcing `LV_INDEV_STATE_REL` (`:1161`); the latency probe's repaint is supplied by the clock and the status bar. Nothing in the serial stream separates "consumed by LVGL" from "logged and dropped". Fix: the scenario reads `--info` first and refuses to run unless `KEYLOCK off`. The trackball half is sound -- `mouse_read()` is not gated by the lock. | **FIXED 2026-09-11** (precondition); the automated `input` result in the H11 checklist is void and re-run is owed |
...
| -------------------- | ------------------------------------------------------------------ | ----------------------------------------------------- |
| `fork-main`          | fork main, **0 behind `upstream/dev` `6edc7499`** (firmware 4.35t) | keep — carries docs, `test/`, `tools/`, bench harness |
| `master`, `gh-pages` | fork mirror / pages                                                | ignore                                                |

No topic branch and no worktree is open. `pr-1114-kiss` was deleted 2026-09-04 after upstream
merged and reverted PR #1114 the same day; `v4.35p_prio` is the former name of `fork-main` and
survives only as the tag `archive/v4.35p_prio-20260903`.

The last upstream sync is `0ba9f063` (merge of `upstream/dev` at `6edc7499`, which includes PRs
#1136 and #1137). Our own PR #1135 was cut against `674413ce` — that SHA is the PR base, not the
sync point; earlier notes that name it as the sync point are wrong.

### 4.2-history Branches as of 2026-09-03 (historical)
...
Complete index of `docs/`, rebuilt 2026-09-11. Everything not listed here is closed and lives in
[`archive/`](archive/README.md) with a status box saying why — see [`archive/README.md`](archive/README.md)
for the rules of that directory. `RESUME.md` is the hand-over and carries the ordered open list;
this file carries the item rows.
...
`review/2026-07-31/` holds the nine unedited reports from the review that produced
`08`, `09` and `10` — eight independent finder angles plus the reconciliation against
`code-audit-20260712.md`. They are archived so every claim in the distilled documents stays
...
| Every `/X=` position key, who parses it, who loses it           | `aprs-parser-drift-20260911.md`                       |
...
| `N-25` (closed, kept)         | `bug-N25-gps-baud-scan-watchdog.md`                                                                                                                               |
| `MEM-04`                      | `mem-headroom-classic-esp32-20260905.md`                                                                                                                          |
| `SL-01`..`07` (bench owed)    | `setlog-instrumentation-impl-plan-20260902.md`                                                                                                                    |
| `TD-15`                       | `tdeck-map-empty-after-reboot-rca-20260905.md`                                                                                                                    |
| `TD-09`..`TD-11`              | `tdeck-findings-20260828.md`, `tdeck-gui-verdict.md`, `review-tdeck-gui-20260828.md`, `tdeck-baseline-20260828.md`, `tdeck-lvgl-agent-guide.md`, `lvgl-research/` |
| deepsleep follow-ups          | `gpio-hold-and-hwcdc-followups.md`                                                                                                                                |
| `DM-01`..`DM-06`              | `proposal-dm-transport-reliability-20260909.md`, `konzept-dm-transportsicherung-20260909.html`                                                                    |
| `OPT-01`..`04`, `OPT-D1..D13` | `optimization-audit-20260910.md` (+ `-appendix.md`), `testplan-dry-unification-20260910.md`, `dry-unification-gantt-20260910.html`                                |
| `CQ-02`..`CQ-12`              | `code-quality-2.0.md`, `codequality-rules.md`                                                                                                                     |
| `WEB-03` (c)–(e)              | `finding-webgui-passwort.md`                                                                                                                                      |
...
| GPS and I2C sensor bench                    | `gps-sensor-bench-20260822.md` |
...
| Earlier audits and their fixes                            | `code-audit-20260508.md`, `code-audit-20260626.md`, `code-audit-fixes-20260627.md`     |
| WiFi: driver analysis, fix report, night soak             | `wifi-findings-20260829.md`, `wifi-report-20260830.md`, `wifi-soak-report-20260831.md` |
| HEY storms and Trickle-HEY suppression                    | `hey-storm-analysis-20260827.md`, `hey-supp.md`                                        |
| BLE: inventory and TX latency                             | `ble-review.md`, `report-ble-tx-latency.md`, `NimBLE.md`                               |
| Ping on the T-Beam 1W (track mode, not a board defect)    | `ping-fix.md`                                                                          |
| OE3 mountain nodes / adaptive relay slot (German, HTML)   | `report-2026-09-02-oe3-bergknoten.htm`, `report-2026-09-02-adaptiver-relay-slot.htm`   |
| All 355 upstream issues, harvested                        | `upstream-issue-harvest-20260829.md`                                                   |
| Review of upstream PR #1114 (KISS/TCP, merged + reverted) | `pr1114-kiss-review-20260901.md`                                                       |
| T-Deck map code analysis (German, HTML)                   | `tdeck-karte-codeanalyse.html`                                                         |

### 5.6 Proposals and designs not yet decided

| Subject                                                        | Document                                        |
| -------------------------------------------------------------- | ----------------------------------------------- |
| DM transport reliability — the line being pursued              | `proposal-dm-transport-reliability-20260909.md` |
| DM store-and-forward outbox — the alternative, not pursued     | `concept-dm-store-and-forward.md`               |
...
| ~~Existing German design docs (`docs/README_LORA_TRX.md`, `docs/adr-*.md`, `docs/prio-talk-flood-networking.md`, ~180 KB) are not linked from this set.~~ **Closed 2026-09-11:** §5 is now a complete index of `docs/`, and closed documents moved to `docs/archive/`. | —                                                                   |
