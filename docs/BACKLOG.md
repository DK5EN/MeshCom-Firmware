...
Runde live bestaetigt (`[PONG] from:DK5EN-14 rssi:-26 snr:7`); RAK4631 3/4, dort dafuer
der zweite Rueckgabepfad `PING_RING_REFUSED` live belegt, den beide ESP32-Boards nie
ausgeloest haben. Bericht: `docs/ping-bench-20260913.md`.
...
| PING-04 | TEST | Mid  | RAK4631 am Bench                         | Der Positivpfad (TRACK off -> `TX_GATE_ENTER`) ist auf dem RAK nicht nachgewiesen: der Knoten traegt das Werks-Rufzeichen `XX0XXX-00`, `addTxRingEntry()` weist damit unbedingt ab (`txring_functions.cpp:449`), bevor TRACK ueberhaupt zaehlt. Braucht `--setcall` -- Betreiber-Entscheidung, der Knoten geht damit unter dieser Identitaet auf die Luft. Ebenso offen: `ec070235` (Marker-Paritaet) ist auf Hardware unbestaetigt, auf dem Knoten laeuft `c570e62e`. | offen                                                                                                                                                                                         |
