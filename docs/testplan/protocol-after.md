# Protocol: after-run (G2)

Template for section 8 of [`testplan-dry-unification-20260910.md`](../testplan-dry-unification-20260910.md).
**Do not edit the row set when filling this in.** Rows are pre-seeded so that the after-run only fills
...
- **Compared to** names the artifact this cell was diffed against -- for the after-run this is always the corresponding
  `G1` artifact. An after-run cell compared to anything else is not a `G2` result.
