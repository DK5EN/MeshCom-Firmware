...
| 4    | `D1-10` loop scheduler                                            | in flight, one writer: shared table + runner in `src/loop_scheduler.cpp`, verbatim action bodies per platform, twin env `native_loop_scheduler`; ~14 of 26 timers in scope, the first-fire/absolute ones stay |
