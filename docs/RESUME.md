...
| 3    | `W7-II(a)` presence flags, `W7-II(b)` `extends=` in variant inis  | in flight, two writers, disjoint files; gates: `variant_macros_effective.py` byte-identical, `variant_ini_effective.py` |
| 4    | `D1-10` loop scheduler                                            | scouting; carve + `native_loop_scheduler` twin                                                                          |
| 5    | `E1` G2 after-run on the FINAL image: `H4`, `H6`, `H11`, `ETH-02` | last, because `DR-28` moves the T-Deck MHeard screen and `H4`/`H6` must capture the end state                           |
...
| `R3-11`/`D2-09`  | **DONE** -- one switch `MC_DIAG` (`configuration_global.h`), default 1, `E22_XML` sets 0. NOT behind `INSTRUMENT_ENABLED`: that is 0 in every env, so the recorded plan would have deleted five commands from all 34 images                                                                                                                                                                                                                                                                                                               |
| `R3-03`          | **DONE** -- eight variants, not one, carried the invalid `OneWire_GPIO 99`; all now `-1`. The real defect was the macro fallback skipping the validity test that `--owgpio` applies                                                                                                                                                                                                                                                                                                                                                       |
| `W5` `GRD-01`    | **DONE** (`46ed0f3c`) -- one `MC_HAS_U8G2` replaces seven hand-written copies of "does this board drive an OLED", plus a nine-arm cascade collapsed to three. Adding an OLED-less board is one line now                                                                                                                                                                                                                                                                                                                                   |
| `W5` `R1-02`     | **CLOSED at step 1.** The duplicated three-way phone-framing switch is one host-tested `blePhoneFrame()` (-976 B flash, 32 envs). Step 2 dropped: the two rings are two QUEUES with different priority and pacing, not duplicated storage                                                                                                                                                                                                                                                                                                 |
| `W5` `R2-01`     | **DONE** -- the mheard text codec is gone: three copies of a pipe-delimited parse replaced by a 20 B struct. **-83 200 B RAM and -54 676 B flash** across 34 envs, -117 lines. 10 host cases in `native_mheard_record`                                                                                                                                                                                                                                                                                                                    |
| `DISP-01`        | **RESOLVED on the bench** -- the audit blamed the ESP32 branch; two boards (Heltec V3, T-Beam) prove it CORRECT. The probe's two log labels were swapped, which misled the row. nRF52 was the inverted copy; the `idtype->u8g2` mapping is now ONE function (`mcSelectU8g2()`), so it cannot drift again. nRF52 half is code-proven only -- the bench RAK has no OLED. `BOARD_TBEAM_1W` hardwired value still open                                                                                                                        |
| `W6a`            | **DONE** (`55a7b4c4`) -- nine decided U1 drift rows unified, twin diff **24->36 agree, 13->1 differ, 0 regressions**; plus `src/ui_common/` for the two LVGL twins (`peri_gps` 417->271 / 387->312, two empty TUs deleted). Advisor pass ran: 4 findings fixed before commit. **`ui_common` is build-verified only** -- neither LVGL board runs here                                                                                                                                                                                      |
| `W6b`            | **DONE** (`3be9a9da`) -- `DR-20`, the six U2 rows and `DR-18` part 2. **Every drift-matrix row a source change can close is now closed**; only `DR-03`/`DR-16` remain, both waiting on `E1`. Advisor caught a real cross-task race (`queueExternAck()` writing the single-producer `externQueue[]` from the main loop while `OnRxDone` writes it from the LORA task) -- fixed by sending directly, since `flushExternQueue()` is in the same task anyway. 5 further findings fixed. U2's proof is hand-built tests, not corpus arithmetic |
| `W5` `R4-01`     | **DONE** -- `config/lv_conf.h` was the fifth `lv_conf.h` and the only one still on `LV_MEM_CUSTOM 0`; LVGL's 48 kB arena was a STATIC array, so internal DRAM by construction. `t_deck_pro` RAM **-49 168 B** (55.2 % -> 40.2 %), flash -1 440. Build+`nm` only: no T-Deck Pro on the bench                                                                                                                                                                                                                                               |
| `W5` `R2-04`     | **DONE** (code) -- `aprsMessage`'s seven `String` fields are fixed `char[]`; 27 src files, 15 suites, 651 usages. **Flash -109 068 B**, RAM +15 024 (of which +464/env is the one global `pendingDisplayMsg`). New `src/mc_text.h` + `native_mc_text`. Heap claim still needs the 1 h `--heap` bench watermark                                                                                                                                                                                                                            |
| `W5` `R3-12`     | **DONE** -- `mheardLat/Lon` `double`->`float`, -648/-640/-400/-240 B per env by `MAX_MHEARD`. Precision measured: 0.85 m worst case, 0.15 % of coordinates render a different last digit at the GUI's `%.3f`                                                                                                                                                                                                                                                                                                                              |
| `W5` `R3-13`     | **SKIPPED** -- row 52->40 B frees 1 200 B but truncates 6-hop relay paths in `--path`; `--maxhop` allows 6. Keeping 6 hops means row 50, worth 200 B                                                                                                                                                                                                                                                                                                                                                                                      |
| `W5` `R4-02/03`  | **CLOSED.** `GRD-01` delivered the "one code path, no board list" part. The dynamic-buffer half is dropped by operator decision: ~1 400 B measured, against an alloc-failure path on 20+ boards                                                                                                                                                                                                                                                                                                                                           |
...
| `W5` all rows    | **unblocked 2026-09-16** -- all operator decisions are made. `R4-02/03` is closed, `GRD-01` and `R1-02` step 1 are committed. Still needs a 12 h nRF52 soak for `R1-04` and a BLE golden for `R1-02` step 2                                                                                                                                                                                                                                                                                                                               |
