...
| `W6b`            | **NEXT** -- `DR-20` (needs `udp_functions.{h,cpp}`: the caller must own connectivity policy), `DR-18` part 2 (JSON ack + the per-type test that row ordered + 3 doc updates), and the U2 rows `DR-21`..`DR-26` incl. the `stat_tx_leak_unconfigured` counter split `DR-25` requires on BOTH platforms                                                |
