...
## ORCHESTRATOR CORRECTION (verified against platformio.ini:108-120 and :518-532)
...
| loop_functions.cpp:289 (WP_DISP)      | `dzeile`                                        | `int[maxdisplines]`                | 7 × 4              | 4            | **28**               | 28                    | 28            |
| loop_functions.cpp:494                | `pageLastLine`                                  | `int[PAGE_MAX][maxdisplines][3]`   | 10×7×3 = 210 × 4   | 4            | **840**              | 840                   | 840           |
| loop_functions.cpp:495                | `pageLastText`                                  | `char[PAGE_MAX][maxdisplines][25]` | 10×7×25 = 1750 × 1 | 1            | **1750**             | 1750                  | 1750          |
| loop_functions.cpp:496                | `pageLastTextLong1`                             | `char[PAGE_MAX][25]`               | 10×25 = 250 × 1    | 1            | **250**              | 250                   | 250           |
| loop_functions.cpp:497                | `pageLastTextLong2`                             | `char[PAGE_MAX][200]`              | 10×200 = 2000 × 1  | 1            | **2000**             | 2000                  | 2000          |
...
| loop_functions.cpp:215,220–224        | `strSOFTSER_BUF`, `strSOFTSERAPP_*` (7 Strings) | String[7]                          | 7 × 16             | 16           | **112+heap**         | 112+heap              | 112+heap      |
| loop_functions.cpp:225                | `strTelemetry`                                  | String                             | 1 × 16             | 16           | **16+heap**          | 16+heap               | 16+heap       |
| loop_functions.cpp:251–252            | `strTime`, `strDate`, `str`                     | String[3]                          | 3 × 16             | 16           | **48+heap**          | 48+heap               | 48+heap       |
| loop_functions.cpp:256                | `strText`                                       | `char[600]`                        | 600 × 1            | 1            | **600**              | 600                   | 600           |
...
| --------------------------------- | ------------------------------------------------------------- | ----------------------------------- | ------------ |
| loop_functions.cpp:313 (WP_DISP)  | `wpMsgFont()::f`                                              | static GFXfont                      | ~48          |
| loop_functions.cpp:314 (WP_DISP)  | `wpMsgFont()::ready`                                          | static bool                         | 1            |
...
| esp32_flash.h:143 | s_meshcom_settings | node_lora_call[10] | 10             | 1                | 10          | (not found, storage only)               | strcmp (loop_functions.cpp:L~1108) |
...
| WisBlock-API.h:305 | s_meshcom_settings | node_lora_call[10] | 10             | 1                | 10          | memcpy (nrf52_flash.cpp:L~62)           | strcmp (loop_functions.cpp:L~1108) |
...
### mheardLine (aprs_structures.h:75–86)

| File:Line            | Container         | Field              | Type   | Write Method                            | Compare Method |
| -------------------- | ----------------- | ------------------ | ------ | --------------------------------------- | -------------- |
| aprs_structures.h:77 | struct mheardLine | mh_callsign        | String | = operator (mheard_functions.cpp:L~180) | == operator    |
| aprs_structures.h:80 | struct mheardLine | mh_sourcecallsign  | String | = operator                              | == operator    |
| aprs_structures.h:81 | struct mheardLine | mh_sourcepath      | String | = operator                              | indexOf        |
| aprs_structures.h:82 | struct mheardLine | mh_destinationpath | String | = operator                              | == operator    |
...
| aprs_structures.h:77, 80–75  | mheardLine.mh_*                                        | String             | Per mheard update: allocated, then memcpy'd to mheardBuffer/mheardCalls, destroyed |
...
- **DRY-21 (nrf_eth.cpp UDP-RX handler vs udp_functions.cpp):** still OPEN. `nrf_eth.cpp:585` sets `print_buff[5]=0x01` for the first ACK branch (and `0x02` later at line 596 for the ACK-of-DM case), while `udp_functions.cpp:419` only logs `print_buff[5]` without the earlier divergent-assignment pattern being visibly unified — the two paths were not merged into one shared handler. Treat as still-open duplication; needs a closer diff before reproposing the exact divergence claim, but the shared-handler refactor was not done.
...
- **Net console protocol (port 2323):** `src/net_console.cpp` (header comment line 2: "Port 2323,
  ...
  `src/command_functions.cpp:818,2453,2472,5985` (`--netconsole on/off`). Client tool:
...
| src/loop_functions.cpp:583 | `addBLEOutBuffer()` | OUT       | `BLEtoPhoneBuff[MAX_RING][MAX_MSG_LEN_PHONE+5]` (20 slots × 305 bytes)    | Slot [0]: length, [1]: status byte (0x91, 0x44, or frame type), [2+]: payload. MTU 245 limit enforced at line 365 (config_global.h). |
| src/loop_functions.cpp:583 | `addBLEOutBuffer()` | OUT       | `BLEComToPhoneBuff[MAX_RING][MAX_MSG_LEN_PHONE+5]` (20 slots × 305 bytes) | Same structure                                                                                                                       |
...
| 10   | 12     | 8     | 88    | src/aprs_functions.cpp:718-736; src/aprs_functions.cpp:746-764; src/aprs_functions.cpp:774-792; src/aprs_functions.cpp:802-820; src/aprs_functions.cpp:830-848; ... (+7 more)                                         |
...
| 13   | 10     | 9     | 81    | src/aprs_functions.cpp:723-743; src/aprs_functions.cpp:751-771; src/aprs_functions.cpp:779-799; src/aprs_functions.cpp:807-827; src/aprs_functions.cpp:835-855; ... (+5 more)                                         |
...
| 18   | 11     | 8     | 80    | src/aprs_functions.cpp:721-738; src/aprs_functions.cpp:749-766; src/aprs_functions.cpp:777-794; src/aprs_functions.cpp:805-822; src/aprs_functions.cpp:833-850; ... (+6 more)                                         |
...
| 25   | 4      | 21    | 63    | src/loop_functions.cpp:1337-1364; src/loop_functions.cpp:1430-1457; src/loop_functions.cpp:1540-1567; src/loop_functions.cpp:2780-2807                                                                                |
...
| 29   | 7      | 9     | 54    | src/loop_functions.cpp:3327-3341; src/loop_functions.cpp:3404-3418; src/loop_functions.cpp:4052-4066; src/loop_functions.cpp:4741-4755; src/loop_functions.cpp:4818-4832; ... (+2 more)                               |
...
| 35   | 3      | 24    | 48    | src/loop_functions.cpp:1426-1460; src/loop_functions.cpp:1536-1570; src/loop_functions.cpp:2776-2811                                                                                                                  |
...
| 37   | 6      | 9     | 45    | src/loop_functions.cpp:5694-5702; src/loop_functions.cpp:5697-5705; src/loop_functions.cpp:5700-5708; src/loop_functions.cpp:5703-5711; src/loop_functions.cpp:5706-5714; ... (+1 more)                               |
| 38   | 5      | 10    | 40    | src/command_functions.cpp:3947-3963; src/command_functions.cpp:3990-4006; src/command_functions.cpp:4015-4031; src/command_functions.cpp:4041-4057; src/command_functions.cpp:4067-4083                               |
| 39   | 5      | 10    | 40    | src/loop_functions.cpp:5696-5705; src/loop_functions.cpp:5699-5708; src/loop_functions.cpp:5702-5711; src/loop_functions.cpp:5705-5714; src/loop_functions.cpp:5708-5717                                              |
...
| 44   | 5      | 9     | 36    | src/loop_functions.cpp:5649-5657; src/loop_functions.cpp:5652-5660; src/loop_functions.cpp:5655-5663; src/loop_functions.cpp:5658-5666; src/loop_functions.cpp:5661-5669                                              |
...
| 49   | 4      | 10    | 30    | src/loop_functions.cpp:5651-5660; src/loop_functions.cpp:5654-5663; src/loop_functions.cpp:5657-5666; src/loop_functions.cpp:5660-5669                                                                                |
...
| 53   | 4      | 9     | 27    | src/command_functions.cpp:558-569; src/command_functions.cpp:3609-3620; src/command_functions.cpp:3641-3652; src/command_functions.cpp:3667-3678                                                                      |
...
| 56   | 2      | 25    | 25    | src/loop_functions.cpp:1425-1460; src/loop_functions.cpp:1535-1570                                                                                                                                                    |
...
| 59   | 4      | 8     | 24    | src/command_functions.cpp:3954-3965; src/command_functions.cpp:4022-4033; src/command_functions.cpp:4048-4059; src/command_functions.cpp:4074-4085                                                                    |
...
| 5    | 16     | 12    | 180   | src/loop_functions.cpp:124-136; src/loop_functions.cpp:125-137; src/loop_functions.cpp:126-138; src/loop_functions.cpp:128-139; src/loop_functions.cpp:129-140; ... (+11 more)                                                                 |
| 6    | 10     | 19    | 171   | src/aprs_functions.cpp:715-756; src/aprs_functions.cpp:743-784; src/aprs_functions.cpp:771-812; src/aprs_functions.cpp:799-840; src/aprs_functions.cpp:827-868; ... (+5 more)                                                                  |
...
| 13   | 12     | 12    | 132   | src/loop_functions.cpp:4243-4255; src/loop_functions.cpp:4245-4256; src/loop_functions.cpp:4246-4257; src/loop_functions.cpp:4247-4258; src/loop_functions.cpp:4248-4260; ... (+7 more)                                                        |
...
| 15   | 11     | 13    | 130   | src/aprs_functions.cpp:708-736; src/aprs_functions.cpp:736-764; src/aprs_functions.cpp:764-792; src/aprs_functions.cpp:792-820; src/aprs_functions.cpp:820-848; ... (+6 more)                                                                  |
...
| 17   | 10     | 14    | 126   | src/aprs_functions.cpp:705-736; src/aprs_functions.cpp:733-764; src/aprs_functions.cpp:761-792; src/aprs_functions.cpp:789-820; src/aprs_functions.cpp:817-848; ... (+5 more)                                                                  |
...
| 20   | 11     | 12    | 120   | src/aprs_functions.cpp:704-728; src/aprs_functions.cpp:732-756; src/aprs_functions.cpp:760-784; src/aprs_functions.cpp:788-812; src/aprs_functions.cpp:816-840; ... (+6 more)                                                                  |
| 21   | 11     | 12    | 120   | src/aprs_functions.cpp:712-738; src/aprs_functions.cpp:740-766; src/aprs_functions.cpp:768-794; src/aprs_functions.cpp:796-822; src/aprs_functions.cpp:824-850; ... (+6 more)                                                                  |
...
| 34   | 9      | 12    | 96    | src/loop_functions.cpp:4460-4471; src/loop_functions.cpp:4461-4472; src/loop_functions.cpp:4462-4473; src/loop_functions.cpp:4463-4474; src/loop_functions.cpp:4464-4475; ... (+4 more)                                                        |
...
| 40   | 6      | 18    | 90    | src/command_functions.cpp:2694-2727; src/command_functions.cpp:2726-2759; src/command_functions.cpp:2758-2791; src/command_functions.cpp:2808-2846; src/command_functions.cpp:2888-2921; ... (+1 more)                                         |
...
| ------------- | --------- | ------ |
| commandAction | 238-6205  | 5968   |

### `src/loop_functions.cpp`

| name                | start-end | length |
| ------------------- | --------- | ------ |
| sendDisplayText     | 2209-2723 | 515    |
| sendMessage         | 3772-4199 | 428    |
| sendDisplay1306     | 942-1306  | 365    |
| sendTelemetry       | 5045-5354 | 310    |
| sendDisplayPosition | 2763-3071 | 309    |
| sendPosition        | 4498-4795 | 298    |
| PositionToAPRS      | 4201-4496 | 296    |
| wpMsgFont           | 311-555   | 245    |
| mainStartTimeLoop   | 1970-2206 | 237    |
| setSMartBeaconing   | 5368-5572 | 205    |
...
| 3    | 36          | `else / if(commandCheck(msg_text+N, (char*)"S") == N) / snprintf(_owner_c, sizeof(_owner_c), "S", msg_text+N);` | 560, 1282, 1301, 1320, 1343, 1366, 1484, 1509, 2411, 2520, 3240, 3367, ...  |
...
| 8    | 25          | `if(ble) / addBLECommandBack((char*)"S"); / save_settings();`                                                   | 968, 986, 2701, 2717, 2733, 2749, 2765, 2781, 2799, 2820, 2836, 2854, ...   |
| 9    | 25          | `addBLECommandBack((char*)"S"); / save_settings(); / return;`                                                   | 970, 988, 2703, 2719, 2735, 2751, 2767, 2783, 2801, 2822, 2838, 2856, ...   |
...
| 14   | 17          | `#endif / return; / else`                                                                                       | 651, 705, 949, 1791, 1811, 3639, 3793, 3822, 3960, 4028, 4054, 4080, ...    |
| 15   | 16          | `#endif / else / if(commandCheck(msg_text+N, (char*)"S") == N)`                                                 | 752, 1009, 1036, 1059, 1188, 1852, 1883, 1912, 2964, 2988, 3354, 3917, ...  |
...
| --------------------- | ----------------------------- | --------------------------- | ------------------------------------------------------------------------- | --------------- |
| `BLEtoPhoneBuff`      | `loop_functions_extern.h:267` | `MAX_MSG_LEN_PHONE+5` = 305 | 1 len + 255 payload + 4 ts = **260** (clamp `loop_functions.cpp:587-590`) | **45**          |
| `BLEComToPhoneBuff`   | `loop_functions_extern.h:272` | 305                         | 1 len + 245 = **246** (clamp `loop_functions.cpp:652-657`)                | **59**          |
...
| `ringbufferRAWLoraRX` | `loop_functions_extern.h:255` | 260                         | `UDP_TX_BUF_SIZE-1` = **254** (`loop_functions.cpp:3236`)                 | 6               |
...
| ----- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ----- | ---- | ---- | ------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R1-01 | Size the two BLE slots to what the producers clamp to                                      | `loop_functions_extern.h:267,272`, `loop_functions.cpp:442,447`                                     | Replace `MAX_MSG_LEN_PHONE+5` by two new derived constants `BLE_TX_SLOT` (260) and `BLE_COM_SLOT` (246).                                                          | **2080 / 2080 / 2080**                                 | 0     | +6   | L    | none          | S      | `pio run -e <3 envs>` map diff; `--bledebug` frame dump byte-identical for a 255-B text, a 244-B JSON and an mheard frame                                    |
| R1-02 | Merge `BLEComToPhoneBuff` into `BLEtoPhoneBuff` with a routing tag                         | `loop_functions.cpp:442-451,581-677`, `phone_commands.cpp:60-210`, `web_functions.cpp:1848-1875`    | One 20-slot ring, byte 1 already carries the type (0x91/0x44/text); add a 1-byte `dest` column `bleSlotIsCom[MAX_RING]` so the web history skips command frames.  | **4980 / 4980 / 4980** (after R1-01)                   | −250  | −70  | M    | none          | M      | drain-order test on a live node: config reply + text burst interleaved; web rxlog shows text only                                                            |
| R1-03 | Trim `ringBufferUDPout` `+20` padding to `+1`                                              | `loop_functions_extern.h:260`, `loop_functions.cpp:437`                                             | `UDP_TX_BUF_SIZE+20` → `UDP_TX_BUF_SIZE+1`; the `+20` was reserve for the `len+1` overread already fixed by WF-01.                                                | **380 / 380 / 380**                                    | 0     | +2   | L    | none          | S      | `--udplog on`, compare UDP hex on the server for a 255-B frame; `sizeof(ringBufferUDPout[0])` uses at `udp_functions.cpp:673`, `nrf52_main.cpp:3029` recheck |
| R1-04 | `ringbufferRAWLoraRX` behind a runtime pointer, allocated on first web/rxlog use           | `loop_functions.cpp:429-431,3217-3237`, `lora_functions.cpp:697-709`, `web_functions.cpp:1307-1349` | Static array → `static unsigned char (*ringbufferRAWLoraRX)[260]`, `malloc` on the first `bWEBSERVER` rxlog request, writer no-ops on `NULL`.                     | **5200 / 2600 / 2600**                                 | +120  | +25  | M    | none          | M      | boot heap print with/without webserver; rxlog page content unchanged; nRF52 soak with webserver off for 12 h (no writer NULL-deref)                          |
...
`UDP_TX_BUF_SIZE-4` / `UDP_TX_BUF_SIZE` (`loop_functions.cpp:587-590`) and appends 4 timestamp bytes
...
(`web_functions.cpp:1848-1875`, whose slot-shortage is already noted at `:874`) keeps showing text
frames only. The merge is a net concurrency _improvement_: `addBLEOutBuffer()` runs its slot fill
under `taskENTER_CRITICAL()` and uses `addRingPointer()` (`loop_functions.cpp:595-629`), while
...
`web_functions.cpp:1347` (the rxlog page). Its single writer is `charBuffer_aprs()`
(`loop_functions.cpp:3217-3237`), called unconditionally from the RX path (`lora_functions.cpp:699`)
...
console table, the web card (`web_functions.cpp:1448`) and the `MH` JSON stay byte-identical.
...
`aprs_structures.h:20-26` has 7 `String`s per `aprsMessage` and `:77-83` 7 more per `mheardLine`;
...
1188,1237,1324-1330,1400`, `udp_functions.cpp:381-423`, `nrf_eth.cpp:554-607`). None of those
...
| ------------------------------------------------------ | --------------------------------------------------------------------- | -------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------ |
| `src/loop_functions.cpp:370-403` (via U8g2 lib)        | `u8g2_1`, `u8g2_2`                                                    | n/a (class instances)            | `esp32_functions.cpp:202,206`, `nrf52_functions.cpp:39,43`                          | 188 + 188 = 376 B                                                              | `.bss` (DRAM)                              |
| U8g2 lib internal (`u8g2_m_16_8_1`)                    | `buf$<n>` (shared, one instance)                                      | n/a                              | both `u8g2_1`/`u8g2_2` setup calls                                                  | 1024 B                                                                         | `.bss` (DRAM)                              |
| `.pio/libdeps/*/U8g2/src/clib/u8g2_fonts.c`            | `u8g2_font_6x10_mf`                                                   | yes                              | `loop_functions.cpp:978`, `esp32_functions.cpp:354`, `nrf52_functions.cpp:58,64`    | 2393 B (confirmed, `nm -S`)                                                    | `.rodata`→flash (DROM)                     |
| `.pio/libdeps/*/U8g2/src/clib/u8g2_fonts.c`            | `u8g2_font_10x20_mf`                                                  | yes                              | `loop_functions.cpp:3288,4653`, `esp32_functions.cpp:352`, `nrf52_functions.cpp:62` | 4194 B (confirmed, `nm -S`)                                                    | `.rodata`→flash (DROM)                     |
| same                                                   | `u8g2_font_6x10_tf`                                                   | yes (declared)                   | `loop_functions.cpp:1155,3282`, `esp32_functions.cpp:346`                           | not linked in measured elves                                                   | n/a — dead branch, see R4-06               |
| `src/loop_functions.cpp:776-780`                       | `pageLastLine`/`pageLastText`/`pageLastTextLong1`/`pageLastTextLong2` | no (mutable ring)                | ~15 call sites, `loop_functions.cpp:1024-2471`                                      | 504+1050+150+1200 = 2904 B (else-branch, `PAGE_MAX=6, maxdisplines=7`)         | `.bss` (DRAM)                              |
...
| ----- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1-01 | Shared GATE/CONF/BEAT frame handler                               | `src/udp_functions.cpp:205-638` (435 L) vs `src/nrf52/nrf_eth.cpp:367-839` (473 L); identical 189, divergent 530                                                                                                                                                                                                                                                                                       | Extract everything after "a datagram is in `inc_udp_buffer`" into new `src/udp_frame.cpp::handleMeshComUdpFrame(uint8_t*, int len, IPAddress src, const McNetOps&)`; both platforms keep only socket read + their own link-reset hook                                                                                   | ~350                                                                                 | **8** — (a) nRF52 zero-scan `for(i=0;i<packetSize;i+=2)` reads `buf[i+1]` past the datagram on odd sizes, ESP32 has `i+1<packetSize` (`nrf_eth.cpp:418` vs `udp_functions.cpp:214`); (b) RX-01 unconfigured-source guard ESP32-only (`udp_functions.cpp:287`) — an nRF52 gateway radiates XX0XXX GATE frames to LoRa; (c) `hb_warn_logged=false` set in all four ESP32 branches, absent on nRF52 → warn latch never clears; (d) `bGATEWAY_NOPOS` honoured ESP32-only (`udp_functions.cpp:339`); (e) `sendDisplayPosition()` + TM-31 early-dedup 0x21 branch ESP32-only → nRF52 shows no server position frames and can double-insert; (f) `decodeAPRS()` return value checked nRF52-only (`nrf_eth.cpp:483`) — ESP32 processes undecodable frames; (g) `hasExternIPaddress` gate on `sendExtern()` ESP32-only (`udp_functions.cpp:270`); (h) CONF guard has no zero-address check on nRF52 (`nrf_eth.cpp:734`) vs `node_hostip==0` on ESP32 (`udp_functions.cpp:541`) | −0.3 kB flash net (one copy of ~350 L); nRF52 loses a 260 B stack array (`nrf_eth.cpp:372`)                 | M                                                                | none                                                                                                                                                                       | L      | byte-compare of UDP 1799 TX and BLE out-frames before/after on RAK4631 + Heltec V3, driven by `tools/injectraw` EXTUDP recipe; `--udplog`/`[GW];rx;type;` line-for-line equal                         |
| D1-02 | Fold nRF52 hand-built ACK phone frame into `buildAckPhoneFrame()` | `src/nrf52/nrf_eth.cpp:578-596` vs `src/ack_attribution.h:122-137` (already used 6× in shared code)                                                                                                                                                                                                                                                                                                    | Replace the 7 literal `print_buff[n]=` assignments + `addBLEOutBuffer(...,7)` with the shared builder + its returned length                                                                                                                                                                                             | ~14                                                                                  | **1** — nRF52 emits a fixed 7-byte frame with `out[6]=0x00`, so the ACK-attribution callsign suffix (`ACK_PHONE_BASE_LEN + n`) that ESP32 sends since ack_attribution.h is never sent by an nRF52 gateway                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | none                                                                                                        | L                                                                | none (superset frame; length byte already 0 today)                                                                                                                         | S      | byte-compare BLE 0x41 frames on both platforms; assert identical bytes when `call==""`                                                                                                                |
...
- **`startNetwork()` (`src/udp_functions.cpp:1099-1376`) ↔ `NrfETH::startETH()`
  (`src/nrf52/nrf_eth.cpp:982-1082`).** Named in the C06 detector list, but there is no shared body
  ...
  (`nrf_eth.cpp:1045-1065`, `udp_functions.cpp` equivalent). Extract that as
...
- `commandAction(char*, bool)` — `src/command_functions.cpp:238-6205`, one function, 5935 lines.
...
- **297 distinct `if(commandCheck(...))` branch heads** (322 `commandCheck` call sites total; the
  extra 25 are second alternatives inside one head, e.g. `:3474` `"weather" || "wx"`, and
  re-checks inside a body, e.g. `:2076`, `:4763`, `:5171`).
- 76 of the 297 sit inside `#if INSTRUMENT_ENABLED` (`:4793`-`:5394`); ~221 are in a normal build,
...
bWifiSetting bNodeSetting bAnalogSetting bReturn`, :253-265) are set by branches and consumed by
  the printer/BLE-register tail at `:5530-6207`.
...
- Order-dependent pairs found (short name would swallow the long one if reordered):
  `heap ` :4801 / `heap` :4807 (already commented at :4798-4800) · `setlog on|off` :533,547 /
  `setlog ` :561 · `gpsdebug 2|on|0|off` :3058,3073 / `gpsdebug ` :3088 · `netconsole on|off`
  :2446,2458 / `netconsole` :2469 · `passwd ` :3368 / `passwd` :3396 · `maxhop ` :4461 / `maxhop`
  :4492 · `pingmax max` :3745 / `pingmax ` :3754 · `tab list` :4995 / `tab ` :5001 ·
  `instreset` :5250 / `instr` :5387 · `loradebug on|off` :2791,2846 / `lora` :5397 ·
  `sethamnet on`/`setinet off` :4086 / `setinet` :4106 · `posshot` :427, `postime ` :434 /
  `pos` :3434 · `mheard` / `mh` :5414 (same head).
- `vmsg[strlen(command)] = 0`: if `strlen(command) > strlen(msg)` this writes past the terminator
  ...
  command, which is what keeps `balledges` :5040 safe next to `balledge on` :5028.
...
| (c) `--info` / `--show` printers           |    30 | 955 `info`, 2469 `netconsole`, 3389 `passwd`, 3427 `pos`, 3467 `weather`/`wx` (+25)         | 7 L in the ladder; the real bodies are the 700-line tail :5530-6207 |
...
(:4759, :4783, :4875, :5167, :5177, :5192) and 2 cross-named pairs (:4086 `sethamnet on`/
`setinet off`, :4106 `setinet`/`sethamnet off`).
...
(`settime` :359-366, `setgrc` :5465), `IPAddress::fromString` once (`srvip` :5277), raw indexing
(`symid` :3530 `msg_text[8]`, `symcd` :3560). `strtol` is never used, so **no branch detects a
...
match `strlen("--" + name)`; `pingcall` :3697 already carries a copy/paste defect
...
| S3 deferred JSON register |    79 | sets `bNodeSetting`/`bSensSetting`/`bWifiSetting`/`bAnalogSetting`/`bInfo`, the tail :5530-6207 emits the frozen BLE JSON (TYP `TM W IO I SE S1 SW S2 G SN AN SA CONFFIN`) |
| S4 serial confirmation    |  ~254 | `printfdeb(...)` ×185 / `printlndeb(...)` ×69 — free-form, ~140 distinct format strings                                                                                    |
| S5 bench marker           |    37 | raw `Serial.printf`/`Serial.println` with `[TAG];k;v` (semicolons survive only outside `printfdeb`'s CSV stripping, :4486)                                                 |
...
- **BLE**: `phone_commands.cpp:248` `readPhoneCommand()` is a **separate binary opcode path**
  ...
  (callsign) and `0x55` (SSID/PWD) duplicate `--setcall` :3612 and `--setssid`/`--setpwd`
  :3770,3799 with _different_ validation: the binary path skips `checkRegexCall()`.
...
| #   | Finding                                                                                                                                                                                                                                                                          | Site            |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| 1   | `--setowndns ` handled **twice**; the second copy is unreachable (identical body, different offset `+11` vs `+12` — so the live one is :3966 with `+12`)                                                                                                                         | :3966 and :4034 |
| 2   | `--softser app0` unreachable: `"softser app"` :3252 is a prefix and returns first                                                                                                                                                                                                | :3261           |
| 3   | `else` missing between `setowngw` and `setownms`, and between `setl76k` and the softser block — chain silently splits into fresh `if`s (currently harmless, every predecessor `return`s)                                                                                         | :4008, :3123    |
| 4   | Commented-out branches still in the file: `compress ` :288-298 (commented out), `softser test0`/`softser test` :3274-3295 (commented out), `softser xml` :3358-3365 (commented out)                                                                                              | 4 blocks, ~35 L |
| 5   | `--spectrum` :710 body has two dead `#ifdef` blocks whose contents are commented out                                                                                                                                                                                             | :710-728        |
| 6   | Bench-only commands **outside** `INSTRUMENT_ENABLED`: `--specstart/--specend/--specstep/--specsamples` :4661-4748 (and `--specstep` writes `node_specsamples` while `--specsamples` writes `node_specstep` — swapped)                                                            | :4703, :4724    |
| 7   | `--ethstat/--wifistat/--udpstat/--udplog` are deliberately kept **outside** the guard (comment :4735-4739) — that is the **INS-01 lesson** (S4): a previous guard change swept these field diagnostics out of shipping builds. Any re-guarding must string-scan the built image. | :4752-4792      |
...
| ----- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | -----------------------------: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D2-01 | Delete the unreachable `--setowndns` duplicate                                   | command_functions.cpp:4034-4058                                                            | Remove the second `setowndns` block; keep :3966                                                                                   |                             25 | −120 B (1 `commandCheck` call ≈ 24 B + 11 B literal + ~85 B body)                                                                                                                                                                                                                                                                                    | 0                                                                                                                                                                                                        | L                                                                                     | none (branch is unreachable today)                                                                                                    | S      | full-command capture diff (§8)                                                                                                                                           |
| D2-02 | Fix `--softser app0` shadowing                                                   | command_functions.cpp:3252-3273                                                            | Move the `app0` head above `app`                                                                                                  |                              0 | 0                                                                                                                                                                                                                                                                                                                                                    | 0                                                                                                                                                                                                        | L                                                                                     | **changes behaviour**: `--softser app0` currently silently does `app`. Treat as a bug fix, not a refactor; needs an operator decision | S      | §8 capture + `--info` before/after                                                                                                                                       |
| D2-03 | Fix the swapped `--specstep`/`--specsamples` targets                             | command_functions.cpp:4703-4748                                                            | Swap the two `meshcom_settings` members                                                                                           |                              0 | 0                                                                                                                                                                                                                                                                                                                                                    | 0                                                                                                                                                                                                        | L                                                                                     | none (print strings unchanged); stored value changes                                                                                  | S      | §8 capture + read back via `config_json` export                                                                                                                          |
| D2-04 | Delete commented-out branches                                                    | :288-298 (commented out), :3274-3295 (commented out), :3358-3365 (commented out), :710-728 | Remove dead text                                                                                                                  |                            ~55 | 0 (already not compiled)                                                                                                                                                                                                                                                                                                                             | 0                                                                                                                                                                                                        | L                                                                                     | none                                                                                                                                  | S      | build all 32 envs, byte-compare `.bin`                                                                                                                                   |
| D2-05 | Normalise bit-clear spelling to `&= ~MASK`                                       | ~40 sites across :507-5202                                                                 | Replace `= x & 0x7Fxx` with `&= ~MASK`                                                                                            |                              0 | −0.2 kB (drops one 16-bit load per site)                                                                                                                                                                                                                                                                                                             | 0                                                                                                                                                                                                        | **M** — the `& 0x7Fxx` form also clears bit 15; auditing each mask is the work        | none                                                                                                                                  | M      | native unit test over `node_sset*` transitions + §8                                                                                                                      |
| D2-06 | Table-drive family (b), the 120 boolean toggles                                  | :457-5202 → new `src/command_table.cpp`                                                    | One `const CmdEntry` row per toggle: name, target bool*, settings word + mask, echo style/payload, optional post-hook fn          |                          ~1500 | **−6.0 kB** on a full-featured ESP32 build (≈90 in-build toggles × ~100 B of per-branch code — `commandCheck` call 24 B, bool store 10 B, RMW on the settings word 22 B, `if(ble)` + echo 20 B, `save_settings()` 4 B, epilogue 20 B — less 90 × 24 B of table rows, less ~1.4 kB of new dispatcher/handler code); **−3.5 kB** on a lean nRF52 build | **0** (table is `const` → `.rodata`/flash; keep it out of `.data` by declaring the row array `static const`, and on ESP32 give string literals no `PROGMEM` treatment — they are already flash-resident) | **M**                                                                                 | must be **none** — S1/S2/S3/S4/S5 payloads move verbatim into the row                                                                 | L      | §8, plus `--info`, `--seset`, `--wifiset`, `--nodeset`, `--analogset`, `--wx`, `--pos`, `--io`, `--tel`, `--aprsset` JSON captured over BLE before/after and byte-diffed |
| D2-07 | Table-drive family (a), the 71 `--set<X> <value>` branches                       | :321-5639 → same table                                                                     | Row carries type, member offset, min/max, `save` flag, reboot flag, echo fmt; the ladder body shrinks to a parse+range+store call |                          ~1100 | **−2.6 kB** (≈60 in-build entries × ~110 B saved on parse/range/store boilerplate, less 60 × 28 B rows; the per-branch `printfdeb` format strings stay, so no literal savings)                                                                                                                                                                       | 0                                                                                                                                                                                                        | **M/H** — each branch's exact `sscanf` offset and clamp semantics must be transcribed | must be **none**                                                                                                                      | L      | §8 + a native golden test on the parse/clamp helper alone (env:native)                                                                                                   |
...
void commandAction(char *umsg, bool ble) {
    ...
    /* fall through to the legacy ladder for families (c)(d)(e), then the tail :5530-6207 */
}
```
...
- **Removing the `bReturn`/`bNodeSetting`-style outer flags in the same pass.** They are the input
  to the 700-line printer tail (:5530-6207); untangling them is a separate audit item and would
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D3-01 | Field-extractor loop for `/X=value/` APRS-comment tags                                                                                 | src/aprs_functions.cpp:708-1201 (`decodeAPRSPOS`, ~11 blocks: bat /B=, alt /A=, press /P=, hum /H=, temp /T=, temp2 /O=, qfe /F=, qnh /Q=, gasres /G=, ncnt /N#, co2 /C=, version /V=, telemetry /Y=) | Replace each ~15-line "find tag, copy digits into `decode_text`, sscanf into field" block with one helper called 11x with (tag-char2, tag-char3-or-digit-range, fmt, dest-ptr)                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ~140 (11 blocks x ~15 lines -> 11 one-line calls + 1 ~20-line helper)                      | ~1.1-1.7 kB (rough, 140 lines x 8-12 B/line)                                                     | none (no new statics; helper uses same locals)                                                 | L                                                                                         | none                                                                                 | S      | bit-for-bit compare of `struct aprsPosition` produced from a corpus of real `/B=/A=/P=.../V=.../Y=` payloads, before/after                                                                                                     |
| D3-02 | `sendPing`/`sendAPPPosition`/`sendHey`/`SendAckMessage`/... "finalize and encode APRS message" epilogue                                | src/loop_functions.cpp:3327-3341 (sendPing), 4741-4755 (sendPosition), 4818-4832 (sendAPPPosition), 4970-4984, 5308-5322 (+2 more per S6 rank 29, 7 copies total)                                     | Extract `msgid++; wrap at 999; save_settings(); checkVia(aprsmsg); encodeAPRS(msg_buffer, aprsmsg); if(bDisplayInfo) printBuffer_aprs(tag, aprsmsg)` into `void finalizeAndSendAPRS(struct aprsMessage &aprsmsg, uint8_t *msg_buffer, const char *dbgtag)` — caller still does its own addTxRingEntry/addNodeData/addBLEOutBuffer after, since those vary                                                                                                                                                                                                                                                                         | ~55 (7 copies x ~9 lines -> 7x2-line calls + 1 ~11-line helper)                            | ~440-660 B (rough)                                                                               | none                                                                                           | L                                                                                         | none — helper produces byte-identical `msg_buffer` via unchanged `encodeAPRS()` call | S      | diff `msg_buffer[0..msg_len]` + `aprsmsg.msg_id` across all 7 call sites for identical inputs before/after                                                                                                                     |
| D3-03 | `addBLEOutBuffer` vs `addBLEComToOutBuffer` — two near-identical BLE ring-insert helpers                                               | src/loop_functions.cpp:583-646 (`addBLEOutBuffer`), 651-682 (`addBLEComToOutBuffer`)                                                                                                                  | Do NOT merge into one function — S5 freezes `BLEtoPhoneBuff` (0x91/0x44/type framing + 4-byte timestamp) and `BLEComToPhoneBuff` (plain length+payload, no timestamp) as two different wire shapes with different overflow-clamp values (`UDP_TX_BUF_SIZE-4`/`245`) and different write-pointer advance (`addRingPointer()` vs manual `ComToPhoneWrite++`). Only the manual advance in `addBLEComToOutBuffer` (664-681) is worth extracting: replace with the existing `addRingPointer(ComToPhoneWrite, ComToPhoneRead, MAX_RING, "com")` helper already used by the other function, dropping the duplicated overflow-reset block | ~10                                                                                        | ~80-120 B                                                                                        | none                                                                                           | L                                                                                         | none (only the internal pointer-advance mechanics change, not the buffer bytes)      | S      | unit-check `ComToPhoneWrite` sequence for MAX_RING+3 calls matches old manual logic                                                                                                                                            |
...
**D3-02** Finalize-and-encode epilogue. Signature: `void finalizeAndSendAPRS(struct aprsMessage &aprsmsg, uint8_t msg_buffer[UDP_TX_BUF_SIZE], const char *dbgtag)`. Varying part is only the `dbgtag` string passed to `printBuffer_aprs()`; everything else (`msgid` wrap, `save_settings()`, `checkVia()`, `encodeAPRS()`) is byte-for-byte identical across the 7 sites, confirmed by direct read of 3 of the 7 (sendPing:3327, sendPosition:4741, sendAPPPosition:4818).

**D3-03** Only the `ComToPhoneWrite++` / overflow-reset block (loop_functions.cpp:671-681) is real duplication — it reimplements `addRingPointer()` (already defined at loop_functions.cpp:5757 and already used by `addBLEOutBuffer` via line 626). Swap in the existing helper; do not touch the buffer-framing code above it (S5 (a): `BLEtoPhoneBuff` vs `BLEComToPhoneBuff` have different frozen byte layouts).
...
| D5-11 | Scattered `%02i:%02i:%02i` time formatting                                                                                      | `src/rtc_functions.cpp:117`, `src/loop_functions.cpp:1677,1679`, `src/command_functions.cpp:6078`, `src/phone_commands.cpp:428` (5 total call sites; task's grep estimate of "many" does not hold — `getTimeString()` at `src/time_functions.cpp` already has 16 callers and covers the common case) | Add one `static inline void fmtHms(char*buf,size_t n,int h,int m,int s)` for the 3 byte-identical `"%02i:%02i:%02i"` sites (`rtc_functions.cpp:117` is `"%02i.%02i.%i %02i:%02i:%02i"` — different, leave it; `loop_functions.cpp:1677/1679` are the true duplicate pair)                                                                                                                                                                                                                                                                                                                    | ~4                                                                                                                 | 0                                                  | 0                                                                                                     | L    | none (display/console text)                                                                           | S      | display-string snapshot diff on T-Deck/T5-epaper battery-line rendering                                                                                  |
...
| `DISABLE_TLS_CONSOLE`                             | `src/command_functions.cpp:5984`                                                                                                                     | never set; net-console TLS toggle stays always-on in every shipped build                                                                                                                                                             |
...
| `SX126X_V3`                                       | `src/command_functions.cpp:847,2943`                                                                                                                 | never set; likely a typo/alias for `SX126X` — worth a targeted check, not proposed as an edit here (touches radio chip selection, out of scope for a dead-code-only audit)                                                           |
...
- D2 defects CONFIRMED: command_functions.cpp:3966 and :4034 both handle "setowndns " (second unreachable);
  :3252 "softser app" precedes :3261 "softser app0" (prefix match -> app0 shadowed);
  :4703-4715 specstep writes node_specsamples, :4724-4736 specsamples writes node_specstep (swapped members).
