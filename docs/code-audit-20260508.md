...
| Severity | File | Line | Finding |
|----------|------|------|---------|
| CRITICAL | loop_functions.cpp | 2401 | `String charBuffer_aprs(...)` returns `String` by value (line shifted from 2205); caller in `lora_functions.cpp:454` invokes it in OnRxDone path — heap churn on every RX packet |
| CRITICAL | lora_functions.cpp | 454 | `memcpy(ringbufferRAWLoraRX[...], charBuffer_aprs(...).c_str(), UDP_TX_BUF_SIZE-1)` — String temp allocated inside OnRxDone execution path |
| MEDIUM | loop_functions.cpp | 176, 186 | Global `String strSOFTSER_BUF`, `String strTelemetry` used in main loop |
| MEDIUM | loop_functions.cpp | ≈multiple | Multiple temporary `String` concatenations in per-message processing |
| MEDIUM | aprs_functions.cpp | 192, 204, 209, 270, 280, 285, 344 | `aprsmsg.msg_source_path.concat(...)` — per-byte String growth during packet decode |

### MEM-03: C++ new without delete (memory leak)

| Severity | File | Line | Finding |
|----------|------|------|---------|
| CRITICAL | spectral_scan.cpp | 108 | `new uint16_t[RADIOLIB_SX126X_SPECTRAL_SCAN_RES_SIZE]` — returned pointer never freed |
| CRITICAL | spectral_scan.cpp | 188 | `new uint16_t[...]{0}` — same pattern in second branch |
...
|----------|------|-------|---------|
| CRITICAL | esp32/at_cmd.h | 30 | `AT_PRINTF` macro: unbounded `sprintf` into 255-byte buffer |
| CRITICAL | nrf52/at_cmd.h | 30 | Same `AT_PRINTF` macro |
| CRITICAL | nrf52/WisBlock-API.h | 658 | `API_LOG`: `int len = sprintf(buff, __VA_ARGS__);` unbounded |
| CRITICAL | esp32/esp32_main.cpp | 613, 842, 959-960, 1119, 1497, 1501, 1503 | 7x `sprintf` into settings/BLE name buffers (lines shifted from 785/895/896/1053/1430/1434/1436) |
| CRITICAL | esp32/esp32_functions.cpp | 83, 134, 141 | `sprintf` into 20-byte `cvers` (lines shifted +1) |
...
| MEDIUM | nrf52/nrf52_main.cpp | (shifted, ≈4 sites) | `sprintf` in init/log paths |
| MEDIUM | nrf52/nrf52_functions.cpp | 64, 196 | 2x `sprintf` into `cvers` |
| MEDIUM | nrf52/nrf52_ble.cpp | 93, 98, 122 | 3x `sprintf` |
| MEDIUM | command_functions.cpp | 4118 | `sprintf(meshcom_settings.node_parm, "%s", "none")` |

**strcpy() — 5 sites (HIGH)**

| Severity | File | Line | Finding |
|----------|------|------|---------|
| HIGH | web_functions/web_functions.cpp | 1800 | `strcpy(message_text, message.c_str())` into 200-byte buffer, unchecked source length (line shifted from 1797) |
| MEDIUM | onebutton_functions.cpp | 77, 81 | `strcpy` for page text buffers |
| LOW | Displays/BaseDisplay/SD.cpp | 368 | `strcpy` for hard-coded 4-byte filename suffix |
