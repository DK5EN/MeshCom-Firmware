...
refutations in §3), [`docs/code-audit-20260712.md`](../code-audit-20260712.md) (`SEC-02`…`BUG-13`),
...
`MAX_MHEARD`/`MAX_MHPATH` are gone — the MeshCom 5 cutover (`ccb3ec23`) replaced the MHeard buffer
and path table with the neighbour matrix, sized separately per family (§1.3).
...
### 1.3 Neighbour-matrix (MeshCom 5) per-family constants

Separate 3-branch ladder in `configuration_global.h:300-347` (own `NBR_FAMILY_*` marker, not the
`MAX_MHEARD` ladder above), sizes at `:464-515`:

| Constant         | `NBR_FAMILY_CLASSIC` (`ENABLE_XML`/`ENABLE_SBUFFER`, and the `else` branch — classic ESP32, `E22_XML`) | `NBR_FAMILY_LARGE` (`CONFIG_IDF_TARGET_ESP32S3` ∥ `BOARD_RAK4630`) | `NBR_FAMILY_DEV` (`ENABLE_TBEAM`) |
| ---------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------ | --------------------------------- |
| `NBR_MAX_ROWS`   | 64 (`:492`/`:346`)                                                                                     | 128 (`:466`)                                                       | 32 (`:479`)                       |
| `NBR_MAX_EDGES`  | 256 (`:495`/`:346`)                                                                                    | 512 (`:469`)                                                       | 128 (`:482`)                      |
| `NBR_EXT_SLOTS`  | 48 (`:498`/`:346`)                                                                                     | 64 (`:472`)                                                        | 16 (`:485`)                       |
| `NBR_HZ_ENTRIES` | 48 (`:501`/`:346`)                                                                                     | 112 (`:475`)                                                       | 16 (`:488`)                       |

`NBR_MASK_WORDS` (`nbr_mask.h:21-23`) is `1` at 64 rows, `2` at 128 rows — one `uint64_t` per 64
rows of `NbrMask`. `NBR_SHARE_PCT 10`, `NBR_CNT_HALVE_MIN 90`, `NBR_SNR_AVG_N 8` and
`NBR_NCNT_AIR_MAX 99` (`:504-515`) are shared across all three families.

Measured store bytes per family, before/after the wave 4 cutover (nm symbol sums,
[`docs/meshcom5-campaign.md`](../meshcom5-campaign.md) wave 4 table): classic (E22, T-Beam)
5,989 → 5,848 B; `E22_XML` 9,599 → 5,848 B; S3 (Heltec V3) 14,823 → 14,504 B; nRF52 (RAK4631)
16,575 → 14,504 B. Linker RAM against the wave-0 baseline: E22 95,584 → 95,456, Heltec V3
101,548 → 101,260, RAK4631 81,408 → 79,748.
...
| `bleBuffer` (MH JSON)       | `mh_phone.cpp:216`           | `[MAX_MSG_LEN_PHONE]`         | 300 stack | `={0}`            | yes — `bleJsonFrameFailSoft(doc, buf, len, BLE_JSON_PAYLOAD_MAX)` (`ble_json_frame.h:42`) caps the ArduinoJson output at 244 B, dropping the newest fields first on overflow |
...
### 2.4 Neighbour matrix / display

The MHeard buffer and path table (`mheard_functions.*`, `mheard_record.h`) were deleted in the
MeshCom 5 cutover (`ccb3ec23`); every board now stores one fixed-layout `NbrMatrix`
(`src/nbr_matrix.h:232-252`), sized per family (§1.3), with no `String` and no per-row `malloc`.
Field sizes are pinned by `static_assert` in `src/nbr_matrix.cpp:64-80`, so a padding regression is
a build error, not a silent size drift (contrast the old `MAX_MHEARD` ladder, §1.2).

| Buffer                                                         | Declared at                                | Size expression                       | Resolved                            | Zeroed?                | Bounded?                                                                                                                                                |
| -------------------------------------------------------------- | ------------------------------------------ | ------------------------------------- | ----------------------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NbrMatrix.call[NBR_MAX_ROWS]`                                 | `nbr_matrix.h:234`                         | `uint64_t[NBR_MAX_ROWS]`              | 8 B/row                             | static BSS             | fixed-width callsign word (`nbrCallEncode()`/`nbrCallDecode()`, `:334-335`)                                                                             |
| `NbrMatrix.row[NBR_MAX_ROWS]` (`NbrRow`)                       | `nbr_matrix.h:122-131,235`                 | 12 B/row, no padding                  | —                                   | static BSS             | index-checked accessor only (`nbrRowGet()`, `:318`); size pinned by `static_assert` (`nbr_matrix.cpp:64`)                                               |
| `NbrMatrix.hears`/`.heardBy[NBR_MAX_ROWS]` (`NbrMask`)         | `nbr_matrix.h:236-237`                     | `NBR_MASK_WORDS*8` B/row              | —                                   | static BSS             | bit ops only (`nbr_mask.h`); `NBR_MASK_WORDS` = 1 (64 rows) or 2 (128 rows)                                                                             |
| `NbrMatrix.edge[NBR_MAX_EDGES]` (`NbrEdge`)                    | `nbr_matrix.h:199-205,238`                 | 6 B/edge, no padding                  | —                                   | static BSS             | index-checked (`nbrEdgeGet()`, `:321`); freed by `nbrSweep()` after `NBR_WINDOW_MIN` (`:305-311`); size pinned by `static_assert` (`nbr_matrix.cpp:65`) |
| `NbrMatrix.ext[NBR_EXT_SLOTS][13]` (direct-slot bits)          | `nbr_matrix.h:219-221,245`                 | 13 B/slot                             | —                                   | static BSS             | only `nbr_matrix.cpp`/`nbr_views.cpp` touch it, via `nbrBitsGet()`/`nbrBitsPut()`; size pinned by `static_assert` (`nbr_matrix.cpp:72`)                 |
| `NbrMatrix.hz_call/hz_entry/hz_meta[NBR_HZ_ENTRIES]` (horizon) | `nbr_matrix.h:222-228,244,248-249`         | `(12 + 8*NBR_MASK_WORDS)` B/entry     | —                                   | static BSS             | size pinned by `static_assert` (`nbr_matrix.cpp:73-76`)                                                                                                 |
| `NbrMatrix.echo_*[4]` (own-TX echo)                            | `nbr_matrix.h:229-230,242-243,247,250-251` | 4 fixed slots                         | —                                   | static BSS             | size pinned by `static_assert` (`nbr_matrix.cpp:77-80`)                                                                                                 |
| `/topo.dat` save buffer                                        | `topo_ui.cpp:84-85`                        | heap, `malloc(nbrSaveSize())`         | `nbrSaveSize()` (`nbr_views.h:120`) | written by `nbrSave()` | `n == need` checked before the SD write (`:96`); `buf` freed unconditionally (`:109`); gated on `node_persist_to_sd` and a plausible clock (`:65,81`)   |
...
### F3-3 — `memcpy` sized by the _destination_ from an Arduino `String` source — heap over-read rebroadcast over the air — **HIGH, historical**

**Status (2026-09-26): no longer applies.** `mheard_functions.cpp` and `mheard_record.h`
(`struct mheardLine`) were deleted in the MeshCom 5 cutover (`ccb3ec23`); the neighbour matrix that
replaced them (§2.4) has no `String` fields to over-read. Kept below as the record of a fixed
defect in removed code.
...
**Status (2026-09-26):** `mheard_functions.cpp` was deleted in the MeshCom 5 cutover (`ccb3ec23`);
its 2 sites are gone, leaving the 13 sites in `command_functions.cpp` above. The MH JSON frame that
replaced it (`mh_phone.cpp:198`, §2.2) is bounded by `bleJsonFrameFailSoft()`, not `measureJson()`,
so this finding does not carry over.
...
### F3-22 — `strncpy` without room for the NUL in the VIA path — **LOW-MEDIUM, historical**

**Status (2026-09-26): no longer applies as cited.** `mheardCalls` was deleted with
`mheard_functions.*` in the MeshCom 5 cutover (`ccb3ec23`), and `via_functions.cpp`'s via-selection
code was rewritten in the same commit (currently a disabled placeholder, not yet reading the
topology — see `docs/meshcom5-campaign.md` wave 4, "stage 4 deferred"). Whether the rewritten code
repeats this NUL-termination bug needs a fresh check; not re-verified in this pass.
...
| b   | `mheard_functions.cpp:323-326`, `:451-454` — **historical, file removed in the MeshCom 5 cutover (`ccb3ec23`)** | `char cBuffer[60]; snprintf(...); memcpy(mheardBuffer[ipos], cBuffer, sizeof(cBuffer));` — copies the uninitialised tail after the NUL into BSS, and `:174` writes the whole array to SD → stack contents persisted to storage                                                                    |
...
| -------------- | --------------------------------------------------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1** MEM-01   | No `malloc`/`new` after init; none in packet paths                    | **VIOLATED (as of 2026-07-31; superseded, see status)** | `printfdeb_functions.cpp:103` mallocs inside the `OnRxDone` logging path (still true). **Status (2026-09-26):** the `String` allocation cited below no longer exists — `aprsMessage`'s text fields are now fixed `char[]` (`aprs_structures.h:70-76`, comment "R2-04", neo core commit `3b2fbb8c`), and `mheardLine`/`mheard_functions.cpp` were deleted in the MeshCom 5 cutover (`ccb3ec23`). This finding needs re-deriving against the current tree, not carried forward as-is                                                                                                                                                                                                                  |
| **1** MEM-01   | Never Arduino `String` in hot paths                                   | **VIOLATED (as of 2026-07-31; superseded, see status)** | `struct aprsMessage` and `struct mheardLine` were constructed per received frame in `OnRxDone` when this was written; `decodeAPRS` performed ≥ 6 heap assignments, `checkVia` 2 more. **Status (2026-09-26):** both structs are `String`-free now (see the row above); the `mheardCalls` mentioned here no longer exists                                                                                                                                                                                                                                                                                                                                                                            |
...
**Status (2026-09-26):** `mheard_functions.cpp` (and `mheard_record.h`, not listed above) were
deleted in the MeshCom 5 cutover (`ccb3ec23`), after this "unchanged" snapshot was taken. Its
citations below (F3-3, F3-18, F3-24b) are historical — see §2.4.
...
| F3-3 mheard heap over-read                   | **HISTORICAL — file deleted `ccb3ec23`**                               | `mheard_functions.cpp:517-533`. New: `mheardPathCalls` has no NUL slot reserved. Superseded by the neighbour matrix, §2.4                                             |
...
| F3-18 `measureJson`                          | **OPEN (13 `command_functions.cpp` sites); 2 mheard sites historical** | Report missed `mheard_functions.cpp:348`. Both mheard sites gone with the file (`ccb3ec23`)                                                                           |
...
| "14 `serializeJson` calls sized by `measureJson`"                                              | **undercount, since reduced**                    | Was 15 (`mheard_functions.cpp:348` was missed); now 13 — the 2 mheard sites were deleted with the file in the MeshCom 5 cutover (`ccb3ec23`)                                                                                                                                                              |
...
| CONC-15/16 (non-atomic ring indices)                                            | `docs/code-audit-20260712.md` | T3-8 restates the buffer-side consequence only; the concurrency analysis is `f2-concurrency.md`'s                                                                                                                                                                                                                                                                                                                                                                                                                                          |
