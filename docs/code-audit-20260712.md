...
**Auditor:** Claude Code (`/fable-review`, multi-angle finder pass with adversarial verification)
**Note:** previously `fable-verdict.md` in the repository root; renamed into the
`docs/code-audit-YYYYMMDD.md` series without content changes.
...
| SEC-04   | High          | CONFIRMED          | URL-decode loop overruns `msg_text_checked[200]` (writes to ~247); length guard runs _after_ the loop                                                     |
