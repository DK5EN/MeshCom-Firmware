...
| --------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BLE to phone                | `phone_commands.cpp`, `ble_json_frame.h`, `command_functions.cpp` tail (:5530-6207)                           | frame status bytes 0x40/0x44/0x91, opcodes 0x10..0xF0, 27 JSON keys, MTU budget 244                                                                                                                                 |
...
| Mesh UDP 1990               | `udp_functions.cpp`, `nrf52/nrf_eth.cpp`, `conf_frame.cpp`                                                    | `GATE`/`BEAT`/`CONF` indicators, payload type at +4, CONF TLV tags 0x00-0x04                                                                                                                                        |
...
| R1-04       | `ringbufferRAWLoraRX` allocated on first web rxlog request, writer no-ops on NULL                             | `loop_functions.cpp:429-431,3217-3237`, `lora_functions.cpp:697`              | **5,200** / 2,600 / 2,600                                    | M    | M      | yes                  | rxlog page unchanged; 12 h nRF52 soak with webserver off (writer runs on timer task, C10)                |
| R1-02       | Merge `BLEComToPhoneBuff` into `BLEtoPhoneBuff` (byte 1 already tags the type), one lock, one overflow policy | `loop_functions.cpp:442-451,581-677`, `phone_commands.cpp:60-210`             | 4,980 / 4,980 / 4,980                                        | M    | M      | yes                  | interleaved config-reply and text burst drain in FIFO order; web history shows text only                 |
| R4-02/03    | Skip U8g2 objects, shared 1 kB frame buffer and page-history ring on boards without an OLED (E22 family)      | `loop_functions.cpp:362-404,776-780`                                          | 4,304 (no-OLED boards) / - / -                               | M    | S      | yes, with board list | per-board hardware confirmation of "no OLED"; E22 already runs `bDisplayOff`                             |
...
| R1-01       | BLE slots sized to the producers' clamps (260 and 246 instead of 305)                                         | `loop_functions_extern.h:267,272`, `loop_functions.cpp:587-590,652-657`       | 2,080 / 2,080 / 2,080                                        | L    | S      | yes                  | `--bledebug` frame dump byte-identical for max-length text, JSON, mheard                                 |
...
| R2-04       | `aprsMessage`/`mheardLine` callsign and path `String` fields to fixed `char[]`                                | `aprs_structures.h:20-26,77-83`                                               | heap churn −450..550 B and ~14 malloc/free per RX (est.)     | M    | M      | yes                  | aprs native tests green; `--heap` watermark over 1 h; BLE connect soak (NimBLE precedent)                |
| R3-05/08/09 | `const String&` parameters on the RX path, `getTimeStr(char*,n)` for hot log lines                            | `aprs_functions.cpp:28,53,563`, `loop_functions.cpp:3144`                     | heap churn only (est.)                                       | L    | S/M    | yes                  | native tests; heap trace over a 100-frame inject                                                         |
...
| R1-07       | Byte-pool slab for the BLE ring                                                                               | `loop_functions.cpp:581-677`                                                  | superseded by R1-02                                          | H    | L      | no                   |                                                                                                          |
...
| D2-06       | Table-driven boolean toggles: 120 `on/off` branches (58 matched pairs) become `const CmdEntry` rows with style and payload columns; six echo styles exist, so the row carries which one                                                                                                                                                   | `command_functions.cpp:457-5202`, new `src/command_table.cpp`                                                                                                                |                          ~1,500 | **−6.0 kB** ESP32, −3.5 kB nRF52 | M    | L      | golden capture D2-V (section 7.3); native `test_command_table` for shadowing and offsets                                                                    |
| D2-07       | Table-driven setters: 71 `--set<X> <value>` branches, one parse/range/store helper                                                                                                                                                                                                                                                        | `command_functions.cpp:321-5639`                                                                                                                                             |                          ~1,100 | −2.6 kB                          | M/H  | L      | D2-V; each branch's `sscanf` offset and clamp transcribed and tested                                                                                        |
...
| D1-01       | Shared GATE/CONF/BEAT frame handler `handleMeshComUdpFrame()`; platforms keep socket read and link reset behind a plain `McNetOps` function-pointer struct (no vtable)                                                                                                                                                                    | `udp_functions.cpp:205-638` vs `nrf52/nrf_eth.cpp:367-839` (189 identical of 435/473)                                                                                        |                            ~350 | −0.3 kB, −260 B nRF52 stack      | M    | L      | byte-compare UDP 1990 TX and BLE frames on RAK4631 and Heltec V3 via the injectraw EXTUDP recipe                                                            |
...
| D3-01       | One `aprsExtractTag()` for the 11 `/X=value/` extractor blocks in `decodeAPRSPOS()`                                                                                                                                                                                                                                                       | `aprs_functions.cpp:708-1201`                                                                                                                                                |                            ~140 | −1.1..1.7 kB                     | L    | S      | `aprsPosition` bit-compare over the field corpus                                                                                                            |
...
| D1-02                   | nRF52 hand-built ACK phone frame via the shared `buildAckPhoneFrame()`                                                                                                                                                                                                                                                                                              | `nrf52/nrf_eth.cpp:578-596` vs `ack_attribution.h:122-137`                                                                   |      14 | L    | closes a drift bug (section 6)                                                                           |
| D3-02                   | `finalizeAndSendAPRS()` for the 7 identical msgid/save/checkVia/encode epilogues                                                                                                                                                                                                                                                                                    | `loop_functions.cpp:3327,4741,4818,4970,5308` and 2 more                                                                     |      55 | L    | `msg_buffer` byte-diff at all 7 sites                                                                    |
| D3-03                   | `addBLEComToOutBuffer` uses `addRingPointer()` like its twin (its raw `Write++` wrap silently overwrites unread slots)                                                                                                                                                                                                                                              | `loop_functions.cpp:664-681`                                                                                                 |      10 | L    | subsumed by R1-02 if that ships                                                                          |
...
| D5-11                   | `fmtHms()` for the two identical `%02i:%02i:%02i` sites                                                                                                                                                                                                                                                                                                             | `loop_functions.cpp:1677,1679`                                                                                               |       4 | L    | the "many time formatters" premise did not hold; `getTimeString()` already covers 16 sites               |
| D2-01/04                | Delete the unreachable second `--setowndns` block and ~55 lines of commented-out branches                                                                                                                                                                                                                                                                           | `command_functions.cpp:4034-4058, 288-298 (commented out), 3274-3295 (commented out), 3358-3365 (commented out), 710-728`    |      80 | L    |                                                                                                          |
...
| D2-09                   | Move `--specstart/end/step/samples` behind `INSTRUMENT_ENABLED`                                                                                                                                                                                                                                                                                                     | `command_functions.cpp:4661-4748`                                                                                            |       0 | M    | operator decision; string-scan the image afterwards (INS-01)                                             |
...
| 2   | `--setowndns ` handled twice; the second block is unreachable                                                                                                                        | `command_functions.cpp:3966` and `:4034`                                       | verified                                             | dead code                                                                    |
| 3   | `--softser app0` shadowed by the prefix match on `--softser app`                                                                                                                     | `command_functions.cpp:3252`, `:3261`                                          | verified                                             | `app0` silently runs `app`; fix changes behaviour, decision                  |
| 4   | `--specstep` writes `node_specsamples`, `--specsamples` writes `node_specstep`                                                                                                       | `command_functions.cpp:4703-4736`                                              | verified                                             | stored values swapped; fix changes stored value, decision                    |
...
| 7   | nRF52 UDP zero-scan reads `buf[i+1]` past the datagram on odd sizes; ESP32 has `i + 1 < packetSize`                                                                                  | `nrf52/nrf_eth.cpp:418` vs `udp_functions.cpp:214`                             | verified                                             | one-byte over-read                                                           |
...
| 11  | `--pingcall` bounds the write into `node_pingcall` with `sizeof(node_call)`                                                                                                          | `command_functions.cpp:3697`                                                   | verified                                             | harmless today (both 10 B), wrong expression                                 |
| 12  | nRF52 hand-built ACK phone frame sends `out[6]=0`, never the attribution suffix ESP32 sends                                                                                          | `nrf52/nrf_eth.cpp:578-596` vs `ack_attribution.h:122-137`                     | analyst report                                       | phone app gets no attribution callsign from nRF52 gateways                   |
| 13  | RX-01 unconfigured-source guard, `bGATEWAY_NOPOS`, `sendDisplayPosition()` for server frames, `hb_warn_logged` reset, `decodeAPRS()` return check: each present on one platform only | `udp_functions.cpp:287,339` vs `nrf52/nrf_eth.cpp:483` etc. (appendix D1-01)   | analyst report                                       | an nRF52 gateway can radiate XX0XXX GATE frames; see D1-01 for the full list |
...
- **Byte-compare on every wire surface** per changed row: BLE frame dumps, UDP 1990 hex on the
