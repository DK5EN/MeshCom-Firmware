...
## Verdict: APPROVED (after re-check of 4a569e6f; the original verdict on 7aeb2ac5 was REWORK)
...
Diff `7aeb2ac5..4a569e6f`. Native re-run: `native` 15/15 (`test_reack_limiter` 8,
`test_dm_stats` 7), `native_aprs:test_txring` 27/27.

| Finding | Status                 | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F1      | CONFIRMED fixed        | Hook moved to `src/lora_functions.cpp:575-586`, after the ch_util window close (`:469-477`), the nRF52 buffer copy/re-arm/CAD abort (`:482-538`) and before `handleACK()` (`:589`). Teardown is byte-for-byte the `handleACK` return path (`:592-609`): `rxBufInUse` release under critical section, `is_receiving = false`, `iReceiveTimeOutTime = millis()`, `csma_timeout = csma_compute_timeout(cad_attempt)`. Nothing else on the normal tail (`:1672-1685`) matters for the next RX/TX: the remaining tail items are the ONRXDONE_TIME debug counter and `test_inject_service()`, both diagnostic. Still skipped by design: `captureFrame()` runs before the hook (`--loradebug` capture shows the airgapped frame; acceptable, it is a debug tap, not mheard/dedup). |
| F2      | CONFIRMED fixed        | `:1621-1622` adds `!startsWith("{ping}") && !startsWith("{pong}")`, matching the original path's `:1024`/`:1041`. `{mcp}` payloads still pass the branch, but `{mcp}` frames carry `{NNN` from `sendMessage()` and are acked on first receipt today, so re-acking them is consistent with the original gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| F3      | CONFIRMED fixed        | `:921-924`: echo counts only TEXT with `indexOf("{", 1) > 0` and no `:ack`. Own ACKs (no brace) and plain broadcasts (no `{NNN`) drop out; the remaining residue is an own `{pong}{...}` echo, negligible.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| F4      | CONFIRMED fixed        | `test/test_txring/test_txring.cpp:1035-1055` exercises DONE/READY/EXT_PENDING with length 10, each expecting 0; a `len != 0`-only implementation now fails. `test/test_reack_limiter/test_main.cpp:77-84` pairs `DK5EN-1`/`DK5EN-10` (shared 7-char prefix), which a short-prefix comparator fails.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| F5      | CONFIRMED fixed        | `src/dm_stats.cpp:42-52` replaces `sent_ms` on a repeated NNN; `dm_stats.h:38-44` now states the contract (note once per message, first attempt only). Test renamed and inverted (`test_main.cpp:108-127`), asserts bucket 0 from the second time.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| F6      | CONFIRMED fixed        | `:928-930`: `!= 0x02 && != 0x03` before the HEARD write.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| F7      | CONFIRMED fixed        | `!aprsmsg.msg_server` removed from the branch condition (`:1619-1622`); the two gates now agree.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| F8      | Partially fixed (open) | `src/dm_stats.h:26` now says 30 s. `test/test_reack_limiter/test_main.cpp:66` still reads "60s-Fenster". Comment only, no code effect; fix in passing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

No new findings. Bench items T-0.1..T-0.5 and the F1 RAK check (`--airgap on`, one RX,
`--airgap off`, next beacon transmits with no intervening RX) remain open as before.
