...
- **Fix**: Early-Return `if (!g_ble_uart_is_connected && !bWEBSERVER) return;` in beiden Funktionen. Daten werden nur gepuffert wenn ein BLE-Client oder der Webserver aktiv ist. Der `BLEtoPhoneBuff` Ringpuffer wird sowohl von BLE als auch vom Webserver (`sub_content_messages()`) gelesen — ein reiner BLE-Guard hatte die Web-UI-Anzeige blockiert.
...
| `src/nrf52/nrf52_main.cpp` | CSMA/CA, Logging, Heap-Monitoring, Deferred-Display, ONRXDONE_STATS |
| `src/lora_functions.cpp` | Retransmit, ACK, CAD, handleACK(), Helper-Funktionen, Double-Buffer, nRF52-Port, Deferred-Display, queueExtern |
| `src/loop_functions.cpp` | Dedup-Puffer, CSMA-Konstanten, volatile Flags, addTxRingEntry(), BLE-Guard fuer Phone-Buffer |
